...
### 2026-09-12 — THE FABLE ARBITER IS WIRED, AND ITS COST IS CAPPED AT ONE ARBITRATION PER REBASE. The build-escalation arbiter had zero production call sites; it now has one. **Supersedes the 2026-09-11 entry below in one respect only** — that entry lists the arbiter rule among the gates that are "ahead of every shipped system and stay", which was true of the rule and not of the code: `buildFableArbiter` was built, unit-tested, exported and constructed nowhere. Everything else in that entry stands. Detail: [`.trident/as-built/wire-fable-arbiter.md`](.trident/as-built/wire-fable-arbiter.md).

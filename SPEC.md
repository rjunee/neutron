...
- **What happens to a row nothing constructs.** Its pane keeps running, its row stays, and the next construction of that substrate reconciles it. The pane is labelled and visible in herdr rather than reparented and invisible, and the pre-existing orphan path still covers it if it wedges. This is a residual and is named as one in the spec item.

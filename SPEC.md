...
**So the spec said replace and the work said repair, and the work is right.** With the cutover blocked on a passing live cycle, repairing the system in front of you beats rewriting it. This entry exists so the method is chosen rather than drifted into — the failure this repo keeps hitting is a marker that records when it was written rather than what is true, and "replace the loop" had become exactly that.

**What does not change:** the acceptance in `docs/spec-items/the-orchestrator-owns-the-build-loop.md`, deliberately written as *properties* — the project REPL runs the build; bounded work splits on MODEL; a headless worker never talks to the owner; exactly one place asks him a question — each with a positive control. Those bind a repaired loop exactly as they would a rewritten one, which is why the correction touches wording and not criteria. The owner's definition of flowing (`docs/getting-to-flow.md`) is untouched: a card is dispatched and reaches merged with no human touching it.

**Consequence for prior art:** `obra/superpowers` (289k★, MIT, Anthropic's official plugin marketplace), pointed at #545 on 2026-09-22, is reading material for the same-model dispatch branch — not a rewrite to port. Its subagent mechanics assume Claude Code's native `Agent` tool and have no cross-model equivalent.

### 2026-09-19 — Reconcile authorized live-chat project survivors before serving traffic.

The locked requirement that a gateway restart restores project REPLs with their
conversation intact includes recovery without a new app turn. This supersedes
the lazy-construction residual of the 2026-09-13 entry for Open's `cc-agent-*`
sessions: after the production graph binds its tools, boot derives exact keys
from the current owner, General and non-deleted projects, their selected
providers, and authorized credential identities. Only matching durable Claude
pane rows enter adoption, each under its own options and a spawn-time auth
fingerprint matching the current credential. No synthetic chat turn is
sent and an absent or dead candidate does not arm supervision or spawn a REPL.
Other substrate families retain lazy reconciliation; all existing identity
probes, claims and fences remain binding. Acceptance lives in
[`a-gateway-restart-keeps-the-project-repls`](docs/spec-items/a-gateway-restart-keeps-the-project-repls.md).
...
### 2026-09-14 — CODEX-CLI 0.154.0 RETURNS NON-ZERO FOR DAEMON CONTROL-SOCKET FAILURES, SO #637'S PROPOSED EXIT-0 WRAPPER GUARD IS CLOSED WITHOUT RUNTIME CODE. Supersedes only the exit-status observation in the 2026-09-12 recurring cross-model-work entry below; its headless-per-call decision and all other measurements stand. Re-derived with no pipe: `codex app-server daemon version` exits 1 when a 107-byte derived pathname reaches the failed connect and exits 1 with `path must be shorter than SUN_LEN` at 108. An ordinary missing-socket case also exits 1. Production TypeScript and shell callers contain no daemon invocation to harden; the existing Codex wrappers invoke `codex exec`, and already treat its non-zero return as failure. GitHub issue #743.
...
- **A PROCESS HOLDS ITS CLAIM BEFORE IT IS CAPABLE OF TOUCHING THE TRANSCRIPT, not before it publishes.** An adopted pane becomes capable at output delivery and the detector set — from that moment a screen can be delivered and a detector can ANSWER it — so the claim is taken first and the wrapper is blind and mute until it succeeds; priming does not cover this, because it latches what was on the FIRST screen and the hazard is a fresh prompt arriving during the race. A fresh spawn becomes capable the instant `claude --resume` starts, since that process appends through startup and readiness and killing it afterwards does not unwrite what it appended — so the spawn RESERVES the session key under the registry lock before any process exists, and the loser never spawns. Both reservations are the `respawn_in_flight_at` idiom (marker, holder, pid, TTL derived from the claim's window, compare-and-set, fail-closed) and both release on every path that stops owning.
- **A PANE IS OWNED BY WHOEVER SERVES IT, not by how that session came to exist.** A freshly spawned REPL CONTENDS for its pane's claim in the same write that records the handle, through the same predicate the adoption compare-and-set uses — writing a claim is not contending for one, and a spawn that only wrote let two gateways serve one transcript. A pane cannot be claimed before it exists, so the loser of that contest kills the child it just spawned and refuses the turn retryably: killing costs one respawn, leaving it alive costs a second owner. A claim stamped with this process's own pid never blocks it, or a replacement spawn would refuse itself on the strength of its dead child's claim — while only adoption claimed, a spawner served a pane it had not claimed, an adopter could take it out from under a live gateway, and the spawner could not notice, because a session with no claim has nothing to renew. Clearing or replacing a handle carries its claim with it, so no ownership marker outlives the child it was taken for; the reverse does not hold, because a handle with no claim is the load-bearing state a surviving shutdown leaves behind and the next boot adopts. The handle and the claim are written in ONE module through four named transitions, and a test fails the build if any other module writes either — the two defects above were four careful call sites, which is what a fifth path would be. **Every ownership write also goes through the one entry point that consumes the lock outcome**, whose failure disposition is a required parameter rather than a rule to remember: the registry lock runs its callback even when it was not acquired, so a write that ignores that rewrites the whole file from a snapshot nobody had the right to read. A child exit that cannot hold the lock leaves the row alone (recoverable); a spawn that cannot record its pane refuses and ends the child (an unrecorded live pane is not). **That refusal carries the `repl_unreconciled` class, stamped on the thrown error rather than inferred from its wording** — an unclassified retryable spawn error is mapped by the composer to a synthetic 429, which would cool the credential the caller just picked: a local registry-lock failure spending provider capacity. Every refusal this subsystem raises joins that class; none of them is the provider's fault.
- **One owner is enforced by a WRITE, not by a comparison.** Two gateway incarnations reconciling one row both read the same restored `child_generation` and the same pane pid, so a check that only READS an unchanged row cannot tell them apart and both would attach. The row claim is a compare-and-set under the registry lock — a marker carrying the claimant's identity and the time — and the loser refuses with a reason distinct from "the row moved" and from "I could not find out". The marker is RENEWED by the owner's supervision tick, so an unexpired claim means a claimant that was alive recently rather than one that claimed recently — a bare age let a healthy owner's claim expire underneath it and handed a live pane to the next gateway to boot. The refresh is the same compare-and-set, so a gateway that was legitimately taken over cannot re-assert ownership; the marker also carries the claiming gateway's pid, so a claimant whose process is provably gone is taken over at once rather than after the threshold, and the threshold is a multiple of the renewal interval rather than an independent number. It is released by every path that stops owning a session, so a claimant that dies mid-claim costs one bounded refusal rather than a pane nothing can ever adopt again. **And a gateway stops serving a pane it can no longer PROVE it owns** — the deadline is measured from its own last confirmed renewal, not from observing a takeover, because the failure that costs a lease is the failure that hides who took it; it is derived from the takeover window by subtracting one renewal interval, so the old holder stops at least a tick before any other gateway may claim. **A gateway that LOSES its claim stops serving that pane** — the renewal's `not-ours` answer fences the session: the wrapper detaches, its own pool entry and registrations go, and turns for that key are refused through the same gate that refuses every other unestablished owner. It does NOT close: the winner's REPL is live, and a loser that closed on its way out would destroy the conversation the takeover just preserved.
...
- **It ships with NO filesystem tools, and that remains its primary confinement.** The turn is granted nothing — not Bash, and not `Read`/`Glob`/`Grep` either. Removing write tools does not prevent DISCLOSURE: `Read` alone lets repository-authored input aim a read outside the worktree while the verdict channel carries the answer out. The profile now also uses restricted mode with `dontAsk` (2026-09-14 decision above), so any future file-tool grant remains cwd-confined and settings enforcement stays active — but the tool-less surface, not the flag, is still what makes the arbiter safe. Toolless is still the stronger arbiter design: the caller assembles and folds every piece of evidence, so **the caller controls exactly what the judge can see**. If it ever cannot decide, add a field to the folded evidence, never restore a tool.

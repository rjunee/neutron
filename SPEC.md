...
### 2026-09-12 — A GATEWAY RESTART KEEPS ITS PROJECT REPLS. The shutdown kill that has terminated the whole warm pool since ISSUES #217 is now GATED: a herdr-hosted child is left running ONLY when a persisted registry row names its exact pane and its exact generation, and the next gateway re-adopts it or closes it. The converse does not hold and the entry does not claim it: a spawn still settling when shutdown reaches the pool is killed when it resolves, whatever row it went on to write (#674) — the conservative direction, costing one `--resume`. **Narrows, and does not supersede, the ISSUES #217 rule recorded against the 2026-06-11 orphan incident**, which stays in force for every other child. Spec item: [`docs/spec-items/a-gateway-restart-keeps-the-project-repls.md`](docs/spec-items/a-gateway-restart-keeps-the-project-repls.md). GitHub issue #539 (herdr step 2c).

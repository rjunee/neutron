...
- **The kill criterion, stated as a criterion — and with the dimension it will be judged along.** `merge_conflict_arbiter_retry_outcome` records whether the retry the arbiter granted actually resolved or escalated anyway; `merge_conflict_arbitration` records every arbitration and its classified decision, so the ratio has a denominator. Both lines also carry the CONFLICT SIZE (`conflict_files`, `hunk_raw_bytes`, `hunk_truncated`), because the hunk payload is bounded at 4 KiB and a larger conflict reaches the judge as a fragment it is told to escalate on — so the tier's useful range is SMALL conflicts, plausibly the range the bounded resolver already handled. **If resolutions cluster entirely on conflicts the judge could see in full, the real claim is "rescues the easy end of escalations", not "rescues escalations"** — and the answer then is the one already written: **stop offering the retry, do not raise the bound and do not re-open the channel.**

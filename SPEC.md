...
- **The kill criterion, stated as a criterion — and with the dimension it will be judged along.** `merge_conflict_arbiter_retry_outcome` records whether the retry the arbiter granted actually resolved or escalated anyway; `merge_conflict_arbitration` records every arbitration and its classified decision, so the ratio has a denominator. **The criterion is a BOOLEAN before it is a magnitude: did the judge see the whole conflict?** `hunk_truncated` answers it, with `conflict_files_omitted` naming what was left out; `hunk_shown_bytes` is what the judge was actually sent, and is deliberately not a pre-bounding total — when `hunk_truncated` is false the two are the same number, so the metric is exact precisely where the criterion turns on it. The hunk payload is bounded at 4 KiB, so a larger conflict reaches the judge as a fragment it is told to escalate on, which narrows the tier's useful range to SMALL conflicts — plausibly the range the bounded resolver already handled. **If resolutions only ever happen when nothing was withheld, the real claim is "rescues the easy end of escalations", not "rescues escalations"** — and the answer then is the one already written: **stop offering the retry, do not raise the bound and do not re-open the channel.**
- **PRE-COMMITTED READING OF THE `truncated: false` CLUSTER — decided before the data arrives, so the result cannot be argued with when it is inconvenient.** If resolutions cluster on `truncated: false`, that does **not** mean "raise the bound". It means `truncated` was never merely the kill criterion but the feature's actual PRECONDITION — the arbiter is useful only on conflicts that fit — and the payload is doing less work than its complexity implies. The right shape then is to **invoke the arbiter only when the payload is complete and delete the truncation machinery entirely**: the per-file and whole-evidence notices, the per-file budget, the omitted-files accounting, and the withholding owner that exists to keep them honest. That is a SIMPLIFICATION reached by measurement rather than a defeat — the feature gets smaller, the disclosure path that produced three defects in three rounds stops existing, and what remains is a judge that either sees the whole conflict or is never asked. A fourth disclosure defect arriving before that data reaches the same conclusion by a different route, and is to be taken the same way.

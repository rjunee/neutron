...
### 2026-09-16 — ADOPTION REQUIRES AN OBSERVABLE BASELINE, AND PANE LOSS IS A RETRYABLE OUTCOME (#1095).

An attached wrapper is not enough evidence to publish an adopted REPL: the pane must deliver one readable screen within the bounded adoption window so detector state has a real baseline. A pane that disappears or remains unreadable is closed and cleared, allowing the waiting dispatch to resume on a fresh child. If a positively identified pane disappearance happens during a turn, the substrate reports `pane_vanished`; the scheduled wakeup retries once immediately, while wall-clock composition expiry reports `compose_timeout` and genuine caller cancellation remains `aborted`. The wakeup summary groups terminal failures by those typed reasons.
...
- **The one defect here that ACTS rather than fails is the adopted pane's first screen.** Every detector latch is in-memory, so a stale tool-approval prompt still on the pane would read as a rising edge and be answered `1`+Enter — on the owner's session. Adoption therefore waits for one readable first screen; without one the pane is closed and the turn cold-resumes. That first screen is a baseline, not a stimulus: latches are primed against it, so signatures already present can fire only after falling and rising again.

...
test('the live 0125 incident boots, applies later migrations, and leaves row 125 untouched', () => {
  ...
  expect(result.applied).toEqual([127, 130, 131, 132, 133, 134, 135, 136, 137, 138, 139, 140, 141, 142, 143, 144, 145, 146, 147, 148, 149, 150, 151, 152, 153, 154, 155, 156, 157, 158, 160])
  ...
  // that put it back, so its absence was the correct expectation for as long as
  // nothing read it (`0131_code_trident_runs_base_sha_repair.sql` says so in
  // as many words: "unused on main"). Dependency-aware dispatch reads it, so
  // 0139 restores it and this replica must now come back with it — a live
  // database that boots without it would fail every file-contention query.
  expect(columns).toContain('claimed_paths')
  expect(
    db
      .query<{ version: number; name: string }, []>(
        'SELECT version, name FROM _migrations WHERE version IN (122, 124, 125) ORDER BY version',
...
test('without the 125 entry the run no longer needs one — it applies 0125 and boots', () => {
  ...
  expect(result.applied).toEqual([125, 127, 130, 131, 132, 133, 134, 135, 136, 137, 138, 139, 140, 141, 142, 143, 144, 145, 146, 147, 148, 149, 150, 151, 152, 153, 154, 155, 156, 157, 158, 160])

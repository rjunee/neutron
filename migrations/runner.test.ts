...
test('first apply runs all migrations in order and records them in _migrations', () => {
  ...
  // 0109 — the vestigial pre-`app_chat_messages` cluster is GONE. This used to
  // assert `sessions` EXISTS; it is inverted deliberately. `messages`,
  // `sessions` and `meters` were dead remnants of 0001/0003 with no production
  // reader, and the empty `messages` table sitting beside the live
  // `app_chat_messages` caused a real misdiagnosis (2026-07-27: queried the dead
  // table, reported "this instance has no chat messages" while 38 rows sat in
  // the live one). Asserting their ABSENCE keeps them from creeping back.
  for (const dead of ['messages', 'sessions', 'meters', 'messages_fts']) {
    const row = db
...
// so the declared REFERENCES + CASCADE constraints were inert and orphaned child rows could
// survive their parent. Originally asserted against `messages`/`sessions`; those were dropped as
// vestigial in 0109, so this now exercises a LIVE relationship
// (`task_reminder_links.reminder_id -> reminders`). The invariant under test is unchanged: a
// fresh connection must enforce foreign keys.
test('applyMigrations enables FK enforcement on every fresh connection', () => {
  ...
  expect(() => {
    db.run(
      `INSERT INTO task_reminder_links (task_id, reminder_id, project_slug, created_at)
       VALUES (?, ?, ?, ?)`,
      ['no-such-task', 'no-such-reminder', 'general', new Date().toISOString()],
    )

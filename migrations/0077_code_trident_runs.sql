...
--
-- This table is the SQLite translation of the legacy harness's `/trident` skill
-- state file (`~/legacy/gateway/trident-<slug>.state.json`). Where the legacy harness
...
--
-- Column rationale (mirrors the the legacy harness state-file schema, SKILL.md
-- "## State file"):
--
...
--
-- * `slug` — the slugified task (the legacy harness's state-file key). UNIQUE per
...
--   matches the the legacy harness state-file + the `tasks` table's TEXT timestamps).

 ...
 *
 * This card's as-built entry publishes the canonical SQL for "how many
 ...
 * entry's own heading, so a `sql` block belonging to any other entry in the same
 * file can never be picked up instead.
 */
function documentedStatements(): string[] {
  const doc = readFileSync(entryPath(), 'utf8')
  const heading = doc.indexOf(ENTRY_HEADING)
...
describe("the as-built record's published counts are the classifier, executed", () => {

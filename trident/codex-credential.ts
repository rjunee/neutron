...
export class CodexCredentialService {
  ...
  private async connectAccountSerialized(
    ...
    pasted: unknown,
    opts?: { slot?: string; label?: string | null },
    // `replaced` rides on BOTH signatures: the serialized inner call is what computes
    // it, and the public wrapper simply forwards whatever the inner call returned.
  ): Promise<CodexConnectResult & { slot?: string; replaced?: boolean }> {
    ...
    //
    // AFTER the duplicate guard above, deliberately: a paste that is refused as
    // `duplicate_account` never wrote anything, so it must not be reported as having
    // replaced a seat.

...
test('G100 resolves full and abbreviated conflict, pushes pinned object and witnesses before refusal', async () => {
  for (const claim of [other, other.slice(0, 7)]) {
    const f = fixture()
    expect(await checkBuildClaim(f.run, 'repo', 'change', claim, snapshot)).toMatchObject({ kind: 'blocked', on: expect.stringContaining('branch preserved') })
...
test('G100 same or nonexistent claim allows; uncertain resolution stays unknown', async () => {
  for (const [result, kind] of [[ok(head), 'allow'], [ok('', 1), 'allow'], [ok('', 128), 'unknown'], [ok('short'), 'unknown']] as const) {
    ...
    expect((await checkBuildClaim(run, 'repo', 'change', 'aaaaaaa', snapshot)).kind).toBe(kind)
...
test('G100 unobserved preservation never claims a preserved conflict', async () => {
  for (const failure of ['push', 'receipt', 'remote', 'malformed'] as const) {
    ...
    expect((await checkBuildClaim(run, 'repo', 'change', other, snapshot)).kind).toBe('unknown')
  }
})
test('G100 an existing preservation receipt needs no push', async () => {
  const f = fixture(); f.setRemote(head)
  expect((await checkBuildClaim(f.run, 'repo', 'change', other, snapshot)).kind).toBe('blocked')
  expect(f.calls.some(c => c.includes('push'))).toBe(false)
})

test('G100 invalid head or branch and thrown host calls stay unknown', async () => {
  const f = fixture()
  expect((await checkBuildClaim(f.run, 'repo', 'change', other, { ...snapshot, head: 'short' })).kind).toBe('unknown')
  expect(f.calls).toHaveLength(0)
  expect((await checkBuildClaim(async () => ok('', 1), 'repo', 'bad ref', other, snapshot)).kind).toBe('unknown')
  expect((await checkBuildClaim(async () => { throw Error('offline') }, 'repo', 'change', other, snapshot)).kind).toBe('unknown')
})

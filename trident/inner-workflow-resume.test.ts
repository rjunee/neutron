...
describe('mid-loop resume — the head UNCHANGED fast paths actually SKIP work', () => {
  ...
  test('the regenerated diff resolves its BASE too — origin/<base> in pr mode (#546)', async () => {
    ...
    for (const pr of [true, false]) {
      ...
      expect({ pr, resolves: cmd.includes("printf %s 'refs/remotes/origin/main' || { git rev-parse --verify -q 'refs/heads/main^{commit}' >/dev/null 2>&1 && printf %s 'refs/heads/main' || printf %s 'main'; }") }).toEqual({ pr, resolves: true })

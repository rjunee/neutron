...
describe('inner-workflow.mjs — #545: the reviewed head is the COMMIT THE DIFF CAME FROM, and is CARRIED', () => {
  ...
  test("round 1 pins to Forge's reported commit sha, fixed BEFORE the review that judges it", () => {
    ...
    const firstReview = SRC.indexOf('synthesis = await reviewAndSynthesize(diffFile, round, pr)')
  ...
  test("every fix round re-pins to the sha THAT round's fix agent reported committing", () => {
    ...
    // lastIndexOf: the SAME two lines open the round-1 review (in the no-resume
    // branch), and it is the fix loop's copy — the last one in the file — that has
    // to come after the re-pin.
    const loopReview = SRC.lastIndexOf('synthesis = await reviewAndSynthesize(diffFile, round, pr)')

...
export class TridentRunStore {
  ...
  agentWakeCompleted(id: string): boolean {
    return this.db.prepare<{ completed: number }, [string]>(
      'SELECT agent_waked_at IS NOT NULL AS completed FROM code_trident_runs WHERE id = ?',
    ).get(id)?.completed === 1
  }

  /** Single writer of the completion stamp, excluded from snapshot saves. */
      ...
      const res = tx.runSync(
        `UPDATE code_trident_runs
            SET phase = ?, round = MAX(round, ?, ${roundAssign}), ralph_round = ?, branch = ?, pr = ?,
                ...
                -- (Keep this comment free of BACKTICKS, though: this statement is a
                -- template literal, so one backtick ends the string mid-SQL.)

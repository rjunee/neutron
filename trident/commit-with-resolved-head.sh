...
scan_commit_argv() {
  ...
  for arg in "$@"; do
    ...
    case "$arg" in
      ...
      --no-allow-empty-message) allow_empty_message=0 ;;
...
# A literal HEAD captured while detached can later become a symbolic ref. Merely
# checking it before update-ref leaves a race; --no-deref alone would detach a switched
# HEAD. Prepare the no-deref transaction first, then check the ref while its lock is held.
# Abort if it is symbolic or unreadable. This also protects a captured branch ref that
# another writer converts to a symref. No foreign branch or HEAD attachment is changed.
update_captured_ref() (
  local exchange update_pid response ref_kind ref_exit update_exit
  exchange=$(mktemp -d) || return $?
  trap 'rm -f "$exchange/request" "$exchange/response"; rmdir "$exchange"' EXIT
  mkfifo "$exchange/request" "$exchange/response" || return $?
  git update-ref -m "$1" --stdin <"$exchange/request" >"$exchange/response" &
  update_pid=$!
  ...
  if ! IFS= read -r response <&4 || [ "$response" != 'start: ok' ] ||
    ...
    wait "$update_pid"
    update_exit=$?
    [ "$update_exit" -ne 0 ] || update_exit=1
    return "$update_exit"
  fi
  ref_kind=$(git symbolic-ref -q "$target_ref")
  ref_exit=$?
  if [ "$ref_exit" -ne 1 ] || [ -n "$ref_kind" ]; then
    ...
    exec 4<&-
    wait "$update_pid"
    echo "commit refused: captured ref $target_ref is now symbolic or could not be checked while locked; no ref was changed" >&2
    return 1
  fi
  printf 'commit\n' >&3
  exec 3>&-
  ...
  wait "$update_pid"
  update_exit=$?
  [ "$update_exit" -ne 0 ] || [ "$response" = 'commit: ok' ] || update_exit=1
  return "$update_exit"
)

withdraw_commit() {
  update_captured_ref "commit-with-resolved-head: withdraw $1 (Claude-Session check)" "$head_oid" "$1"
}
...
if [ "$cat_exit" -ne 0 ]; then
  # The re-probe can already name another writer's commit. A successful CAS against that
  # oid would discard their work: until the object is read there is no provenance check.
  echo "commit refused: the commit $new_head observed on $target_ref could not be read back for the Claude-Session check (git cat-file exited $cat_exit); provenance could not be established, so nothing was rewritten and nothing was withdrawn; the commit this invocation created was not verified clean and carries the trailer if the message had one -- inspect the refs before retrying" >&2
  exit "$cat_exit"
fi
...
if strip_session_trailer <"$raw_object" >"$stripped_message"; then
  ...
  update_captured_ref "commit-with-resolved-head: strip Claude-Session (rewrite $new_head)" "$candidate" "$new_head" 2>"$scratch_err"
  swap_exit=$?

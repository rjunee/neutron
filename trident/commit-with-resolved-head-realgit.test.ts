...
test('#1133: an initial read-back failure refuses without withdrawing an unproven commit', () => {
  const { tree, branch, parent } = fixture()
  ...
  expect(result.stderr).toContain('provenance could not be established')
...
test('#1133: an initial read-back failure never attempts a ref update, even when updates also fail', () => {
  const { tree, branch, parent } = fixture()
  ...
  expect(result.status).toBe(3)
  expect(result.stderr).not.toContain('shim: update-ref refused')
  expect(result.stderr).toContain('provenance could not be established')
  expect(result.stderr).toContain('carries the trailer if the message had one')
  expect(result.stderr).not.toContain('the commit was withdrawn')
...
function runGuardWithShim(tree: string, branch: string, bin: string, script = guard) {
  return spawnSync('bash', [script, branch, '-m', 'feat: subject', '-m', SESSION, '-m', CO_AUTHOR], {
    cwd: tree,
    encoding: 'utf8',
    env: { ...process.env, PATH: `${bin}:${process.env.PATH ?? ''}` },
  })
...
test('#1133: a read-back failure after another writer advances the ref refuses without any withdrawal', () => {
  const { tree, branch, parent } = fixture()
...
test('#1133: a branch switch after the commit cannot redirect the withdrawal -- the CAPTURED branch is withdrawn and the other branch left alone', () => {
  ...
  // to a new branch at the same commit, then the build fails AFTER provenance is known.
  // Updating mutable HEAD would roll back the wrong branch.
  const { bin, count } = shimmedGitFailingNth('commit-tree', 1, '$REAL_GIT checkout -q -b other HEAD; echo "shim: commit-tree refused" >&2; exit 8')
...
test('#1133: an unreadable foreign commit observed by the re-probe is never withdrawn', () => {
  const { tree, branch, parent } = fixture()
  const { bin, count } = shimmedGitFailingNthRevParse(2, CONCURRENT_ARM, { 'cat-file': 8 })
...
test('#1133: an unreadable detached commit followed by a branch switch never rewinds the new branch', () => {
  const { tree, branch, parent } = fixture()
  git(tree, 'checkout', '-q', '--detach')
  const { bin, count } = shimmedGitFailingNth('cat-file', 1,
    '$REAL_GIT checkout -q -b other HEAD; echo "shim: cat-file refused" >&2; exit 8')
...
for (const messageOption of ['-m', '--message', '-am', '-qm']) {
  test(`#1133: ${messageOption} consumes a literal --amend message without changing provenance`, () => {
    const { tree, branch, parent } = fixture()
    const result = run(tree, 'bash', [guard, branch, messageOption, '--amend', '-m', SESSION, '-m', CO_AUTHOR])

    expect(result.status, result.stderr || result.stdout).toBe(0)
    expect(git(tree, 'log', '-1', '--format=%B')).toBe(`--amend\n\n${CO_AUTHOR}`)
    expect(git(tree, 'rev-parse', 'HEAD^')).toBe(parent)
    expect(git(tree, 'show', '--format=', 'HEAD:change.txt')).toBe('change')
  })
}

test('#1133: a literal --amend message with no trailer remains an ordinary unmodified commit', () => {
  const { tree, branch, parent } = fixture()
  const result = run(tree, 'bash', [guard, branch, '-m', '--amend'])
...
for (const withdrawing of [false, true]) {
  test(`#1133: a detached ${withdrawing ? 'withdrawal' : 'publish'} refuses a branch switch inside update-ref without detaching HEAD`, () => {
    const { tree, branch, parent } = fixture()
    git(tree, 'checkout', '-q', '--detach')
    const failures = withdrawing ? { 'commit-tree': 8 } : {}
    const arm = '$REAL_GIT checkout -q -b other HEAD'
    // Same real-Git path without the switch: valid detached publish/withdrawal works.
    const control = shimmedGitFailingNth('update-ref', 0, arm, failures)
    const valid = runGuardWithShim(tree, branch, control.bin)
    expect(valid.status, valid.stderr || valid.stdout).toBe(withdrawing ? 8 : 0)
    expect(run(tree, 'git', ['symbolic-ref', '-q', 'HEAD']).status).toBe(1)
    if (withdrawing) {
      expect(git(tree, 'rev-parse', 'HEAD')).toBe(parent)
      expect(valid.stderr).toContain('was withdrawn')
    } else {
      expect(git(tree, 'rev-parse', 'HEAD^')).toBe(parent)
      expect(git(tree, 'log', '-1', '--format=%B')).toBe(`feat: subject\n\n${CO_AUTHOR}`)
      git(tree, 'reset', '-q', '--soft', parent)
    }
    expect(readFileSync(control.count, 'utf8').trim()).toBe('1')

    const fault = shimmedGitFailingNth('update-ref', 1, arm, failures)
    const result = runGuardWithShim(tree, branch, fault.bin)

    expect(result.status).toBe(withdrawing ? 8 : 74)
    expect(result.stderr).toContain('captured ref HEAD is now symbolic')
    expect(result.stderr).not.toContain('the commit was withdrawn')
    expect(git(tree, 'symbolic-ref', 'HEAD')).toBe('refs/heads/other')
    expect(git(tree, 'rev-parse', 'other^')).toBe(parent)
    expect(git(tree, 'log', '-1', '--format=%B', 'other')).toContain(SESSION)
    expect(git(tree, 'rev-parse', branch)).toBe(parent)
    expect(readFileSync(fault.count, 'utf8').trim()).toBe('1')
  })
}

test('#1133: the detached-state check holds the HEAD lock against a concurrent attachment', () => {
  const { tree, branch, parent } = fixture()
  git(tree, 'checkout', '-q', '--detach')
  git(tree, 'branch', 'other', parent)
  const { bin, count } = shimmedGitFailingNth('symbolic-ref', 2,
    '$REAL_GIT symbolic-ref HEAD refs/heads/other; switch_exit=$?; echo "$switch_exit" > "$(dirname "$0")/switch.status"')

  const result = runGuardWithShim(tree, branch, bin)

  expect(result.status, result.stderr || result.stdout).toBe(0)
  // Publish redirects the transaction's stderr to a scratch file, so record the competing
  // writer's actual failure in the shim directory as well as checking the final refs.
  expect(readFileSync(join(bin, 'switch.status'), 'utf8').trim()).not.toBe('0')
  expect(run(tree, 'git', ['symbolic-ref', '-q', 'HEAD']).status).toBe(1)
  expect(git(tree, 'rev-parse', 'HEAD^')).toBe(parent)
  expect(git(tree, 'log', '-1', '--format=%B')).toBe(`feat: subject\n\n${CO_AUTHOR}`)
  expect(git(tree, 'rev-parse', 'other')).toBe(parent)
...
test('#1133: semantic under/over controls distinguish unsafe symbolic updates from valid detached publishes', () => {
  const source = readFileSync(guard, 'utf8')
  const condition = 'if [ "$ref_exit" -ne 1 ] || [ -n "$ref_kind" ]; then'
  expect(source.split(condition)).toHaveLength(2)
  const scripts = mkdtempSync(join(tmpdir(), 'trident-head-mutations-'))
  roots.push(scripts)
  for (const mutation of ['under', 'over'] as const) {
    const script = join(scripts, mutation)
    writeFileSync(script, source.replace(condition, `if ${mutation === 'under' ? 'false' : 'true'}; then`))
    const { tree, branch, parent } = fixture()
    git(tree, 'checkout', '-q', '--detach')
    const { bin } = shimmedGitFailingNth('update-ref', mutation === 'under' ? 1 : 0,
      '$REAL_GIT checkout -q -b other HEAD')
    const result = runGuardWithShim(tree, branch, bin, script)
    if (mutation === 'under') {
      // Removing the type guard spuriously succeeds and silently detaches switched HEAD.
      expect(result.status).toBe(0)
      expect(run(tree, 'git', ['symbolic-ref', '-q', 'HEAD']).status).toBe(1)
      expect(git(tree, 'rev-parse', 'HEAD')).not.toBe(git(tree, 'rev-parse', 'other'))
      expect(git(tree, 'rev-parse', 'other^')).toBe(parent)
    } else {
      // Rejecting every prepared update kills an otherwise valid detached strip.
      expect(result.status).toBe(74)
      expect(git(tree, 'log', '-1', '--format=%B')).toContain(SESSION)
      expect(git(tree, 'rev-parse', 'HEAD^')).toBe(parent)
    }
...
test('#1133: an unreadable ref type after prepare aborts and releases the transaction lock', () => {
  const { tree, branch, parent } = fixture()
  git(tree, 'checkout', '-q', '--detach')
  const { bin, count } = shimmedGitFailingNth('symbolic-ref', 2, 'exit 128')

  const result = runGuardWithShim(tree, branch, bin)

  expect(result.status).toBe(74)
  expect(result.stderr).toContain('could not be checked while locked')
  expect(run(tree, 'git', ['symbolic-ref', '-q', 'HEAD']).status).toBe(1)
  expect(git(tree, 'rev-parse', 'HEAD^')).toBe(parent)
  expect(git(tree, 'log', '-1', '--format=%B')).toContain(SESSION)
  expect(readFileSync(count, 'utf8').trim()).toBe('2')
  // A real subsequent writer can take the same lock: abort did not strand it.
  git(tree, 'symbolic-ref', 'HEAD', `refs/heads/${branch}`)
  expect(git(tree, 'rev-parse', 'HEAD')).toBe(parent)
})

test('#1133: a failed ref transaction reports the proven commit remaining after a failed rebuild', () => {
  const { tree, branch, parent } = fixture()
  const bin = shimmedGitFailing({ 'commit-tree': 8, 'update-ref': 9 })

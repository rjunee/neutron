...
export function parseInnerResult(raw: string | null | undefined): InnerResult | null {
  ...
  return {
    ...
    // row that held pr=267: `result.pr_number ?? run.pr` (orchestrator.ts:892, written
    // at :967) keeps a 0, because 0 is not nullish. Mapping it to null HERE restores
    // the known PR at every `?? run.pr` site at once.
    pr_number:

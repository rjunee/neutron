...
describe('interpretFailure — an early stop must not be told as a review outcome', () => {
  ...
  test('a BRANCH NAME cannot buy back the retry advice this class forbids', () => {
    ...
    // The review-unresolved control carries the ROW SHAPE a review outcome has —
    // a terminal REQUEST_CHANGES with the findings the write site insists on. The
    // reason string alone no longer buys that class (see `interpretFailure`'s
    // disposition gate): a never-reviewed row whose reason happens to say
    // "exhausted" is exactly the lie this file was written about, pointed the other
    // way. The shape is the same one the positive control above uses.
    expect(
      interpretFailure(
        run({
          inner_verdict: 'REQUEST_CHANGES',

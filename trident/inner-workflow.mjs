    ...
    mutationClaim: {
      ...
      properties: {
        ...
        control: {
          ...
          items: { type: 'string' },
          description:
            'argv that MUST stay GREEN under the mutation. A REAL test invocation, in the SAME shapes as the guard and under the same argument rules — no load hook, no shadowable runner, no branch-authored resolution map, every argument repo-relative and inside the worktree — and DISTINCT from the guard: one argv cannot be both the thing that goes red and the thing that stays green. A WRAPPER (`npm test`, `npm run …`, `pnpm run …`, `yarn run …`, `make …`) is refused as a GUARD only and remains a LEGAL CONTROL — not because a control is harmless (it is branch-authored code that runs between the RED and GREEN observations of the guard), but because the prover re-provisions the worktree to the pinned commit before the restored-guard observation and refuses outright when tracked files were edited, so nothing a control writes into the tree can reach the observation that decides the proof. `sh -c …` and any other general shell is refused outright.',
        },

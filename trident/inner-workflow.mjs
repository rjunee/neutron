    ...
    mutationClaim: {
      ...
      properties: {
        ...
        control: {
          ...
          items: { type: 'string' },
          description:
            'argv that MUST stay GREEN under the mutation. A REAL test invocation, in the SAME shapes as the guard and under the same argument rules — no load hook, no shadowable runner, no branch-authored resolution map, every argument repo-relative and inside the worktree — and DISTINCT from the guard: one argv cannot be both the thing that goes red and the thing that stays green. A WRAPPER (`npm test`, `npm run …`, `pnpm run …`, `yarn run …`, `make …`) is refused as a GUARD only and remains a LEGAL CONTROL: a hidden load can redden a guard dishonestly, while a control staying green says less. BUT A CONTROL MAY NOT WRITE INTO THE WORKTREE. It runs between the guard\'s RED and the guard\'s GREEN, so the proof records what git says the tree looks like before the mutation and again before the restored guard, and REFUSES OUTRIGHT when any path other than the mutated file differs — a control (or a wrapper recipe) that rewrites the guard\'s own test file would otherwise forge the green. `sh -c …` and any other general shell is refused outright.',
        },

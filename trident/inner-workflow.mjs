...
Nominate nothing ONLY when your branch's ENTIRE diff is INERT documentation — \`.md\`/\`.mdx\`, LICENSE, NOTICE: then do NOT write the file at all and ignore the rest of this block — such a diff has no legal target to nominate (the gate exempts it instead; it reads a nomination blob as inert too — but that dispensation runs AFTER the gate's denylist, so a branch NAME carrying a \`skills\`, \`prompts\`, \`.claude\`, \`agent-dispatch\` or \`.github\` path segment would forfeit the exemption, which is the other reason not to write one here).

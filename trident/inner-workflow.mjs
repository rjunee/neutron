    ...
    mutationClaim: {
      ...
      properties: {
        ...
        control: {
          ...
          items: { type: 'string' },
          description:
            'argv that MUST stay GREEN under the mutation. A REAL test invocation, in the SAME shapes as the guard and under the same argument rules — no load hook, no shadowable runner, no branch-authored resolution map, every argument repo-relative and inside the worktree — and DISTINCT from the guard: one argv cannot be both the thing that goes red and the thing that stays green. A WRAPPER (`npm test`, `npm run …`, `pnpm run …`, `yarn run …`, `make …`) is refused as a GUARD only and remains a LEGAL CONTROL — but a control is NOT unobserved: it is branch-authored code executing between the guard\'s RED and its restored GREEN, so after the mutated runs the prover asks git whether the worktree still matches the provisioned commit (`git status --porcelain --ignored` must come back empty) and refuses the proof when the guard or control wrote ANYTHING into the worktree — a rewritten test file, a build artifact, an ignored path. `sh -c …` and any other general shell is refused outright.',
        },

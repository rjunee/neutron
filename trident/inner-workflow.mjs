    ...
    mutationClaim: {
      ...
      properties: {
        ...
        guard: {
          ...
          items: { type: 'string' },
          description:
            'argv that MUST go RED under the mutation. A SEPARATE test OF the behaviour: it may not run the mutated file itself — not by naming it (in an argument or inside an option such as --preload=…), and, whenever a runner could collect the mutated file wholesale, not via a directory that collects it, not via a glob or `unittest discover`, and not by naming no path at all (a bare `bun test` / `python3 -m unittest` / `npm run test-all` discovers from the repo root). That is a tautology and is refused before anything runs. TWO OPAQUE SHAPES ARE REFUSED FOR EVERY TARGET, because the command they really run lives in a file the branch wrote and this argv does not show it: a WRAPPER (`npm test`, `npm run …`, `pnpm run …`, `yarn run …`, `make …`, whose script body or recipe may itself preload the mutated file), and a LOAD HOOK (`--preload=…`, `--require=…`/`-r…`, `--import=…`, `--loader=…`, `--experimental-loader=…`, and a REPORTER that is a module the runtime loads: `--test-reporter=…`, `--reporter=…`, `--reporters=…`, and a CONFIG or ENV FILE the runner obeys: `--config=…`/`-c…` (whose `[test] preload` is that same hook), `--experimental-config-file=…` and the valueless `--experimental-default-config-file` (node reads `node.config.json` and obeys it), `--env-file=…`/`--env-file-if-exists=…` (whose `NODE_OPTIONS` is), whatever file any of them names — and a long hook option is refused on its NAME whether or not it names a file at all, because that file may import the mutated one). Spell the runner out instead — `bun test path/to/the.test.ts`. NAME THE SEPARATE TEST FILE: an argv whose only arguments are options (`bun test --timeout 1000`) counts as naming nothing. Every argument is repo-relative: an absolute path or one containing `..` is refused outright.',
        },

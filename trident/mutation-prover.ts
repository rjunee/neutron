 ...
 *
 * IT IS A PREFIX, NOT THE WHOLE SENTENCE. `missingClaimRefusalReason` builds on
 * it — appending which legal target the build could have named — so the
 * orchestrator matches it with `startsWith`, and that function INTERPOLATES this
 * constant rather than re-typing it so the two cannot drift apart. The other
 * shapes that function returns (an unreadable diff, an empty one, a
 * deletion-only diff, a gate defect) deliberately do NOT start with it: those
 * are not "the build nominated nothing", and the reader's note does not explain
 * them.
 */
 ...
 *
 * The suffix is matched CASE-SENSITIVELY on purpose: the writer's path is
 * lowercase by construction, so a `.JSON` spelling is never the nomination — it
 * gets neither the inert-prose dispensation nor the self-nomination refusal. It
 * is an ordinary data file and, like ANY co-committed non-test data file in the
 * branch diff, it remains NOMINATABLE: the self-nomination rule closes only the
 * narrow hole where a nomination names ITSELF (in the diff by construction), not
 * the general one, which only the guard actually reddening under the mutation
 * bounds.
 */
...
/**
 * STATIC overrides on EVERY proof-tree provisioning. `git worktree add` runs
 * `post-checkout`, and may also execute `core.fsmonitor`. Both commands can live
 * in the shared config outside the disposable tree. The dynamic checkout-filter
 * overrides are added by `proofProvisioningConfig` below.
 *
 ...
 */
const PROOF_PROVISIONING_CONFIG = [
  '-c',
  'core.hooksPath=/nonexistent/trident-proof-no-hooks',
  '-c',
...
export function createMutationProver(deps: MutationProverDeps): MutationProver {
  ...
  /**
   * Close the configured checkout-filter command family before `worktree add`.
   * A control can add arbitrarily named `filter.<driver>.smudge` or `.process`
   * entries to the shared config, so a fixed list cannot close it. Query the
   * effective config immediately before each provisioning and shadow every
   * matching key with an empty command at command-line precedence. Git treats
   * an empty filter command as unspecified and passes the committed bytes
   * through. A config query we cannot interpret REFUSES; uncertainty never
   * falls through to an unguarded add.
   */
  async function proofProvisioningConfig(repo: string): Promise<string[] | null> {
    const configured = await deps.run_host(
      ['git', '-C', repo, 'config', '--includes', '--null', '--name-only', '--get-regexp', '^filter\\..*\\.(smudge|process)$'],
      repo,
    )
    // `git config --get-regexp` exits 1 when there were no matches.
    if (!configured.ok && configured.exit_code !== 1) return null
    const keys = configured.stdout.split('\0').filter((key) => key.length > 0)
    if (keys.some((key) => !/^filter\..+\.(?:smudge|process)$/i.test(key))) return null
    return [...PROOF_PROVISIONING_CONFIG, ...[...new Set(keys)].flatMap((key) => ['-c', `${key}=`])]
  }
   ...
   *
   * WHY PROVISIONING IS NOT ENOUGH ON ITS OWN. The provisioning config stops
   * known shared-config commands from executing, and a fresh tree throws away
   * whatever a nominated command wrote into the old one — but neither says the new tree
  ...
  async function prove(input: ProveInput): Promise<MutationEvidence> {
    ...
    // can move a ref. Every add gets the highest-precedence provisioning config:
    // hooks, fsmonitor, and configured checkout-filter commands are all external
    // execution paths which would otherwise survive removal of the proof tree.
    const wt = proofWorktreePath(repo, run)
    await deps.run_host(['git', '-C', repo, 'worktree', 'remove', '--force', wt], repo)
    const provisioningConfig = await proofProvisioningConfig(repo)
    if (provisioningConfig === null) {
      return refuse(run.id, claim, 'could not inspect git config before provisioning the proof worktree')
    }
    const add = await deps.run_host(
      ['git', ...provisioningConfig, '-C', repo, 'worktree', 'add', '--detach', '--force', wt, headSha],
      repo,
    )
    ...
    // module body rewrote the guard's own red test file turned the restored
    ...
    // re-provisioned at the SAME pinned sha, with a fresh config inspection so
    // the re-add itself runs nothing the branch wrote. The two --force flags are two
    ...
    const reprovision = async (): Promise<string | null> => {
      ...
      const reProvisioningConfig = await proofProvisioningConfig(repo)
      if (reProvisioningConfig === null) {
        return 'could not inspect git config before re-provisioning the proof worktree — an unclassified shared-config command must refuse, never run'
      }
      const readd = await deps.run_host(
        ['git', ...reProvisioningConfig, '-C', repo, 'worktree', 'add', '--detach', '--force', wt, headSha],
        repo,
      )
      if (!readd.ok) {
  ...
  async function proveInWorktree(
    ...
    try {
      ...
      // ALWAYS restore, even if a command threw. This in-place write is THE
      // restore where no git exists — the in-memory `fs` seam the unit tests
      // run on — and it keeps the throw path tidy either way. Where git does
      // exist, the observation below reads the RE-PROVISIONED tree, and a
      // restored digest that does not match `before` still refuses at
      // `evaluate`.
      await fs.write(target, before).catch(() => undefined)
    }

    // THE TREE THE SECOND OBSERVATION READS IS A FRESH ONE. The line above
    // restored one file; everything else in this tree was writable by the
    // guard's and control's own code while they ran. So the restored-guard
    // observation is made of a worktree re-provisioned at the same pinned
    // sha: an edit planted in the old tree dies with it, and the restored
    // digest below is read from the tree the guard actually sees.
    //
    // The escape this closes was reproduced end to end at this branch's head,
    // in two spellings: an unconditionally-red guard that never imports the
    // target, plus a control — a `bun test` file whose MODULE BODY runs on
    // load, and again a `make` goal whose recipe runs the same write — that
    // rewrote the guard's committed test file into a passing stub. Red, green,
    // green, with nothing having tested the target. The same shape needs no
    // control at all: a guard whose first (red) run rewrites its own file is
    // the one-actor variant, which no inspection of the CONTROL could see.
    //
    ...
    // tree; the provisioning overrides on both `worktree add` calls stop that. And a
...
export function isProseOnlyChange(files: readonly string[] | null | undefined): boolean {
  ...
  return files.every((raw) => {
    ...
    // THIS predicate admits as a mutation TARGET — one predicate, so the two sets
    // cannot drift. A path it does NOT admit (a `.JSON` spelling, a suffix-less
    // file) is ordinary on both sides: not inert here — it forfeits a docs diff's
    // exemption, fail-closed — and not refused there, so if it is in the branch
    // diff it stays nominatable exactly like any other co-committed non-test data
    // file; the self-nomination rule is that narrow on purpose. `.json` is not a
  ...
  if (named !== undefined) {
    const caveat =
      executable === undefined
        ...
        : ''
    return `${NO_NOMINATION_REFUSAL} — a legal mutation target existed: ${asReason(named)} changed in this diff and is neither a declared test nor documentation${caveat}`
  }

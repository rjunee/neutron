/**
 * Ported battle-tested-fix parity — one explicit assertion per fix, pinned on
 * the LIVE exec-model path.
 *
 * Trident-port mandate: every battle-tested fix from the legacy harness's `/trident`
 ...
 * is that map — each `test` names the the legacy harness fix (and, where relevant, the
...
// as in-flight (the legacy harness: "don't poll a phantom session", 2026-04-15 + 2026-06-09).
...
// ---------------------------------------------------------------------------
// FIX 2 — Reap / "dispatch lost on restart" → re-dispatch, bounded. the legacy harness
...
// in one shot (the legacy harness silent-exit trigger). Open v2 analogs: the live
...
// ---------------------------------------------------------------------------
// FIX 5 — Phantom-ID / async-registry-write race. the legacy harness's registry row was
...
// the legacy harness: a Forge that never emitted the PR contract is a failure (not silent
...
// ---------------------------------------------------------------------------
// FIX 9 — Fleet PREMATURE-COMPLETION reconciliation (the legacy harness PR #164 + #160).
// the legacy harness incident (fleet-wide, 2026-06-23): a Forge pushed its branch then

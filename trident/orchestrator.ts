...
} from './infrastructure-retry.ts'
export {
  classifyInnerFailure,
  DEFAULT_MAX_INFRA_RETRIES,
  INFRA_CAUSE_WORDS,
  INFRA_RETRY_BACKOFF_MS,
  type InnerFailureClass,
} from './infrastructure-retry.ts'
...
import { applyMergeApproval } from './merge-approval.ts'
import { runMutationProofGate, type MutationGateInput, type MutationGateOutcome } from './mutation-prover.ts'
import {
...
export function buildTridentOrchestrator(
  ...
  async function launch(run: TridentRun): Promise<AdvanceOutcome> {
    ...
    const boundReview = await advanceBoundReview(run, {
      ...
      ...(opts.gh_data_dir !== undefined ? { gh_data_dir: opts.gh_data_dir } : {}),
      ...(opts.gh_owner_handle !== undefined ? { gh_owner_handle: opts.gh_owner_handle } : {}),
      ...(opts.resolve_kimi_configured !== undefined ? { resolve_kimi_configured: opts.resolve_kimi_configured } : {}),
      ...(opts.resolve_phase_models !== undefined ? { resolve_phase_models: opts.resolve_phase_models } : {}),
      panel_timeout_ms: maxInflightMs,
      failed_run: failedRun,
      now,
    })
    if (boundReview !== null) return boundReview
  ...
  async function applyResult(run: TridentRun, result: InnerResult): Promise<AdvanceOutcome> {
    ...
    const infrastructureRetry = await tryInfrastructureRetry({
      run, result, beginInfraRetry, maxInfraRetries, onInfraRetry, now, nowMs, failedRun,
      infraRetryNotBefore, warn: log.warn.bind(log),
    })
    if (infrastructureRetry !== null) return infrastructureRetry
    ...
    const mergeApproval = await applyMergeApproval({
      run,
      result,
      run_host: opts.run_host,
      merge_deps,
      ...
      truncateNote,
      truncateStageReason,
      ...(opts.record_stage !== undefined ? { record_stage: opts.record_stage } : {}),
      log,
    })
    if (mergeApproval !== null) return mergeApproval

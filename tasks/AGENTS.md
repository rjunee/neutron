...
append-queue plus the `tasks.md` / `DASHBOARD.md` projections, modelled on the legacy harness — was **RETIRED in
...
- ~~LLM-driven daily nudge engine~~ — **SHIPPED.** The once-daily "pick one most important thing" LLM pass is `gateway/tasks/p6/nudge-engine.ts` (gated on `tasks.enable_nudge_engine_cron`), which reads the top-N slate, feeds it through `buildNudgePrompt`, and UPSERTs `current_focus_pick`. The LLM-**primary** backlog *ranking* also landed (`prioritize-llm.ts`, WAVE 3 PR-7 — see above). No longer a follow-up.
  - **Correction (this line said "SHIPPED" while the cron was OFF).** "Built and gated" is not shipped. `open/composer.ts` set `enable_nudge_engine_cron` when the pair first landed, dropped it when the idle-nudge sweep was deferred, and did not restore it when the sweep came back — so from 2026-06-27 until this fix the engine never ran on an Open install and `current_focus_pick` stayed empty, which in turn made the idle-nudge sweep (`readTodayPick` → `no_pick`) a guaranteed no-op. The composer now sets the flag, and `open/__tests__/open-proactive-activation.test.ts` asserts it against the PRODUCTION composer's output so dropping it again reds. When updating this file, only claim SHIPPED for something a fresh install actually runs — grep the composer, not the module.

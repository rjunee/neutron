...
import {
  verifyOriginals,
  type DroppedOriginal,
  type ExtractedOriginal,
} from './verbatim.ts'
...
export interface ExtractedRelation {
   ...
   * to retire — out of RB4 scope). Acted on by the `write-to-gbrain.ts` supersede
   * path (always on — belief evolution is the base behavior): the superseded
   * object's compiled-truth sentence is dropped (so the gbrain edge falls out via
   * the writer's existing `removedLinks`→`remove_link` machinery) while the
   * append-only timeline keeps the dated history.
   */
  supersedes?: string
}
...
export interface ScribeExtraction {
  ...
  /**
   * VERBATIM originals — the owner's own reflective passages, copied exactly.
   * the legacy harness's largest entity class (949 `entities/originals/` pages) and the
   * whole point is that the words are HIS, so every entry here has passed the
   * `scribe/verbatim.ts` guard and its `passage` is an exact byte-range slice
   * of the source turn.
   *
   * OPTIONAL on the type so existing constructors of a `ScribeExtraction`
   * literal keep compiling; `parseExtraction` always populates it (`[]` when
   * the response carries none), and every consumer must read it as `?? []`.
   */
  originals?: ExtractedOriginal[]
  /**
   * Guard telemetry — candidate passages the verbatim guard REFUSED, with the
   * reason. A silent drop would make "my originals stopped growing"
   * undiagnosable, so the rejection set travels with the extraction and
   * `scribe/index.ts` logs it.
   */
  originals_dropped?: DroppedOriginal[]
}

const EMPTY: ScribeExtraction = Object.freeze({
  entities: Object.freeze([]) as unknown as ExtractedEntity[],
  relations: Object.freeze([]) as unknown as ExtractedRelation[],
  originals: Object.freeze([]) as unknown as ExtractedOriginal[],
  originals_dropped: Object.freeze([]) as unknown as DroppedOriginal[],
}) as ScribeExtraction
...
Extract three things from the message below and return them as a SINGLE JSON object — no preamble, no markdown fence, JSON only:

{
  ...
  "originals": [
    { "title": "<what this passage is about>", "passage": "<the user's words, COPIED EXACTLY>" }
  ]
}
...
- If the message contains nothing worth remembering, return {"entities": [], "relations": [], "originals": []}.

ORIGINALS — the user's OWN WORDS, preserved:
An "original" is a passage of the user's genuinely reflective, substantive first-person thinking, preserved because the WORDS THEMSELVES matter — a worked-through idea, a position they are arguing, a realisation, a principle, a piece of reasoning about their life or work.

- "passage" MUST be COPIED CHARACTER-FOR-CHARACTER from the message. Copy, do not write. Do NOT paraphrase. Do NOT summarise. Do NOT shorten, expand, tidy, correct spelling or grammar, re-punctuate, change capitalisation, swap a word for a better one, or join text from two separate places in the message. If you cannot reproduce the run of text exactly as it appears, leave it out.
- Select a CONTIGUOUS run of the message — start at the first character of the thought and end at the last. Never stitch fragments together.
- "title" is the ONE field you compose: a short label for what the passage is about. It is not a quote.
- Only genuinely reflective / substantive thinking qualifies. NOT chit-chat, NOT questions to the agent, NOT instructions or requests, NOT logistics, NOT a bare statement of fact (that is what "entities" and "fact" are for), NOT anything the user is quoting from someone else.
- One thought per entry. Most messages contain NO original — that is the normal case. When in doubt, skip: over-extraction is worse than under-extraction, and a passage that is not the user's exact words is worse than no passage at all.
- If the message contains no such passage, return "originals": [].

MESSAGE:
`

/**
 * RB4 supersede guidance — ALWAYS spliced into the extraction prompt (belief
 * evolution is the base behavior now; see `composeExtractionPrompt`). Teaches the
 ...
 * (they are genuinely larger mechanisms, out of RB4 scope).
 */
...
export function composeExtractionPrompt(text: string): string {
  // Temporal invalidation (belief evolution) is the base behavior now (managed
  // SPEC Decisions Log 2026-07-20, P0-4): the supersede guidance is ALWAYS
  // spliced in so the extractor may emit the optional `relations[].supersedes`
  // marker.
  const base = SCRIBE_EXTRACTION_PROMPT.replace('MESSAGE:\n', `${SUPERSEDE_GUIDANCE}\nMESSAGE:\n`)
...
export async function runExtraction(
  ...
  // The SOURCE turn is threaded into the parse so the verbatim guard can prove
  // every captured passage is a real copy of it (`scribe/verbatim.ts`).
 ...
 * chat turn.
 *
 * `sourceText` is the ORIGINAL user turn the extraction was run over. Every
 * `originals[]` passage is verified against it and DROPPED unless it is a
 * contiguous copy; keepers are rewritten to the exact source byte range (see
 * `scribe/verbatim.ts`). OMITTING `sourceText` is fail-CLOSED: verbatimness is
 * then unprovable, so every candidate is dropped with
 * `reason: 'unverifiable_no_source'`. There is no path that yields an
 * unverified original.
 */
export function parseExtraction(text: string, sourceText?: string): ScribeExtraction {
  const obj = extractJsonObject(text)
  if (obj === null || typeof obj !== 'object') return EMPTY
  const o = obj as Record<string, unknown>
  const guarded = verifyOriginals(normOriginals(o['originals']), sourceText)
  return {
    entities: normEntities(o['entities']),
    relations: normRelations(o['relations']),
    originals: guarded.kept,
    originals_dropped: guarded.dropped,
  }
}
...
/**
 * Shape-normalise the raw `originals` rows. Deliberately does NO verbatim
 * checking — that is `verifyOriginals`' job, and keeping the two apart means
 * the guard is a single choke point rather than a condition sprinkled through
 * the parser. Whitespace is NOT trimmed off `passage` here: the guard needs
 * the model's bytes as emitted, and the passage it ultimately stores comes
 * from the SOURCE anyway.
 */
function normOriginals(v: unknown): ExtractedOriginal[] {
  if (!Array.isArray(v)) return []
  const out: ExtractedOriginal[] = []
  for (const row of v) {
    if (row === null || typeof row !== 'object') continue
    const r = row as Record<string, unknown>

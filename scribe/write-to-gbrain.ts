...
export interface WriteExtractionReport {
  ...
  /** VERBATIM originals: `original`-kind pages created or appended to. */
  originals_written: number
  /** VERBATIM originals: candidates that produced no page change (unusable
   *  slug, already-present passage, or a writer failure). */
...
export async function writeExtractionToGBrain(
  ...
  const report: WriteExtractionReport = {
    pages_written: 0,
    pages_skipped: 0,
    edges_emitted: 0,
    originals_written: 0,
  ...
  // 4. VERBATIM originals — the owner's own words, filed as `original`-kind
  //    pages. Same entity-writer → GBrain boundary, same per-page scribe lock,
  //    same append-only discipline; only the BODY differs (a quoted passage
  //    instead of composed compiled-truth prose).
  await writeOriginalPages(input, deps, report, writeDeps, timelineSource)

  return report
}

/** Heading that opens an `original` page's compiled-truth. */
const ORIGINAL_TITLE_LINE = (name: string): string => `# ${name}`

/**
 * Write each VERBATIM original as an `original`-kind entity page whose
 * compiled-truth carries the owner's passage EXACTLY as he wrote it.
 *
 * The property that makes this safe is the entity-writer's own contract:
 * `compiledTruth` is "the rewritten state; the writer never edits it"
 * (`runtime/entity-writer.ts:96-99`). So the bytes handed in here are the bytes
 * that land on disk — the writer only normalises the page's TRAILING newline
 * (`runtime/entity-format.ts:122` `ensureTrailingNewline(compiledTruth.trimEnd())`).
 *
 * APPEND, NOT REPLACE (the ambiguity the brief asked to resolve, resolved
 * toward append). the legacy harness's `entities/originals/` pages ACCRETE — a page is a
 * running collection of the owner's passages on a theme, added to over months.
 * So an existing page's body is preserved byte-for-byte and the new passage is
 * appended after a blank line; only a brand-new page is composed from scratch.
 * A passage already present in the body is a NO-OP (idempotent replay: the
 * writer then sees a byte-identical page and reports `changed: false`), which
 * is also what keeps a re-delivered turn from duplicating the owner's words.
 *
 * The passage is NOT blockquoted, indented, or fenced. Any per-line prefix
 * would mean the stored bytes are not the owner's bytes, which is the one thing
 * this feature exists to guarantee. It goes in raw.
 */
async function writeOriginalPages(
  input: WriteExtractionInput,
  deps: WriteExtractionDeps,
  report: WriteExtractionReport,
  writeDeps: { syncHook?: SyncHook },
  timelineSource: string,
): Promise<void> {
  const originals = input.extraction.originals ?? []
  if (originals.length === 0) return

  for (const original of originals) {
    const slug = slugify(original.title)
    if (slug === null) {
      report.originals_skipped += 1
      continue
    }
    const passage = original.passage
    try {
      const out = await withScribePageLock(`original/${slug}`, async () => {
        const existing = await readExistingPage(input.ownerDataDir, 'original', slug)
        const compiledTruth =
          existing !== null
            ? appendPassage(existing.compiledTruth, original.title, passage)
            : composeOriginalCompiledTruth(original.title, passage)
        const frontmatter: Record<string, unknown> = {
          ...(existing?.frontmatter ?? {}),
          slug,
          type: 'original',
        ...
        return deps.writeEntity(
          {
            ...
            body: {
              frontmatter,
              compiledTruth,
              // Provenance only — the timeline row records THAT a passage was
              // captured; the passage itself lives in compiled-truth (the
              // timeline renderer flattens newlines, so it could never hold a
              // multi-line quote byte-exactly). Deterministic body ⇒ a replayed
              // identical turn dedups to a true no-op.
              timelineAppend: {
                ts: input.ts,
                source: timelineSource,
                body: `Verbatim passage captured (${passage.length} chars)`,
              },
            },
            originInstance: input.originSlug ?? input.ownSlug,
            receivingInstanceSlug: input.ownSlug,
      ...
      if (out.changed) report.originals_written += 1
      else report.originals_skipped += 1
    } catch (err) {
      report.originals_skipped += 1
      const sink =
        deps.logFailure ??
      ...
      sink(err, { kind: 'original', slug })
    }
  }
}

/** Fresh `original` page: a title heading, then the owner's words, untouched. */
function composeOriginalCompiledTruth(title: string, passage: string): string {
  return `${ORIGINAL_TITLE_LINE(title)}\n\n${passage}\n`
}

/**
 * Append a passage to an existing `original` page, preserving every existing
 * byte. A passage already present is a no-op (idempotent replay + no duplicate
 * capture of the same words).
 */
function appendPassage(existing: string, title: string, passage: string): string {
  const base = existing.replace(/\s+$/, '')
  if (base.length === 0) return composeOriginalCompiledTruth(title, passage)
  if (base.includes(passage)) return `${base}\n`
  return `${base}\n\n${passage}\n`
}

 ...
 *             → VERBATIM GUARD over the extracted originals (`verbatim.ts`)
 *             → entity pages + typed edges + `original`-kind passage pages
 *               written to GBrain via the per-project GBrainSyncHook
 *             → budget release.
 *
 * The extraction has THREE channels: `entities`, `relations`, and `originals` —
 * the owner's own reflective passages, stored UNPARAPHRASED (the legacy harness parity: 949
 * `entities/originals/` pages, whose whole value is that the words are his).
 * Nothing reaches an `original` page unless `verbatim.ts` PROVES it is a
 * contiguous copy of the source turn; refusals travel back on
 * `extraction.originals_dropped` and are logged below.
 *
...
// VERBATIM originals — the guard that proves a captured passage is a real copy
// of the owner's words before it is ever filed as his own (`verbatim.ts`).
export {
  verifyOriginals,
...
export function createScribe(deps: CreateScribeDeps): Scribe {
  ...
  async function extractAndWrite(input: {
    ...
    try {
      ...
      // VERBATIM-GUARD OBSERVABILITY. A passage the guard refused is NEVER
      // silently discarded — a paraphrase slipping through would forge the
      // owner's words, and a silent drop would make "my originals stopped
      // growing" undiagnosable. Both failure modes are worse than a log line.
      const droppedOriginals = extraction.originals_dropped ?? []
      if (droppedOriginals.length > 0) {
        const detail = droppedOriginals
          .map((d) => `${d.reason}:${JSON.stringify(d.excerpt)}`)
          .join(' | ')
        logFailure(
          `verbatim guard dropped ${droppedOriginals.length} passage(s): ${detail}`,
          droppedOriginals,
        )
      }

...
describe('clusterCorrections', () => {
  ...
  test('ordering stability — shuffled input yields the same seed slug', () => {
    ...
    expect(seedA).toBe('correction-pattern-c-aaa')
  })
})

describe('composePatternPage', () => {
  test('slug is SLUG_REGEX-safe from the oldest id; title/learning from the newest', () => {
    const page = composePatternPage([TAB_1, TAB_2, TAB_3])
    expect(page.slug).toBe('correction-pattern-c-aaa')

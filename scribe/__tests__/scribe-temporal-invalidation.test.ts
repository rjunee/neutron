...
describe('RB4 temporal invalidation (belief evolution) — real PGLite round-trip', () => {
  ...
  test('a LEGACY timeline entry that reads like a note is inert (additive-only history model)', async () => {
    ...
    const scribe = createScribe({
      ...
      syncHook,
      ownerDataDir,
      owner_slug: 'rb4-legacy', // → source `chat:rb4-legacy`, matching the legacy entry
      budget: createState(join(ownerDataDir, '.scribe-budget.json'), t0),
      writeEntity,
      now: () => t0,
    })

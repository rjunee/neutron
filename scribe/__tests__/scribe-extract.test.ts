...
describe('parseExtraction', () => {
  ...
  test('garbage → empty extraction', () => {
    // The empty extraction now also carries the (empty) VERBATIM-originals
    // channel + its guard-rejection report — see `scribe/verbatim.ts`.
    const empty = { entities: [], relations: [], originals: [], originals_dropped: [] }
    expect(parseExtraction('not json at all')).toEqual(empty)
    expect(parseExtraction('')).toEqual(empty)
  })

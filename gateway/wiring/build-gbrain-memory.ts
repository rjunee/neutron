/**
 * @neutronai/gateway/wiring — per-instance GBrain memory wiring.
 *
 ...
 *   3. `linkWriter`  — a ONE-METHOD typed edge-write seam (`addLink`). The
 *      `MemoryStore` contract carries no edge surface (memory-store.ts:32-66),
 *      so a caller that must write typed edges without holding the raw
 *      transport needs this. Consumed by an external memory import lane
 *      (the migration importer (removed from this repo)), whose `LinkWriter` interface this
 *      structurally satisfies.
 *   4. `pageReader`  — a ONE-METHOD typed single-page READ seam (`getPage`).
 *      `MemoryStore.query('')` routes to `list_pages`, which gbrain hard-clamps
 *      to 100 rows with no offset, so it cannot prove a 1000+ page write landed;
 *      an authoritative per-slug read can. Consumed by the same lane's
 *      `PageReader` verifier seam.
 *   5. `close`        — tears down the `gbrain serve` child on SIGTERM.
 *
 * `linkWriter` / `pageReader` are TYPED CONTRACT surfaces exactly like
 * `memoryStore` — one named method each, no way to name an arbitrary op — so
 * they do not reopen the RA5 raw-transport hole the `client` field would. The
 * raw `.call('<op>', …)` still happens ONLY inside this exempt composer file
 * (and `gbrain-memory/`), which is what both the depcruise `memory-backend-swap-seam`
 * rule and the AST op-name ban (`gbrain-memory/__tests__/raw-op-seam-ban.test.ts`,
 * `isExemptFromBan`) permit.
 *
...
export interface GBrainMemoryWiring {
  ...
  syncHook: SyncHook
  /**
   * Typed typed-edge WRITE seam. One method, no op names — see the header for
   * why this is not an RA5 hole. Structurally satisfies an external memory lane's
   * `LinkWriter` (the migration importer (removed from this repo)).
   */
  linkWriter: GBrainLinkWriter
  /**
   * Typed single-page READ seam (authoritative retrievability check). One
   * method, no op names. Structurally satisfies an external memory lane's
   * `PageReader` (the migration importer (removed from this repo)).
   */
  pageReader: GBrainPageReader
...
/** Typed typed-edge write surface (no op names reachable by the caller). */
export interface GBrainLinkWriter {
  addLink(input: { from: string; to: string; link_type: string; context: string }): Promise<void>
}

/** A page as GBrain stores it: the parsed frontmatter block plus the body. */
export interface GBrainStoredPage {
  frontmatter: Record<string, unknown>
  body: string
}

/** Typed single-page read surface (no op names reachable by the caller). */
export interface GBrainPageReader {
  /** Resolves the stored page, or null when the slug does not exist. */
  getPage(slug: string): Promise<GBrainStoredPage | null>
}

/**
 * Normalize whatever `get_page` returned into {@link GBrainStoredPage}, or null
 * when the slug is absent.
 *
 * GBrain does NOT store a page as an opaque blob: `put_page` parses the markdown
 * and returns the YAML frontmatter as a structured `frontmatter` object with the
 * remaining body under `compiled_truth` (observed directly against a real brain
 * during a 1179-page external memory import, 2026-07-28). The other field names
 * are defensive fallbacks for non-gbrain shapes; this must never throw, because
 * a read-back verifier that crashes on an odd shape reports a phantom failure.
 */
function normalizeStoredPage(res: unknown): GBrainStoredPage | null {
  if (res === null || res === undefined || typeof res !== 'object') return null
  const outer = res as Record<string, unknown>
  // Some transports wrap the row (`{page: {...}}` / `{result: {...}}`).
  const inner =
    typeof outer['page'] === 'object' && outer['page'] !== null
      ? (outer['page'] as Record<string, unknown>)
      : typeof outer['result'] === 'object' && outer['result'] !== null
        ? (outer['result'] as Record<string, unknown>)
        : outer
  const fmRaw = inner['frontmatter']
  const frontmatter =
    typeof fmRaw === 'object' && fmRaw !== null && !Array.isArray(fmRaw)
      ? (fmRaw as Record<string, unknown>)
      : {}
  const bodyRaw = inner['compiled_truth'] ?? inner['content'] ?? inner['body'] ?? inner['text']
  if (typeof bodyRaw !== 'string' && Object.keys(frontmatter).length === 0) {
    // Neither a body nor frontmatter — this is an absent page, not a malformed
    // one. Say "missing" rather than inventing an empty page that would let a
    // verifier pass over a slug that never landed.
    return null
  }
  return { frontmatter, body: typeof bodyRaw === 'string' ? bodyRaw : '' }
}
...
export function buildGBrainMemory(input: {
  ...
  // The two narrow typed seams. They close over the SAME `client` (one
  // `gbrain serve` child, one init guard, one `close`) — there is deliberately
  // no second construction path for a caller that needs edges or a per-slug
  // read. The raw op names stay inside this exempt file.
  const linkWriter: GBrainLinkWriter = {
    addLink: async (input): Promise<void> => {
  ...
  const pageReader: GBrainPageReader = {
    getPage: async (slug): Promise<GBrainStoredPage | null> => {
      // A slug that does not exist is a NULL, not an exception — gbrain reports
      // absence by throwing on some paths and by an empty result on others, and
      // the verifier's whole job is to distinguish "absent" from "broken read".
      // Only a genuine transport failure propagates.
      let res: unknown
      try {
        res = await client.call('get_page', { slug })
      } catch (err) {
        const msg = err instanceof Error ? err.message : String(err)
        if (/not found|no such page|does not exist/i.test(msg)) return null
        throw err
      }
      return normalizeStoredPage(res)
    },
  }

  // `client` stays a LOCAL — used to construct memoryStore + syncHook + the two
  // narrow seams and for the close closure, but never surfaced (see
  // GBrainMemoryWiring).
  return {
    memoryStore,
    syncHook,

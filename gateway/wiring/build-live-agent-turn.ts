 ...
 * a token refresh — no changes needed).
 *
 * ONE-CLICK RECONNECT (2026-07-24, this build): the bubble now carries a "Reconnect"
 * button (value `RECONNECT_AUTH_VALUE`). A tap routes back through the turn pipeline
 * (like the freeze-timeout Retry button) but does NOT re-run the last message —
 * instead the runner mints a FRESH install-token handoff on demand (the SAME
 * `/initiate` flow the first-time auth gate drives, via the `reconnectHandoff` seam)
 * and replies in-chat with the copy-paste terminal command. `claude setup-token`
 * itself opens the real browser OAuth consent, so this IS the "opens a browser and I
 * auth" flow Ryan asked for, within what Anthropic's CLI supports (a terminal trigger
 * is unavoidable — the token is captured from the CLI's stdout). The static body text
 * below stays the durable-history fallback for a client that can't tap.
 */
...
/**
 * Reconnect-affordance routing value (2026-07-24) — the "Reconnect" button's `value`
 * on the auth-reconnect bubble (`AUTH_RECONNECT_BODY`). A tap routes it back as the
 * next turn, and the runner special-cases it: instead of re-running the last message
 * (that would just hit the same invalid token), it mints a FRESH install-token handoff
 * on demand (`reconnectHandoff` seam — the SAME `/initiate` flow the first-time auth
 * gate drives) and replies in-chat with the copy-paste terminal command. Short
 * (≤ VALUE_BYTE_CAP), NOT a reserved option value (so it validates) and NOT a
 * forbidden inbound sentinel (so the tap is not rejected at the wire guard).
 ...
 * one-liner, wrapped in the SAME instruction shape the first-time auth gate page uses
 * ("In a terminal on this machine, run: …") so the on-demand reconnect UX matches the
 * onboarding step it reuses. The command runs `claude setup-token` (which opens the
 * real browser OAuth consent), captures the new token, and reconnects Neutron
 * automatically (a one-time restart). Exported for unit testing.
 */
export function buildReconnectCommandBody(command: string): string {
  return [
    'To reconnect, run this in a terminal on the machine running Neutron. It opens a',
    'browser to sign in to Claude, captures a fresh token, and reconnects Neutron',
    'automatically (it restarts once, then you can send your message again):',
    '',
    '```',
    command,
    '```',
...
export interface BuildLiveAgentTurnInput {
  ...
  /**
   * On-demand Claude reconnect (2026-07-24). When the owner taps the "Reconnect"
   * button on an auth-reconnect bubble (`turn.user_text === RECONNECT_AUTH_VALUE`),
   * the runner calls this to mint a FRESH install-token handoff — the SAME
   * `/initiate` → capture-token → restart flow the first-time auth gate drives — and
   * replies in-chat with the returned copy-paste terminal command. Returns the
   * command (else `null` when the handoff couldn't be minted). Optional: when the
   * seam is omitted the auth-reconnect bubble ships buttonless (the static manual
   * instructions), so no dead button is ever offered. Best-effort: a throwing seam
   * degrades to the static reconnect body, never kills the turn.
   */
...
export function buildLiveAgentTurn(
  ...
  async function runTurnBody(
    ...
    // "Reconnect" button routes back `RECONNECT_AUTH_VALUE`. Unlike the Retry
    // affordance below (which re-runs the last real message), this is an ACTION —
    // mint a FRESH install-token handoff (the SAME `/initiate` → capture-token →
    // restart flow the first-time auth gate drives) and reply in-chat with the
    // copy-paste terminal command. Intercepted HERE, before user-turn persistence
    // and the last-message recording, so the opaque sentinel never renders as a
    // user bubble nor becomes a later Retry's recovered text. The LLM is never
    // dispatched. A seed turn never carries this value.
    if (turn.seed_turn !== true && turn.user_text === RECONNECT_AUTH_VALUE) {
      await sendReconnectCommand(input.buttonStore, turn, input.reconnectHandoff)
 ...
 * reload re-hydrates the actionable message, not a ghost) and ships it live.
 *
 * When `includeReconnectButton` is true (the `reconnectHandoff` seam is wired — the
 * live-agent path), the bubble carries a single "Reconnect" button whose value is
 * `RECONNECT_AUTH_VALUE`. Unlike a Retry button this does NOT re-run the failed turn
 * (that would just hit the same invalid token): a tap routes back through the turn
 * pipeline, where the runner mints a fresh install-token handoff and replies with the
 * copy-paste reconnect command (see `sendReconnectCommand`). When the seam is absent
 * the bubble ships buttonless so no dead button is offered. `allow_freeform` stays
 * true either way so the owner can send again once they have reconnected. When the
 * persist fails we still ship the live envelope. Called ONLY for a real (non-seed)
 * user turn.
 */
...
/**
 * Handle a tap on the auth-reconnect "Reconnect" button. Mint a fresh install-token
 * handoff via the injected `reconnectHandoff` seam (the SAME `/initiate` flow the
 * first-time auth gate drives) and reply in-chat with the copy-paste terminal command,
 * wrapped in the gate's "In a terminal on this machine, run: …" UX (see
 * `buildReconnectCommandBody`). When the seam is absent or throws (LLM-less box / mint
 * failure), degrade to the static `AUTH_RECONNECT_BODY` manual instructions rather
 * than a dead end. Persists the reply as a durable inert agent turn so a reload
 * re-hydrates the command. The LLM is never dispatched for a reconnect tap.
 */
async function sendReconnectCommand(
  buttonStore: ButtonStore,
  turn: LiveAgentTurnRequest,
  reconnectHandoff: (() => Promise<{ command: string } | null>) | undefined,
): Promise<void> {
  let body = AUTH_RECONNECT_BODY
  if (reconnectHandoff !== undefined) {
    try {
      const minted = await reconnectHandoff()
      if (minted !== null && minted.command.length > 0) {
        body = buildReconnectCommandBody(minted.command)
      }
    } catch (err) {
      moduleLog.warn('reconnect_handoff_failed', {
        project: turn.project_slug,
        topic: turn.topic_id,
        error: err instanceof Error ? err.message : String(err),
      })
  ...
  try {
    await buttonStore.persistInertAgentTurn({ topic_id: turn.topic_id, body })
  } catch (err) {
    moduleLog.warn('reconnect_command_persist_failed', {
      project: turn.project_slug,
      topic: turn.topic_id,
      error: err instanceof Error ? err.message : String(err),
    })

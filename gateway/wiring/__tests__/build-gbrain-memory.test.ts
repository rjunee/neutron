...
describe('buildGBrainMemory', () => {
  test('returns ONLY the typed seams (no raw transport) + a close() that resolves', async () => {
    ...
    // The two narrow typed seams the the legacy harness memory import lane consumes. One
    // named method each — no way to name an arbitrary op through them.
    expect(typeof wiring.linkWriter.addLink).toBe('function')
    expect(typeof wiring.pageReader.getPage).toBe('function')
    expect(Object.keys(wiring.linkWriter)).toEqual(['addLink'])
    expect(Object.keys(wiring.pageReader)).toEqual(['getPage'])
    ...
    // typed seams — still no `client`. `linkWriter`/`pageReader` are likewise
    // one-method TYPED contracts, not the raw op-name transport.

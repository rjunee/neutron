...
describe('synthesizeMatchFromSignal (GAP2)', () => {
  test('returns null when there is no related signal', () => {
    expect(
      synthesizeMatchFromSignal('Wellness-lab', { entities: [], topics: [], interests: [] }),
    ).toBeNull()
  })

  test('synthesizes a matched-shaped object from topics/interests/entities', () => {
    const out = synthesizeMatchFromSignal('Wellness-lab', {
      entities: ['Wellness-lab gear'],
      topics: ['Wellness-lab cold plunge protocol'],
      interests: ['wellness-lab'],
    })
    expect(out).not.toBeNull()
    expect(out!.name).toBe('Wellness-lab')
    expect(out!.rationale.toLowerCase()).toContain('wellness-lab')
    // suggested_topics drive the next-move pick; topics rank first.
    expect(out!.suggested_topics[0]).toBe('Wellness-lab cold plunge protocol')

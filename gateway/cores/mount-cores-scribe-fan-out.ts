 ...
 *
 * Gap #1 (the legacy harness→Neutron parity, 2026-06-25 scan §5.1): the `scribeFanOut` seam
 * is fully built — Calendar scheduling and the email pipeline accept a
 * `scribeFanOut?` and call it with the already-fetched, flattened
 ...
 *      shape the producers expect, fire-and-forget + error-swallowing
 *      so a failed extraction never throws into a Core's brief/poll path. It
 ...
 *      from the live Google client and starts it; email reuses the binding through
 *      its existing pipeline poller. `stop()` drains in-flight extractions and
 *      tears the scheduler + handles
 *      down. The Calendar scheduler owns its own self-tick (no external loop, no new
 *      poller — preserving the "no duplicate poller" invariant the phase-2
 *      static tests guard).
 *
 * WHY LATE-BINDING (the reflectLoop precedent): the scheduler needs the live
 * OAuth-backed `CalendarClient`, which is built by
 ...
 * composer calls `arm({ calendarClient })` once `mountOpenCores`
 ...
 * `CalendarClient` when the owner has connected Google, and an in-memory fallback
 * otherwise. `arm` always receives a real client instance; with the fallback the
 * scheduler runs harmlessly against an empty calendar and fans out nothing. The
 * composer only mounts this when scribe is live (it
...
export interface MountedCoresScribeFanOut {
  /**
   * Build the calendar scheduler from the live Google client and START it. Call
  ...
  /** The callback threaded into the email pipeline's existing poller. */
  readonly fanOut: ScribeFanOut
  /** Await all in-flight fan-out extractions (shutdown drain + test sync). */
  idle(): Promise<void>
  /** Stop the scheduler (no-op before `arm`), drain in-flight extractions, close handles. */
  stop(): Promise<void>
}

/**
 * CONSTRUCT the Cores fan-out binding and Calendar scheduled fan-out into scribe
 ...
 * the Calendar scheduler once the live Google client exists, and whose `stop()` is
...
export function mountCoresScribeFanOut(
  ...
  async function stop(): Promise<void> {
    if (stopped) return
    stopped = true
    // The `start()` is awaited before `stop()`: the initial tick's `fire()` →
    ...
    // The initial tick has settled (its fan-outs are queued); drain all
  ...
  function arm(clients: ArmCoresScribeFanOutInput): void {
    ...
    try {
      ...
      // Start the calendar scheduler. Email extraction rides the existing email
      // pipeline poller through `fanOut`; no second email scheduler remains.
      startCalendar = calendarScheduler.start().catch((err: unknown) => {
        logFailure('calendar scheduler start failed', err)
      })
    } catch (err) {
      // Failure-atomic: a synchronous throw while building/starting the
      // scheduler must not leak it (mirrors reflectLoop's

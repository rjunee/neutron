...
export function composeReplModelSurface(opts: {
  ...
  readCodex?(projectId: string | null): Promise<ReplModelState>
  switchCodex?(projectId: string | null, request: ReplModelSwitch): Promise<ReplModelState>
}) {
  return createAppReplModelSurface({
    ...
    read: async (scope) => {
      ...
      if (provider === 'openai-codex') {
        if (opts.readCodex) return opts.readCodex(scope.projectId)
    ...
    switch: async (scope, request) => {
      if (opts.provider(scope.projectId) === 'openai-codex' && opts.switchCodex) {

 ...
 * the the legacy harness-parity daily brief: it composes from whatever live context is
...
/** System prompt for the LLM brief — lifts the legacy harness's morning-brief intent
 *  (`~/legacy/prompts/morning-brief.md`): structured, <20 lines, evidence-only,
...
export async function runMorningBrief(deps: MorningBriefDeps): Promise<MorningBriefResult> {
  ...
  // Prefer the LLM composition over real sources (the legacy harness parity); the pure
  // template is the deterministic fallback when no LLM is wired or it fails

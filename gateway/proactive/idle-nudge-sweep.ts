...
/** Default sweep cadence — hourly (the legacy harness parity). */
...
/**
 * The the legacy harness dual-rating quality floor. A candidate nudge is rated 1–10 on TWO
 * dimensions; BOTH must be ≥ this to post. Ported verbatim from
 * `~/legacy/prompts/re-engagement-nudge-agent.md` ("If EITHER rating is below 7,
 ...
 * the legacy harness "output NONE" path) — treated as a skip, never a post. Production
...
export interface IdleNudgeSweepDeps {
  ...
  /**
   * Optional dual-rating quality gate (the legacy harness parity). When supplied, a
 ...
 * "output NONE") or either dimension below the floor rejects. the legacy harness parity:
...
/** System prompt for the LLM nudge rater — the the legacy harness "Quality floor" audit. */
 ...
 * skip) — the safe default, mirroring the legacy harness's "output NONE when uncertain."
 */
...
export async function runIdleNudgeSweep(
  ...
  for (const candidate of candidates) {
    ...
    // Dual-rating quality gate (the legacy harness parity) — only when a rater is wired.

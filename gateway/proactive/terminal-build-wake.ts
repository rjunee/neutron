...
export function buildTerminalBuildWakePrompt(args: { run: TridentRun; board_item_id: string | null }): string {
  ...
  const escalationInstruction = escalation === null ? null : [
    ...
    escalation.kind === 'missing-dependency'
      ? 'Read `work_board_list` and the linked specs to independently identify the blocked card and its dependency. If an existing dependency must precede the blocked card, use `work_board_reorder` with the dependency id and `precedes` set to the blocked card id. This atomically ensures earlier ordering, leaves an already earlier dependency unchanged, and returns the actual sequencing report; include that report in your reply. Keep the blocked card blocked until the dependency is satisfied. If the dependency has no card, follow intake: spec first, then card. If the identity or product requirement is unclear, report the specific decision needed; do not guess a card from the run payload.'

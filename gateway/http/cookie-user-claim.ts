 ...
 * `owner_handle`; after a post-onboarding slug rename the two
...
export interface CookieClaimRegistry extends OwnerHandleLookup {
  getByOwnerHandle(
    owner_handle: string,
  ): { owner_handle: string; url_slug: string; owner_user_id: string | null } | undefined
}
...
export function buildCookieUserClaim(
  ...
  const log_owner = opts.log_owner ?? opts.owner_handle
  ...
  return async (req: Request): Promise<CookieUserClaim | null> => {
    ...
    const row = opts.registry.getByOwnerHandle(opts.owner_handle)
    ...
    if (ownerIdentityMismatch(cookieSlug, row.owner_handle, resolveOwnerHandle)) {
      return null
    }

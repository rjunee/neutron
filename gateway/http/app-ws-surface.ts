 ...
 */
/**
 * The scheme the CLIENT actually spoke, which is NOT always the scheme this
 * process received. In production Caddy terminates TLS and proxies onward over
 * plain-HTTP loopback, so `new URL(req.url).protocol` reads `http:` for a
 * request the browser made over `https:`.
 *
 * Prefer `X-Forwarded-Proto`, take its FIRST comma-separated token (a proxy
 * chain appends rather than replaces), and TRUST it only when it is exactly
 * `http`/`https`; otherwise fall back to the actual request (socket) scheme,
 * else `http`.
 *
 * On the trust question: this header is only reachable from the reverse proxy
 * on the deployments that use one. It is the same signal `requestSelfOrigin`
 * has always used for the same-origin check, and the failure mode is
 * deliberately asymmetric — a spoofed `https` can only ADD the `Secure` cookie
 * attribute (which fails closed: the browser then withholds the cookie over
 * plain http), never remove it. A direct-bind self-hoster sends no such header
 * and keeps the plain-http behaviour.
 */
/**
 * The app-ws topic a socket binds to. A `project_id` scopes the socket to that
 * project (`app:<user>:<project>`); its absence is General (`app:<user>`).
 *
 * ISSUES #399 — this used to be gated on `platform === 'web'`, leaving native
 * on a single `app:<user>` socket that was supposed to switch by reading a
 * `project_id` FIELD off each frame. The frames carry no such field, so native
 * received General's transcript with no way to tell projects apart and its
 * per-project history was never delivered.
 *
 * The gate was removed rather than the field added because `seq` is allocated
 * PER TOPIC (the store indexes `(topic_id, seq)` and resumes from
 * `lastSeenSeq(topic_id)`). One topic for every project would mean ONE cursor
 * for the whole account, so opening a single project would replay everything
 * and filter client-side — O(account) per open, forever. Per-topic scoping is
 * O(that project), and is what the server already PERSISTED to; only delivery
 * was gated.
 *
 * Extracted so the invariant is testable and stated once. All platforms share
 * it, and clients derive the same keys from `wire-types/topic-id.ts`.
 */
export function resolveChannelTopicId(user_id: string, project_id: string | null): string {
  return project_id !== null && project_id.length > 0
    ? appWsProjectTopicId(user_id, project_id)
    : appWsTopicId(user_id)
}

/**
 * Should a session cookie carry `Secure`?
 *
 * FAILS CLOSED (ISSUES #392). The previous rule derived `Secure` from
 * {@link resolveRequestScheme}, which prefers `X-Forwarded-Proto` — so a
 * client-supplied `X-Forwarded-Proto: http` could STRIP `Secure` from the
 * owner's session cookie. Not exploitable on the hosted deployment (Caddy
 * overwrites the client value; verified live 2026-07-25), but Open is
 * self-hosted software and we do not control every proxy in front of it.
 *
 * The polarity is now inverted: `Secure` is the DEFAULT, and it is omitted only
 * for a request that is demonstrably a plain-http LOOPBACK dev session:
 *
 *   - no `x-forwarded-*` header at all — any of them means a proxy is in front,
 *     and a proxied request is never a direct dev session. Crucially this keys
 *     on PRESENCE, not value, so an attacker-supplied `X-Forwarded-Proto: http`
 *     now forces `Secure` ON rather than off;
 *   - the browser-supplied `Host` is loopback (`localhost`, `127.0.0.0/8`,
 *     `[::1]`) — not the socket bind, which is loopback on every hosted install
 *     that sits behind a TLS-terminating proxy (that is exactly #303);
 *   - and the socket scheme is plain http.
 *
 * Loopback has to stay exempt because a `Secure` cookie is dropped by browsers
 * over plain http, which would lock a self-hoster out of `bun start` on
 * 127.0.0.1 — the reason the original polarity was chosen.
 *
 * CONSEQUENCE, stated rather than buried: a self-hoster serving plain http on a
 * NON-loopback address (a LAN IP, say) now gets a `Secure` cookie the browser
 * will refuse to store, and must terminate TLS. That is the intended direction
 * of the trade — an unexpected deployment gets a protected cookie and a visible
 * failure, instead of an unprotected cookie and a silent one.
 */
export function shouldSetSecureCookie(req: Request): boolean {
  for (const [name] of req.headers) {
    if (name.toLowerCase().startsWith('x-forwarded-')) return true
  }
  if (resolveRequestScheme(req) === 'https') return true
  return !isLoopbackHost(requestHostForCookiePolicy(req))
}

/**
 * The host to judge loopback-ness by: the browser-supplied `Host` header when
 * present, else the request URL's own host.
 *
 * `Host` is preferred because it is what the cookie is actually scoped to. The
 * URL fallback matters for a direct-bind server whose client sent no `Host`
 * (and for a constructed `Request` in a test, where the header is absent). It
 * cannot re-open the #392 hole: a proxied request is already forced Secure by
 * the `x-forwarded-*` check above, which every real reverse proxy sets.
 */
function requestHostForCookiePolicy(req: Request): string | null {
...
/** True for a `Host` header naming the local machine. Port is ignored. */
export function isLoopbackHost(host: string | null): boolean {
  ...
  if (trimmed.startsWith('[')) {
    ...
    return inner === '::1' || inner === '::ffff:127.0.0.1'
  }
...
export function resolveRequestScheme(req: Request): 'http' | 'https' {
  ...
  return fwdFirst === 'http' || fwdFirst === 'https'
    ? fwdFirst
  ...
  return {
    ...
    handler: async (req, server) => {
      ...
      if (pathname === '/ws/app/chat') {
        ...
        const channel_topic_id = resolveChannelTopicId(resolved.user_id, project_id)

...
/**
 * Slug → display name. Exported ONLY so the external-import drift-guard
 * (the importer test suite (removed from this repo)) can assert the importer's
 * leaf-local copy stays byte-identical to this one — same arrangement as
 * `slugifyProjectId` / `defaultProjectIdSlugifier`. Not part of a public API.
 */
export function humaniseProjectId(project_id: string): string {
...
export class InMemoryProjectSettingsStore implements ProjectSettingsStore {
  ...
  async get(project_slug: string, project_id: string): Promise<ProjectSettings | null> {
    ...
    // Twin of the SQLite store (ISSUES #412): synthesise a canonical doc so the
    // settings drawer renders, but do NOT insert it. `list()` reads `rows`, so
    // inserting here would make a READ create a project — which is exactly how
    // a stray navigation manufactured a duplicate "General" in production.
    // The row appears on the first real write (`update`), not on a GET.
  ...
  async update(
    ...
    if (
      ...
      // this the seam would create where production does not — the exact
      // seam/production divergence that lets a surface test pass on behaviour
      // the real store never has.
      return current
    }

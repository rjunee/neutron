...
async function seededStore(owner: string, ids: string[]): Promise<InMemoryProjectSettingsStore> {
  ...
  // A project comes into existence on a WRITE. This used to call `get`, back
  // when a read auto-created the row — the ISSUES #412 defect. The name is the
  // same one `buildDefaultSettings` derives, so every assertion below is
  // unchanged; only the verb that creates the row is.
  for (const id of ids) await store.update(owner, id, { name: buildDefaultSettings(id).name })
  return store
}

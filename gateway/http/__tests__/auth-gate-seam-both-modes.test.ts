 ...
 *              non-resumable cookie cold-starts; bare `GET /` routes into
 *              openFetch (cold-start with no cookie, `302 /chat` with one) —
 *              see the ISSUES #367 note below, where this suite used to pin the
 *              opposite because it characterized `main`'s 404; SPA deep links
 *              mint+bounce; `/api/app/*` is not gated by the owner gate.
 */
...
describe('C5b seam — OPEN mode (anonymous single-owner) through the real compose chain', () => {
  ...
  // ── bare GET / reaches openFetch (ISSUES #367) ──
  //
  // THIS TEST PREVIOUSLY ENCODED A DEFECT. It was written as a pure
  // CHARACTERIZATION test: C5b's job was to prove the seam refactor reproduced
  // `main` EXACTLY, so it asserted whatever `main` did — and on `main` the
  // gate's `invokesOpenFetch` predicate never matched a bare `/`
  // (`isLandingRoute('/','GET',false)` is false because the landing page is
  // invite-gated), so the root fell through to the default handler and 404'd.
  // The old assertions (`defaultCalled === true`, `status === 404`) therefore
  // pinned the bug in place: that 404 IS ISSUES #367 — typing your own
  // instance URL returned "Not Found" and only `/chat` worked. Nothing in the
  // old comment ever argued the 404 was DESIRABLE; it only explained the
  // mechanism that produced it.
  //
  // The gate now routes bare `GET /` into `openFetch`, so these tests assert
  // the CORRECTED target: the root behaves exactly like the already-working
  // `/?invite=` root — cold-start for a fresh visitor, `302 /chat` with a valid
  // owner cookie — and NEVER reaches the default handler. The routing is still
  // pinned precisely; only the destination changed.
  test('bare GET / (no ?invite, no cookie) → openFetch `/` cold-start 302 to /chat?start= + owner cookie, NOT the default handler', async () => {
  ...
  test('bare GET / WITH a valid owner cookie → openFetch `/` branch 302 to /chat', async () => {
    let defaultCalled = false
    const { handler } = buildOpenHandler({
      defaultHandler: () => {
        defaultCalled = true
    ...
    const res = await handler.fetch(openReq('/', OPEN_SLUG), FAKE_SERVER as Server<never>)
    expect(res.status).toBe(302)
    expect(res.headers.get('location')).toBe('/chat')
    expect(defaultCalled).toBe(false)
  })

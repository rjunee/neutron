...
function modelError(status: number, code: string, message: string): Response {
...
export function createAppReplModelSurface(opts: AppReplModelSurfaceOptions) {
  return {
    handler: async (req: Request): Promise<Response | null> => {
      ...
      if ('code' in auth) return modelError(401, auth.code, auth.message)
      const segment = resolveScopeSegment(match[1])
      if (segment === null) return modelError(400, 'invalid_project_id', 'Invalid project scope')
      ...
      try {
        if (!await opts.canAccess(scope)) return modelError(404, 'project_not_found', 'Project not found')
        ...
        if (req.method === 'GET') {
          ...
          if (body === null || typeof body !== 'object' || Array.isArray(body)) {
            return modelError(400, 'invalid_request', 'Expected model and sessionId')
          }
          const { model, sessionId } = body as Record<string, unknown>
          if (typeof model !== 'string' || model.trim().length === 0 ||
              typeof sessionId !== 'string' || sessionId.length === 0) {
            return modelError(400, 'invalid_request', 'Expected nonempty model and sessionId')
          }
          // Runtime owns the mutex, offered-model validation and native acknowledgement.
          state = await opts.switch(scope, { model, sessionId })
        } else {
          return modelError(405, 'method_not_allowed', 'Expected GET or POST')
        }
        ...
        if (error instanceof ReplModelError) {
          ...
          return modelError(status, error.code, error.message)
        }
        return modelError(503, 'unavailable', 'Live model control is unavailable')
      }
    },
  }
}

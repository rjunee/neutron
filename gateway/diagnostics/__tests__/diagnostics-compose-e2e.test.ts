...
function composed(resolverSlug: string = SLUG): ComposedHttpHandler {
  const surface = createAppDiagnosticsSurface({
    ...
    // A REAL store on a throwaway path — the dependency is required (remote
    // diagnostics ships ON), and this suite exercises the READ route only.
    client_reports: new FileClientReportStore({
      path: join(mkdtempSync(join(tmpdir(), 'neutron-diag-e2e-')), 'client-reports.jsonl'),
    }),
  })

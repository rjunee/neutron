...
describe('app-ws gateway surface — WS /ws/app/chat', () => {
  ...
  // ISSUES #364 — an EXPLICIT budget for the one test that flaked. Measured:
  // with `waitFor`'s private cap removed, a never-satisfied wait fails via
  // bun's per-test default at 5007ms. That default is only ~3x the old 1500ms
  // cap, so it would remain the binding constraint under full-suite load — the
  // very condition that produced the intermittent red. Stating the budget here
  // makes it visible and adjustable instead of inherited, and 20s is generous
  // for a loopback WS upgrade + round-trip while still failing a real break
  // promptly.
 ...
 *
 * ISSUES #364 — the cap used to be 1500ms, which made THIS helper the binding
 * constraint on every WS assertion in the file and produced an intermittent red
 * line under full-suite load (a WS upgrade + round-trip competing with many
 * spawned subprocesses). It failed once in three full-suite runs and passed 3/3
 * in isolation.
 *
 * The cap is now deliberately NON-BINDING. Bun's own per-test timeout is the one
 * budget that governs, so a genuinely broken round-trip still fails — just via
 * the runner's timeout rather than a second, tighter, invisible one. Two
 * competing deadlines where the smaller is a private constant is the shape that
 * makes a deterministic assertion look flaky.
 *
 * NOTE the assertions here are deterministic (`session_ready` either arrives or
 * does not), which is why widening the allowance is the right move — unlike the
 * anchor-race test in ISSUES #408, whose ASSERTION is nondeterministic and which
 * no timeout could ever fix.
 */

...
async function start(): Promise<Harness> {
  ...
  const server = Bun.serve({ port: 0, fetch: (req, srv) => composed.fetch(req, srv), websocket: composed.websocket })
  cleanups.push(() => {
    void server.stop(true)
    db.close()
  })
  return { server, base: `http://127.0.0.1:${server.port}`, db, close: async () => { await server.stop(true) } }
}

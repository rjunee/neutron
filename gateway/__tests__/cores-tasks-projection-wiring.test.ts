...
/**
 * Wait for the debounced projection writer to flush, then return STATUS.md.
 *
 * The writer is debounced (30ms here) and flushes asynchronously, so a FIXED
 * sleep is a race: it only ever bought ~90ms of slack, and on a loaded CI
 * runner the flush landed after it — `readFileSync` then threw ENOENT before
 * a single assertion ran (observed on shard 4/4, run 30419827372). Reproduced
 * locally by shortening the sleep: identical ENOENT, identical line.
 *
 * Polling removes the timing dependence WITHOUT relaxing anything. It returns
 * as soon as the file contains every expected row, and on timeout returns
 * whatever is there (or '') so the caller's `toContain` assertions still fail
 * loudly with a readable diff. A genuinely broken subscriber chain — the
 * regression this file exists to catch — still fails, it just takes the full
 * deadline to do it.
 */
async function statusAfterFlush(
  path: string,
  ...
  for (;;) {
    ...
    if (content !== '' && expected.every((row) => content.includes(row))) return content
...
describe('cores-tasks wiring — projection writer fires on Core-driven tasks_create', () => {
  ...
  test('tasks_create via the Core surface rewrites STATUS.md within the debounce window', async () => {
    ...
    const status = await statusAfterFlush(statusPath, ['- [ ] core-driven task [P1]'])
    expect(status).toContain('- [ ] core-driven task [P1]')

    const actions = await statusAfterFlush(actionsPath, ['- [ ] core-driven task [P1]'])
  ...
  test('a Core write + an immediate canonical write coalesce to one STATUS.md rewrite', async () => {
    ...
    const statusPath = join(harness.owner_home, 'Projects', PROJECT, 'STATUS.md')
    const status = await statusAfterFlush(statusPath, [
      '- [ ] core-write [P1]',
      '- [ ] canonical-write [P2]',
    ])

...
/**
 * Bring a project into existence.
 *
 * These tests used to do this with a settings GET, back when reading an
 * unknown project_id auto-created it. That behaviour was the ISSUES #412
 * defect (a stray navigation manufactured a real project in production), so a
 * read no longer creates anything and the setup has to write. The name matches
 * `buildDefaultSettings` exactly, so every downstream assertion about names and
 * emoji is unchanged — only the verb that creates the row is.
 */
async function createProject(store: InMemoryProjectSettingsStore, id: string): Promise<void> {
  await store.update(PROJECT_SLUG, id, { name: buildDefaultSettings(id).name })
}
...
describe('app-projects surface — GET /api/app/projects (list)', () => {
  ...
  it('returns every project that exists', async () => {
    await createProject(harness.store, 'neutron')
    await createProject(harness.store, 'acme')
    const res = await authedFetch(harness.base, `/api/app/projects`)
    expect(res.status).toBe(200)
    const json = (await res.json()) as ListResponse
...
describe('app-projects surface — archive / restore / list-archived (0095)', () => {
  ...
  it('archive removes a project from the list and surfaces it under /archived; restore returns it', async () => {
    await createProject(harness.store, 'keep')
    await createProject(harness.store, 'gone')

...
/**
 * Default upstream port used when neither a `--port=N` argv flag nor a
 * `NEUTRON_PORT` env override is set. Production deployments set the port via
 * the `NEUTRON_PORT` env var — Open's `install.sh` exports it into the
 * launchd/systemd service, and a hosted deployment's unit sets it in the unit
 * env (neither passes a `--port=` argv flag). So this default only fires under
 * a direct `bun run gateway/index.ts` dev run with no env set.
 */
export const DEFAULT_LISTEN_PORT = 7_800
 ...
 *   2. `--port=<N>` on `process.argv` (a dev/CLI override; not used by shipped units).
 *   3. `NEUTRON_PORT` env var (the production mechanism — install.sh / the hosted unit env).

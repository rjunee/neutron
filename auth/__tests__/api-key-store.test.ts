...
test('add stores ciphertext via SecretsStore + writes api_keys metadata row', async () => {
  ...
  const list = await api_keys.list({ owner_handle: asOwnerHandle('alice') })
...
test('list filters by project + optional provider', async () => {
  const { api_keys } = buildStores()
  await api_keys.add({ owner_handle: asOwnerHandle('alice'), provider: 'anthropic', label: 'a', plaintext: 'k' })
  await api_keys.add({ owner_handle: asOwnerHandle('alice'), provider: 'openai', label: 'b', plaintext: 'k' })
  await api_keys.add({ owner_handle: asOwnerHandle('bobby'), provider: 'anthropic', label: 'c', plaintext: 'k' })
  const aliceAll = await api_keys.list({ owner_handle: asOwnerHandle('alice') })
  expect(aliceAll.map((r) => r.label).sort()).toEqual(['a', 'b'])
  const aliceAnthropic = await api_keys.list({ owner_handle: asOwnerHandle('alice'), provider: 'anthropic' })
...
test('delete drops both api_keys row AND the underlying secret', async () => {
  ...
  await api_keys.delete({ owner_handle: asOwnerHandle('alice'), provider: 'anthropic', label: 'tmp' })
  const list = await api_keys.list({ owner_handle: asOwnerHandle('alice') })

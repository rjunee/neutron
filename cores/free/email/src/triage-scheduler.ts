...
export function buildTriageScheduler(opts: TriageSchedulerOpts): TriageScheduler {
  ...
  /**
   * Has a triage ALREADY landed for this project on this local day, per the
   * DURABLE store? ISSUES #103.
   *
   * The per-day guard is the in-memory `lastFired` Map, which a gateway restart
   * wipes. `start()` then runs its immediate tick, and a restart during the
   * exact fire minute re-fires the day's triage — a duplicate CHAT POST to the
   * owner. Scribe fan-out is watermark-protected, so nothing corrupts; the
   * duplicate message is the whole defect.
   *
   * `triage_cache` already persists `fired_at` per project, so the restart-proof
   * answer is one query. Consulted ONLY on the immediate tick — deliberately.
   * `upsertTriage` is also written by the manual `/triage` chat command and by
   * the tools surface, and the row carries no source column, so a durable check
   * cannot tell a scheduled fire from a manual one. Applying it to the RECURRING
   * tick would let a manual triage silently suppress the day's scheduled post,
   * which is a behaviour change well beyond this bug. Narrowing it to the
   * restart window fixes exactly what is broken and nothing else.
   */
  async function firedTodayDurably(now: Date): Promise<boolean> {
    try {
      const project_id = await opts.targetProjectId()
      const cache = await opts.cacheFor(project_id)
      const [latest] = cache.listRecentTriage(1)
      if (latest === undefined) return false
      return ymdKey(new Date(latest.fired_at), opts.userTz) === ymdKey(now, opts.userTz)
    } catch {
      // Best-effort: a store read failure must not stop the day's triage. The
      // in-memory guard still covers the common case, and firing twice is far
      // less bad than never firing at all.
      return false
    }
  }
  ...
  const self: TriageScheduler = {
    async start(): Promise<void> {
      ...
      try {
        const now = nowFn()
        // ISSUES #103 — only the IMMEDIATE tick consults the durable store, and
        // only when it would actually fire. A restart during the fire minute
        // otherwise re-posts the day's triage because `lastFired` is in-memory.
        if (!isFireTime(now, opts.userTz) || !(await firedTodayDurably(now))) {
          await self.tick(now)
        }
      } catch {

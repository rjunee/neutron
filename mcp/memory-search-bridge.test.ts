/**
 * P0-2 — `memory_search` over the native-MCP tools-bridge boundary.
 *
 ...
 *
 * Together these prove the agent can DISCOVER + CALL memory_search natively —
...
describe('memory_search over the tools-bridge', () => {
  test('is advertised in the agent manifest alongside the Hermes stubs being hidden', () => {
    ...
    registerMemorySearchToolSurface(reg, recordingStore([]))
    ...
    const schema = server.listToolSchemas().find((s) => s.name === 'memory_search')!
  ...
  test('dispatch routes a native call into the MemoryStore and returns the recall', async () => {
    const seen: string[] = []
    const reg = new ToolRegistry()
    registerMemorySearchToolSurface(reg, recordingStore(seen))
    const server = new McpServer({ project_slug: 'acme', registry: reg })

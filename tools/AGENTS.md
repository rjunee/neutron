...
This module owns the tool registry and per-instance approval gates lifted from Legacy-agent `bash-tools.exec-approval-{request,followup}.ts` (4-runtime-seam shape) plus the auto-discovery pattern lifted from Hermes `tools/registry.py` (zero-config registration). Implementation lands in P1; P0 sets the directory and these rules.

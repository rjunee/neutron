...
### Follow-up: stable-mount collision guard

The stable-mount failure stopped at an obsolete General board expectation: the fixture expected `/projects/general/work-board`, while the shared HTTP resolver produces `/projects/~general/work-board` (`landing/chat-react/general-scope.ts:29-43`, `landing/chat-react/__tests__/stable-mount.test.tsx:338-355`). Correcting that boundary exposes and retains the three requested separation checks. Surface and transcript identity resolve General to `~general` and the named project to `__general__` (`landing/chat-react/ChatApp.tsx:2442-2462`, `landing/chat-react/__tests__/stable-mount.test.tsx:270-272`). Board identity resolves to the corresponding distinct URL segments (`landing/chat-react/__tests__/stable-mount.test.tsx:338-373`). The rendered surfaces are different DOM nodes and the named-project transcript never contains General's message (`landing/chat-react/__tests__/stable-mount.test.tsx:352-369`). None of the three identities collide.

The surface/cache resolver now imports the existing HTTP scope constant instead of owning a fourth spelling (`landing/chat-react/ChatApp.tsx:28-32`, `landing/chat-react/ChatApp.tsx:2442-2462`). The client sentinel remains empty because the board scope is still derived from nullable `hostVm.projectId` (`landing/chat-react/ChatApp.tsx:2919-2927`); only the surface/cache key and HTTP segment use the reserved scope. Continuous separation is maintained by the project-id character constraint and by the resolver applying the reserved constant only to the null/empty class (`channels/adapters/app-ws/envelope.ts:391-400`, `landing/chat-react/general-scope.ts:35-43`). No new error, verdict, state, or refusal was added, so no outcome vocabulary changes.

| Guard | Printed mutation site and change | RED | Restored GREEN |
| --- | --- | --- | --- |
| Stable surface/cache scope resolver | `landing/chat-react/ChatApp.tsx:2452`: replace the shared constant with the legal `__general__` slug | Exact resolver assertion received `__general__` instead of `~general` at `landing/chat-react/__tests__/stable-mount.test.tsx:271` | Stable-mount file passed 2/2, including separate surface, board, and transcript checks |

The five explicitly enumerated affected test files—stable mount, General docs mapping, Work Board client mapping, activity scope mapping, and mobile rail collision—passed 74/74. `bun scripts/ci/stale-prose-guard.ts origin/main` passed. Repository lint passed every reported gate, and the typecheck matrix passed all 51 configurations. A source search for `#general|~general` across `landing/chat-react` and `app/lib/project-rail-view.ts` used the retained `~general` definitions as positive controls; its only remaining `#general` hit explains why that rejected alternative is unsuitable for a mobile URL (`app/lib/project-rail-view.ts:48`). The former web `#general` assertion and definition have no remaining hits. `git diff --check` passed.

Deliberately not built: no special case for the named project, no alternate accepted spelling, no project-id reservation, and no change to the client sentinel. `SPEC.md` and the spec item remain unchanged because this follow-up applies the existing ruling.

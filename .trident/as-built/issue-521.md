...
Final validation: `bun test cores/free/email/__tests__/pipeline-setup.test.ts cores/free/email/__tests__/pipeline-classify.test.ts cores/free/email/__tests__/pipeline-store.test.ts` passed 39 tests; `bunx tsc --noEmit -p cores/free/email/tsconfig.json`, focused ESLint, and `git diff --check` passed. The repository has no `typecheck` script, so `bun run typecheck` reported `Script not found`; the package-scoped TypeScript command above is the available equivalent. The leak gate found zero findings from its runnable checks but reported its local PII denylist unavailable, so it was incomplete rather than clean.

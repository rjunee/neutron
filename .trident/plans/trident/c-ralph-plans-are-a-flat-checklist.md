...
DESIGN LOCK (decided this round; later iterations execute, they do not re-litigate):
...
- [ ] T2: store wave-child rows — migration (next free ordinal, 0132 at planning time; update migrations/expected-schema.txt) adding `parent_run_id`, `wave_task_id` + UNIQUE(parent_run_id, wave_task_id) to `code_trident_runs`; `TridentRunStore` create/list/COLS support, `listChildren(parentId)`, child slugs `<slug>--w<taskId>`; board-facing reads exclude children | requires: none | surface: trident/store.ts, trident/store.test.ts, migrations
- [ ] T3: member-mode inner iteration — `buildWorkflowArgs`/`inner-workflow.mjs` accept a PINNED task (id + member branch): plan seat writes the exec spec for THAT task only (verbatim body, no re-selection), forge builds on the member branch, skips the plan-file write, skips Argus, reports built-terminal with commitSha; `applyResult` treats a child run's built result as terminal done (no merge, no re-fire, no review gate) | requires: T2 | surface: trident/inner-loop.ts, trident/inner-workflow.mjs, trident/inner-workflow.test.ts, trident/orchestrator.ts, trident/orchestrator.test.ts
- [ ] T4: wave-selecting re-fire — `refireNextRalphTask` reads + parses the committed plan (plan-graph), REFUSES cycles loudly (run fails naming the cycle, nothing launches), computes the wave; size ≤1 → byte-exact existing reset; size >1 → cut member branches at run-branch head, idempotently spawn child rows, parent → `wave-wait:<ids>`; MAX_WAVE_SIZE=3; ralph_round counts waves | requires: T1, T3 | surface: trident/orchestrator.ts, trident/orchestrator.test.ts

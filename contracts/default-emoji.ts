/**
 * @neutronai/contracts — deterministic default project emoji (L3 node-free leaf).
 *
 * L3 (2026-07) — moved VERBATIM out of `gateway/projects/default-emoji.ts` into
 * this node-free `contracts` leaf so BOTH the gateway AND `open/composer.ts`
 * (and any future `onboarding` consumer) import the pure glyph heuristic from a
 * leaf instead of reaching across composition bands. Zero dependencies — a pure,
 * deterministic string function. `gateway/projects/default-emoji.ts` re-exports
 * it so existing gateway specifiers stay valid (test-policy §2.2 barrel rule).
 *

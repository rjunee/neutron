...
export const SUBSTRATE_ERROR_CODES: Readonly<Record<SubstrateErrorClass, SubstrateErrorCodeSpec>> = {
  ...
  repl_unreconciled: {
    retryable: true,
    description:
      'The persistent-REPL substrate REFUSED to start or serve a REPL rather than risk two owners on one transcript, or than guess which transcript it is (#539, #676). THREE CAUSES, and the class covers all three: (a) a PREVIOUS REPL for this session key could not be accounted for — a pane it could not inspect, a close it knows failed, a host that cannot reach the pane, or another gateway holding the live ownership claim; (b) a REPL it had just spawned could not have its pane ownership RECORDED, because the registry lock was not acquired — so the child was ended rather than left running unfindable; and (c) the registry could not be READ, so which conversation to resume is UNKNOWN — distinct from a registry that is readable and has no row for this key, which is a true absence and still starts fresh. Causes (b) and (c) were added at r41 and #676 respectively, and each is named here because a class whose description enumerates only some of its causes is a claim the code has already falsified. Starting a second `claude` on one transcript corrupts it, and starting a FRESH one abandons a live transcript, so the turn fails instead. NOTHING TO DO WITH THE CREDENTIAL, and that is why this class exists rather than an unstamped message: an unstamped retryable error maps to a 429-shaped pool cooldown (`mapStatusForPoolCooldown(null, true)`), so a reconciliation problem — or a local file read failure — would have parked a healthy credential, five in a row for an hour. Retryable because the next turn re-probes: a transient failure to see the pane or read the file costs one turn, and a real unreaped owner or a genuinely corrupt registry keeps costing turns until it is resolved.',
  },
  oauth_refresh: {
    retryable: false,

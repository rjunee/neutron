...
export const SUBSTRATE_ERROR_CODES: Readonly<Record<SubstrateErrorClass, SubstrateErrorCodeSpec>> = {
  ...
  repl_unreconciled: {
    retryable: true,
    description:
      'The persistent-REPL substrate REFUSED to start or serve a REPL rather than risk two owners on one transcript (#539). TWO CAUSES, and the class covers both: (a) a PREVIOUS REPL for this session key could not be accounted for — a pane it could not inspect, a close it knows failed, a host that cannot reach the pane, or another gateway holding the live ownership claim; and (b) a REPL it had just spawned could not have its pane ownership RECORDED, because the registry lock was not acquired — so the child was ended rather than left running unfindable. The second cause was added at r41 and is named here because a class whose description enumerates only one of its causes is a claim the code has already falsified. Starting a second `claude` on one transcript corrupts it, so the turn fails instead. NOTHING TO DO WITH THE CREDENTIAL, and that is why this class exists rather than an unstamped message: an unstamped retryable error maps to a 429-shaped pool cooldown (`mapStatusForPoolCooldown(null, true)`), so a reconciliation problem would have parked a healthy credential — five in a row for an hour. Retryable because the next turn re-probes: a transient failure to see the pane costs one turn, and a real unreaped owner keeps costing turns until it is resolved.',
  },
  oauth_refresh: {
    retryable: false,

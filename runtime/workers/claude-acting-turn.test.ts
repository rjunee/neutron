...
test('project runner preserves the correlated provider block and never replays the reserved child', async () => {
  ...
  const recovered = await (await build()).inRepl!.observe!(f.input.request)
  expect(recovered?.usage).toEqual(first.observation?.usage)
  expect(recovered?.model_reported).toBe('provider-model')
  expect(f.commands).toHaveLength(1)
  expect(f.released()).toBe(1)

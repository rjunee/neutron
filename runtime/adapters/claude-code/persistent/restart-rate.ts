 ...
 * of silently churning through restart after restart (the legacy harness mechanism #20,
 * `restart-rate.ts`).
 *
 * Incident this encodes (Nova/the legacy harness "willow" 2026-05-21): the gateway
 ...
 * makes it worse (it wipes in-flight timers, the very bug that lost the willow
...
/** Two restarts closer than this == a crash loop (the legacy harness <5min). */

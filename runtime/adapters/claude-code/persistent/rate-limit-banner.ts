 ...
 *
 * § Terminal-detection port, master-table row #10 (docs/research/legacy-terminal-
 * detection-keystroke-port-2026-06-25.md). Ports the legacy harness's
 ...
 * `(session.scanner ≡ threadId) × (detector id ≡ severity)` — exactly the the legacy harness
 ...
 *     printed can't fire (the the legacy harness PR #117 autonomous-work-brief false-positive).
 ...
 *  (the legacy harness's third severity, `org-monthly-cap`, is fired EXCLUSIVELY by the separate
 ...
 *  tripping. (the legacy harness `RATE_LIMIT_ACTIVE_TAIL_LINES`.) */
 ...
 *  (the `::severity` half of the legacy harness's `${threadId}::${severity}` latch key). */
...
// ---------------------------------------------------------------------------
// Idle-prompt / chrome guard (the legacy harness `IDLE_PROMPT_PATTERN` + `CC_STATUS_LINE_PATTERN`)
// ---------------------------------------------------------------------------
 ...
 *  banner has by definition cleared. Carried verbatim from the legacy harness; the leading
 ...
 *  see — ◆ ADAPTED-AT-BOUNDARY). The trailing `(?:\s|$)` (vs the legacy harness's bare `\s`)
 ...
 * preserving the legacy harness's "no idle prompt → assume active" semantic.
 */

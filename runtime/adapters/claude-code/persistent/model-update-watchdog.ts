 ...
 * sessions onto it (the legacy harness port row #16, docs/research/legacy-terminal-detection-
 * keystroke-port-2026-06-25.md).
 *
 * THE INCIDENT (the legacy harness 2026-04-16): Opus 4.7 shipped overnight and the gateway
 ...
 * first probe (the brief's incident) — the legacy harness's seed-silently-on-first-probe
...
export interface SessionIdleSignals {
  /** A turn is actively streaming (mid-turn) — maps the legacy harness's `isTyping`. */
  isTyping: boolean
  /** An interactive tool-use / menu prompt is up (or recovery in flight) —
   *  maps the legacy harness's `hasToolUsePrompt`. */
 ...
 * the legacy harness `isTopicIdleForUpgrade`):
...
/**
 * Round-robin idle-gated upgrade (mirrors the legacy harness `runGracefulUpgrade`): each round

 ...
 * affordance (the legacy harness port row #13, docs/research/legacy-terminal-detection-
 * keystroke-port-2026-06-25.md).
 *
 * THE INCIDENT (the legacy harness 2026-04-16): a long-lived topic's turn JSONL grew
...
export interface SessionSizeWatchdogDeps {
   ...
   * compaction POLICY (the legacy harness port row #13, gap #4) — it closes the
   ...
   * default) to keep the watchdog surface-only — the legacy harness's gateway, which DOES
...
export function startSessionSizeWatchdog(deps: SessionSizeWatchdogDeps): SessionSizeWatchdog {
  ...
  /**
   * The idle-gated AUTOMATIC compaction POLICY (the legacy harness port row #13, parity gap

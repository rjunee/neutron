 ...
 * (docs/research/legacy-terminal-detection-keystroke-port-2026-06-25.md). Ports
 * the legacy harness's `session-error-watcher.ts` + `pane-scan-watchdog.ts ADDENDUM`.
 *
 ...
 *
 * WHY A JSONL WATCHER, NOT A RING SCAN. The the legacy harness original anchored on the
...
/**
 * The 5xx / overload signatures. CARRIED VERBATIM from the the legacy harness spec
 ...
 * the legacy harness. The allowlist (§3) is the real guard against prose false-fires anyway.
 */
 ...
 * what the legacy harness did ("match error patterns on result / system lines"). It is safe:

...
describe('buildSettings — WAVE 3.5 task B TodoWrite→Work Board PostToolUse hook', () => {
  test('wires a TodoWrite-matched PostToolUse hook ALONGSIDE the Stop hook', () => {
    const dir = freshDir()
    const settingsPath = join(dir, 'settings.json')
    buildSettings({
      settingsPath,
    ...
    // PostToolUse scoped to TodoWrite, sink coords baked as an env prefix.
    expect(parsed.hooks.PostToolUse[0].matcher).toBe('TodoWrite')
    expect(parsed.hooks.PostToolUse[0].hooks[0]).toEqual({
      type: 'command',
      command: 'SINK_PORT=54321 SINK_TOKEN=sink-tok SESSION_ID=sess-9 bun /abs/todo-sync.ts',
    })
  })

  test('todoSync coexists with the permissions block', () => {
    const dir = freshDir()
    const settingsPath = join(dir, 'settings.json')
    buildSettings({
      settingsPath,
      hookPath: '/abs/hook.ts',
      todoSync: { sinkPort: 1, sinkToken: 't', sessionId: 's', hookPath: '/abs/ts.ts' },

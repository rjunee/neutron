...
export function startReplWatchdog(
  ...
  // Restart-rate crash-loop guard (the legacy harness mechanism #20). Each boot records a
  // restart marker; two restarts <5min apart == a crash loop (the 2026-05-21
  // willow signature: restart, then restart again 118s later). Edge-latched →
...
// ---------------------------------------------------------------------------
// Model-update watchdog wiring (the legacy harness port row #16).
// ---------------------------------------------------------------------------

/** Snapshot a pooled warm session's idle signals for the graceful-upgrade gate,
 *  or `null` if the session is gone / its child exited. Reads the four the legacy harness idle
...
/**
 * Start the model-update watchdog for THIS instance (the legacy harness port row #16). A

...
export interface PersistentReplSubstrateOptions {
  ...
  projectsDir?: string
  /** the legacy harness port row #13: notified when a warm session's POST-COMPACT transcript
  ...
  /** Path to the restart-rate crash-loop guard's marker file (the legacy harness mechanism
  ...
  /** the legacy harness port row #16: notified once (edge) when a genuinely-new top-tier

...
/** Quiescence window the size-watchdog's idle-gated auto-compaction (the legacy harness port
...
function dispatchWedgeRecovery(
  ...
  fireAndForget('signatures.runWedgedRecovery', runWedgedRecovery({
    ...
    // re-capture (the the legacy harness tmux `capture-pane` lesson).
...
/** Surface a session-size warn/critical affordance (the legacy harness port row #13). The

...
let spawned = false, childRound = 0, restoredRound = 0
...
try {
  ...
  const started = await call('thread/start', { cwd, model: 'gpt-5.5', modelProvider: 'fixture', approvalPolicy: 'never', sandbox: 'workspace-write' })
  ...
  await lease.start([{ type: 'text', text: 'PARENT_REVIEW_PROBE' }])
  ...
  console.log('PASS: broker provisioned and selected native exact-stage profile; real child stage write allowed; repository, unrelated temporary, canonical writes refused; prior sticky policy restored and verified')
} catch (error) {
  writeFileSync('/tmp/codex-review-permissions-spike-evidence.json', JSON.stringify({ notifications, requests, native }, null, 2))
  throw error
} finally { broker?.close(); transport.close(); fixture.stop(true); rmSync(root, { recursive: true, force: true }) }

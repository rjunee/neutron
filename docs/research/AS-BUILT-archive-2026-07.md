...
`reviewAndSynthesize`). The owner requires a cross-model (OpenAI GPT-5) second opinion,
mirroring the legacy harness, using the ChatGPT SUBSCRIPTION (`codex login`) — NEVER a metered
...
- `trident/codex-review.sh` — a synchronous foreground wrapper (ported from
  the legacy harness's `scripts/codex-review.sh`; NEVER `run_in_background`). Distinguishes,
  ...
  must never be treated as an approval (the legacy harness never-silent-downgrade rule).
...
- **The chokepoint + ask-gate.** `trident/board-dispatch.ts:dispatchBoardBoundBuild`
  ...
  untracked dispatches, owner-locked); (2) the item must EXIST; (3) **ask-before-acting**
...
**What shipped (owner-directed "build the proper full chat log solution").** The
...
**What shipped (AUTH-CORRECTION, owner-locked 2026-06-28).** A fresh Open
...
exact disappointment that gates the owner's fresh reinstall + import.
...
on the owner's live install, `bun install`, `launchctl kickstart -k …neutron-server`
...
**OAuth-vs-embeddings (the owner's steer, verified).** OpenAI browser OAuth
...
**The gap (lift, not reinvention).** the legacy harness exposes real Claude Code skills under
...
- `skills/` (NEW, repo root) — bundled, version-controlled native `SKILL.md`
  packs lifted from the legacy harness `~/.claude/skills/`: `impeccable` + its design
...
is the SAME native loader + `SKILL.md` frontmatter + `Skill` tool the legacy harness's
...
person/company/decision and the agent literally cannot look it up. the legacy harness solves
this with native `mcp__qmd__query` / `mcp__memory-store__search` recall
...
**The gap (lift, not reinvention).** Neutron lifted the legacy harness's spawn argv verbatim
...
which short-circuits the user's `/cmd` BEFORE the model runs. This ports the legacy harness's
multi-server `generateMcpConfig` pattern (`~/legacy/gateway/index.ts:949-968`):
...
Revives two the legacy harness-lifted subsystems that were present but DEAD. No feature flags
— both ship ON once wired. (Audit: `legacy-neutron-architecture-lift-audit-2026-06-27.md`
§ P1-3 + § P1-4.)
...
- **LLM brief composition (the legacy harness parity):** `buildLlmBriefComposer` routes the
...
- **Dual-rating ≥7 quality gate (the legacy harness parity) — ported + tested + wired.**
...
**Scope.** Fix the React chat-client regressions the owner hit dogfooding the local
...
**Scope (owner-locked, final).** Finish the consolidation the prior P1b entry
...
**What shipped (Path A, single-owner localhost trust — owner-locked; no flags, one
...
scope=beta elapsed_ms=6678`, reply "PONG"); the docs tree returns real content
...
**What shipped (no feature flags, no dual path — owner-locked).**
...
fails `no MCP server configured with that name` → `channel-wedged`. Unlike the legacy harness
...
- **`all_messages`** (the DEFAULT, owner-confirmed) — every member post triggers an
...
## 2026-06-26 — Compose Skill Forge into the Open boot path (the legacy harness parity gap #5, the LAST gap)
...
(parity scan `docs/research/legacy-neutron-feature-parity-scan-2026-06-25.md`
...
## 2026-06-26 — IG/X scraping via Apify, optional-until-credentialed (the legacy harness parity gap #6)

**What shipped.** A new Tier 1 free Core `cores/free/scraping/`
(`@neutronai/scraping-core`) porting the legacy harness's `ig-scrape.sh` + `tx-scrape.sh` to
...
**Optional-until-credentialed (cross-cutting pattern, the owner 2026-06-25).** The
...
## 2026-06-25 — model-update watchdog + graceful upgrade (P3, the legacy harness port row #16)
...
every warm session onto it, so the box never drifts on a stale model (the legacy harness
...
## 2026-06-25 — cwd-drift watchdog (P3, the legacy harness port row #12)
...
liveness + `/health` and is blind to this. Ports the legacy harness's `cwd-drift-watchdog.ts`.
...
missing (§2.F / §5.3: "the the legacy harness Forge/Atlas/Sentinel/Argus family is collapsed
...
watchdog tick registered in Open; the rest of the legacy harness's persona set + cross-topic
...
escape→escape→ctrl-c ladder (the owner 2026-06-25 SPEC Decisions Log: detect+RECOVER,
not kill). Ports the legacy harness's `pane-scan-watchdog.ts isWedgedInteractivePrompt /
runWedgedRecovery`.
...
the legacy harness's `cc-update-doctor` (which keeps Claude Code current). Closes the
...
**What shipped.** The substrate the entire the legacy harness→Neutron terminal-detection +
...
- **F1 — public ring-read accessor (`pty-ring.ts`, NEW).** `PtyRing` +
  ...
  guards can see content rendered below the footer (Northwind 2026-06-16).
...
- **F3 — output-scan tick (`output-scan.ts`, NEW).** `OutputScanner` +
  ...
  the legacy harness invariants are baked in: edge-triggered LATCHED firing, doc-quote guards
...
ported signature strings against a live Neutron PTY frame before wiring (the legacy harness's
...
gap #1 of the 2026-06-25 the legacy harness→Neutron parity audit.
...
**What shipped.** The first high-value the legacy harness→Neutron fix-reconciliation pass
(SPEC.md WAVE 2 step 22 checkpoint), porting the legacy harness's fleet premature-completion /
cross-model-review-wedge fixes (the legacy harness PR #164 + #160) onto Neutron's in-process
...
- **`trident/substrate-dispatch.ts` — false-completion race.** A Forge/Argus turn
  ...
  loudly. Open analog of the legacy harness #160 "paused ≠ finished".
- **`trident/prompts.ts` — `FORGE_SYSTEM_PROMPT` cross-model-review hard rule.**
  ...
  analog of the legacy harness #164's `prompts/forge.md` rewrite.

**Why no codex stop-gate pin.** the legacy harness #164 also pinned the openai-codex plugin's
...
audit: `docs/research/legacy-neutron-fix-reconciliation-2026-06-24.md`.

**Tests.** `trident/substrate-dispatch.test.ts` +1 (stream-ends-without-terminal
→ failed); `trident/legacy-fixes.test.ts` +1 `FIX 9` block (3 cases: PR-first
...
workflow the owner develops once becomes a registered, agent-discoverable skill
...
per the owner's SPEC Decisions Log (2026-06-23) **no feature flags** — the plan's
...
**Out of scope (needs the owner's sign-off, per the audit):** P0 make-runner-default,
...
**Out of scope / not needed here:** the the legacy harness-gateway "completed-bullet
...
Evidence-based leanness pass (the owner's mandate, SPEC Decisions Log): delete only
...
captured contemplative phrases. Gap-audit item 10: "the legacy harness's SOUL.md is active
...
- **Wired into `composeFirstTurnPrompt`** (`build-live-agent-turn.ts`): the
  ...
  turn on that session (the warm-REPL analogue of the legacy harness re-reading SOUL.md each
  turn).
...
4. SHOULD: owner-derived / general, not owner-specific. NOW: owner-agnostic
...
- **BLOCKER 2 — false-green fixture.** The new test fabricated an
  ...
  (matched only on the token "Acme"), so it stayed green while prod stayed
  broken.
...
- **`parseGapFillFreeformList` (NEW, CONSERVATIVE).** Splits a gap-fill list
  ...
  tidy comma list AND the proper-noun-rich shape the owner actually typed both recover
...
Alpha and Acme. Side project Neutron (open source agent harness), side
project Northwind, and meditation."* — yet the flow re-asked at the end with
...
owner's project folders. the legacy harness agents hit QMD across ~1700 docs BEFORE asking
...
1. **Double-spawn.** `spawnSubagent` minted a fresh `run_id` on every call, so
   ...
   the the legacy harness incident class (registry-only pid never killed → two processes on
...
as the agents' system prompts still carried the legacy the legacy harness/Nova SELF-DELIVERY
...
only in SQLite + the web app — there was **no markdown surface**. The owner's
workflow is markdown-first (modelled on the legacy harness's `tasks.md` /
...
`~/repos/neutron-managed/docs/research/legacy-neutron-daily-driver-gap-audit-2026-06-20.md`):
Drive/Sheets/Docs were MISSING entirely, and the Email Core was draft-only
(no send). Both are daily owner workflows. Scope: `cores/` + the tool layer only
...
process), SECURITY.md (private vuln reporting via GitHub advisories / owner@example.com,
...
## 2026-06-19 — Trident-port PR-5 (FINAL): remaining the legacy harness fixes + restart-resume + `/code` → foundational Trident
...
battle-tested the legacy harness fixes with explicit regression tests, hardens
...
**1. Remaining the legacy harness fixes — mapped + explicitly tested.** NEW
`trident/legacy-fixes.test.ts` (23 tests) is the one-file map from each the legacy harness
...
- **Spawn validation / no phantom in-flight** — `TridentSessionManager.spawn`
  ...
  of the legacy harness's poll-up-to-60s spawn confirm); a blank minted id throws. So a
...
control-plane restart — the Open analog of the legacy harness's "is the tmux window / PID
...
fail (was 121; +40 new across legacy-fixes, restart-resume, code-command).
...
### Decisions Log (PR-5)
- **Resume = bounded re-dispatch, not just reap.** the legacy harness reaped a dead agent
...
of the legacy harness's `gateway/overnight-dispatcher.ts`, with the owner-locked correction
that each item is a Trident run, NOT a single throwaway substrate turn.

**Why:** the owner rejected the earlier fake-copy fix — the overnight work must
...
- **NEW `onboarding/overnight/status-md-sync.ts`** — the agent-maintained
  ...
  `[context:<path>]` HARD GATE (the legacy harness grammar, re-pointed to the per-project
...
loop into those phases — ported from the legacy harness's `~/.claude/skills/trident/SKILL.md`
...
of Trident, ported from the legacy harness's `/trident` SKILL.md onto Open's substrate
(no tmux, no `spawn-agent.sh`, no ScheduleWakeup — one in-process tick
loop drives every run).

- **NEW `trident/prompts.ts`** — trident-OWNED port of the legacy harness's
...
- **NEW `trident/session.ts`** — `TridentSessionManager` bridges the
  ...
  `subagent_run_id`. Ported the legacy harness fixes: **no phantom-id poll** (the
...
- **NEW `trident/merge.ts`** — fills the PR-2 `cleanupAfterMerge` seam.
  ...
  then delete the local branch. owner-locked: **NO `git worktree remove`**
...
Second of ~5 sequential PRs porting the legacy harness's full Trident into Neutron Open
...
- **NEW migration `migrations/0077_code_trident_runs.sql`** — the SQLite
  translation of the legacy harness's `/trident` skill state file
  (`~/legacy/gateway/trident-<slug>.state.json`). One row per pipeline.
...
First of ~5 sequential PRs porting the legacy harness's full Trident into Neutron Open
...
### BUG 1 — signup double-asked the name (wouldn't advance on a bare "Owner")
...
populated `PACK_SIGNUP.advance_examples` with bare-name exemplars ("Owner",
...
- **NEW** `onboarding/interview/__tests__/signup-router-prod-path.test.ts` —
  ...
  `advance("Owner")` through the REAL router classification (not a stubbed
...
- **ND2 — history import never runs in Path-1 onboarding.** Upload accepted +
  ...
  orphaned, no synthesis. Reproduced with the owner's real 14MB Claude export.
...
substrate, the owner's live install untouched. Report: `docs/research/fullpipe-e2e-2026-06-28.md`.
...
plain chat. (the owner's live stall = 1/5 required fields ever collected, not a finalize bug.)

...
hunters across the CORE M1 happy paths the owner will actually exercise in the
...
All confirmed still present; none re-filed (awaiting the owner's design decisions):
...
## Adjacent observations — documented, NO PR (for the owner's triage)
...
loop can close pending the owner's morning pass + the triaged design calls.

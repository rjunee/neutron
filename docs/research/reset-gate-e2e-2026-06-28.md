...
**Gate:** Before the orchestrator fully resets the owner's machine (uninstall →
...
`CLAUDE_CODE_OAUTH_TOKEN`), gbrain reachable. The owner's live install (`~/neutron/*`,
...
The real export is `/home/owner/Downloads/Claude Data Batch (1).zip` → unzips to a
...
- **(c) PARTIAL:** synthesis wrote 4 real project repos to disk —
  `Projects/{delta-playbook, epsilon-house,
  example-ventures-studio, theta}/history.md` — BUT the `projects` DB table
  held only the 3 interview projects (`beta, acme, alpha`). The import
  projects are unregistered → invisible to the Documents tab.
- **(d) FAIL:** `gbrain list` → only the 3 interview projects + 2 concepts;
  `gbrain search "delta playbook"` → **No results**. `entities/` has no
...
3 interview (`beta, acme, alpha`) **+ 4 imported**
(`gamma-info-playbook, epsilon-brand, example, theta`); all 7
...
`gbrain search "delta playbook"` → **[1.0911] gamma-info-playbook**
("Deep-research knowledge base on e-commerce conversion…"). (Run 1: the same search → **No
...
**Recommendation for the orchestrator:** do NOT reset the owner's machine onto bare
...
- `onboarding_state.phase=completed`, `completed_at` set; `projects` table = **10
  rows** (3 interview + 7 imported: `delta-initiative, owner,
  epsilon-object-brand, zeta-learning, example,
  example-ventures, theta`); import job `completed`.

...
Two Documents-surface improvements from the owner's live test.
...
`trident/ralph.test.ts`; updated `trident/{store,legacy-fixes,substrate-dispatch,
...
and the user was left to notice and fix it. Per the owner's lock ("shouldn't be a
...
hits outside the package + tests (`docs/research/legacy-neutron-feature-parity-scan-2026-06-25.md`
...
(`docs/research/legacy-neutron-feature-parity-scan-2026-06-25.md` §5 #2, lines
...
closing parity gap #4 (`docs/research/legacy-neutron-feature-parity-scan-2026-06-25.md`
...
infinite-restart incident the watchdog was built to prevent). the legacy harness is the *same*
...
that was the gap. the legacy harness (`~/legacy/gateway/session-size-watchdog.ts`) likewise only
...
- **Tests.** `session-size-watchdog.test.ts` +6 (idle critical → actuate once;
  ...
  **pre-existing, unrelated** failure (`dev-channel — exit on MCP transport close
  #217`, fails identically on unmodified `origin/main` in this checkout — see
  STATUS.md).
...
wiring, closing the legacy harness master-table **port row #16** (the last MISSING watchdog).
...
(the legacy harness 2026-04-16: Opus 4.7 shipped overnight, the gateway sat on 4.6 for hours).
...
- **Adopt + graceful upgrade.** On `notify`: `setBestModelOverride(newModel)` in
  ...
  **only when idle** — `isSessionIdleForUpgrade` requires all four the legacy harness signals
...
**What shipped.** A verify-first pass on the legacy harness master-table **port row #9**
(stuck-typing reaper + pane-scrape recovery). the legacy harness scraped the byte-static tmux
...
`onRateLimitBanner` DI seam on `PersistentReplSubstrateOptions`. Ports the legacy harness's
...
- **Edge-latch invariant.** The framework's per-detector edge-latch IS the the legacy harness
...
- **Guards.** doc-quote (framework `stripDocQuotes`), bottom-30
  ...
  ADAPTED-AT-BOUNDARY vs the legacy harness's tmux capture).
...
the legacy harness's `findLatestSessionForTopic`). Closes master-table **row #7** (the legacy harness
...
master-table **row #11** (the legacy harness `session-error-watcher.ts` +
...
and the user sees nothing — the turn dies silently (the owner 2026-06-16). The
...
before the session grows large enough to block `claude --resume`. Port of the legacy harness's
...
refused and the session falls into an infinite restart loop (the legacy harness 2026-04-16:
...
`/rate-limit-options` org-monthly-cap picker. Port of the legacy harness's
`pane-scan-watchdog.ts decideRateLimitOptionsAction` (research row #4). The owner
...
- **Positional bottom-30 guard is LOAD-BEARING.** Pressing `3` STOPS CC, so NO
  ...
  `3`+Enter into dead input for days (the legacy harness PR #132 r1). Once CC has stopped,
...
- **Fire-once / no double-press.** `debounceMs: 60_000` floor (the legacy harness
...
- **Viewport pre-check lesson — architecturally obviated.** the legacy harness's Argus PR #132
...
- **Codex cross-model review (P2, deliberate tradeoff — NOT mitigated).** Codex
  ...
  production-hardened the legacy harness reference (`decideRateLimitOptionsAction`) made the
  same call: 2 cues + doc-quote + bottom-N, no option-number cue. Honoring the
  spec + the legacy harness precedent; tracked as a known residual if a live spawn ever
...
Port of the legacy harness's `gateway-core.ts isCompactResumePicker` (research row #3).
...
> that name"** (`channel-unwired-detector.ts`, ported verbatim from the legacy harness's
...
**What shipped.** The two remaining pieces of the legacy harness master-table row #20
...
"alpha" incident lesson: **disk JSONL is the source of truth; never rely on a
...
permission prompt. Port of the legacy harness's `gateway-core.ts isToolUsePrompt` +
...
- **Known limitation (Codex cross-model review, tracked).** The F1 ring is an
  ...
  wedge-recovery detector (#1) is the designed backstop for a genuinely-stuck
  prompt.
## Stuck-turn watchdog keys off JSONL turn-progress, not `last_event_at` (the legacy harness P1 port)
...
lesson from the legacy harness `stuck-turn-watchdog.ts` (incident 2026-04-21): a CC turn
...
- **The reader (`turn-progress.ts`, new).** Pure + injectable, mirroring the legacy harness:
...
the legacy harness's `cc-update-doctor`. Closes the follow-on gap from PR #51: `ensure_gbrain`
...
2026-06-25 the legacy harness→Neutron parity audit.

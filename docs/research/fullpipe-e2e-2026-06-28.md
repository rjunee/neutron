...
The owner's live install (`~/neutron/core|data`, PID 51810 on :7800) was **never touched**.
...
**Re: the owner's live stall** (`phase=work_interview_gap_fill`, `completed_at=NULL`): his live
`phase_state_json` contained ONLY `user_first_name="Owner"` (1 of 5 required fields); he
...
- **DOCUMENTS:** 10 real project repos on disk (193 files) — e.g. `Projects/beta/docs/history.md`
  is a genuine synthesized summary ("Agentic AI operating system for a regulated vertical,
  preparing an investor pitch + fundraising") with Overview / Open threads / Source conversations,
  plus real verbatim `research/transcripts/*.md` slices from the owner's actual chats.
- **PROJECTS DB TABLE:** 9 rows (`acme, beta, alpha, example-ventures,
  example-ventures-studio, gamma-ecommerce-playbook, delta-business,
  epsilon-objects-brand, theta`) → the Documents tab surfaces them.
- **MEMORY:** 12 entity files (`entities/projects/*` + `entities/concepts/{beta,acme,
  alpha}`) and 9 pages in `gbrain list`. The import populated durable memory.
...
`signup_via='web'` (what the real button-driven flow sets — the owner's live install HAS it)
...
1. **Does onboarding COMPLETE cleanly on the fixed build?** **YES.** Fresh onboarding driven
   ...
   plain chat (tab bar + normal reply). The owner's live stall was 1/5 fields ever collected (he

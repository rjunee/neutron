...
> The owner does. **Verify-before-assert**: nothing is marked WORKS without
...
- What is **dead**: the entire **gbrain** memory layer — scribe→gbrain write,
  ...
  **Root cause (verify-before-assert, fully reproduced):**
  ...
  5. **Affects the LIVE install too:** `GBRAIN_HOME=~/neutron/data/gbrain
     gbrain list` → "No brain configured." The owner's own gbrain memory layer is
...
`docs/overnight/seed-context.md`. **Pending (the owner's steer):** drive his REAL
...
The owner's steer. Fresh instance (:7812, fresh home). Real Chromium: import

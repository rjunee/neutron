...
**Robustness is co-equal with leanness (the owner's mandate): no test deletion without sign-off, and never trade coverage for a leaner number.**
...
- **Any deletion or merge requires the owner's sign-off and must preserve the asserted behavior elsewhere.** Default verdict on ambiguous "redundant" cases = KEEP (edge-case coverage > leaner count). Output a candidate list with justification, not a PR that deletes.
...
**Explicitly out of scope unless the owner asks:** mass test deletion, faking PGLite, shared-DB fixtures that weaken isolation, teardown "cleanup" (already clean).

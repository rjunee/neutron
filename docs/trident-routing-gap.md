...
**Change:** bind per-call authority to the worker/orchestrator role at the receiving boundary. Check the live grant when executing `/tool-call`, `/todo-sync`, direct ask/reply operations and app notices; require a child identity for native subagents instead of accepting parent authority. Worker output becomes a bounded result/need addressed to the project conversation. That conversation remains able to ask. Keep deterministic status/progress and durable proposal/result storage, but route decisions and worker-authored questions through the existing project acting-turn queue.

**Delete:** terminal delivery's automatic sending of `input_needed` asks, the resolver-question-to-owner delivery shortcut, worker-triggered skill offer delivery bypassing the project decision, and session-wide authorization of worker calls to conversational tools. Delete the assumption that MCP discovery omission is authorization. Keep `interpretFailure` and `composeTerminalDelivery` as the sole evidence/advice formatters, consumed as data by the project decision turn; their existing distinctions remain pinned by `trident/escalation-block.test.ts:191`. Delete the unconditional owner send, not those pure distinctions. The REPL must make the delivery decision. Keep the old build loop executing until step 3; no second build executor is added. No emit/checkpoint/resume question protocol is introduced.

**Files to change:** `trident/delivery.ts`, `trident/delivery.test.ts`, `trident/terminal-observer.ts`, `gateway/proactive/terminal-build-wake.ts`, `open/composer.ts`, `open/wiring/substrates.ts`, `gateway/wiring/substrate-profiles.ts`, `gateway/wiring/build-llm-call-substrate.ts`, `docs/SYSTEM-OVERVIEW.md`, `runtime/adapters/claude-code/persistent/pool-state.ts`, `runtime/adapters/claude-code/persistent/repl-session.ts`, `runtime/adapters/claude-code/persistent/spawn.ts`, `runtime/adapters/claude-code/persistent/boot-adoption.ts`, `runtime/adapters/claude-code/persistent/types.ts`, `runtime/adapters/claude-code/persistent/tools-bridge-impl.ts`, `mcp/server.ts`, `gateway/http/system-notice-surface.ts`, `agent-dispatch/service.ts`, `agent-dispatch/substrate-turn.ts`, `skill-forge/proposal-message.ts`, `trident/conflict-resolver.ts`, `trident/arbiter.ts`, `trident/leak-fixer.ts`, `trident/store.ts`, `trident/codex-build.sh`, `trident/codex-review.sh`, `gateway/composition/build-core-modules.ts`, `gateway/composition/input/misc-input.ts`. Proposed new state/authority files: `trident/worker-result-inbox.ts`, `runtime/worker-authority.ts`, and one numbered `worker_result_inbox.sql` migration (reserve the `migrations/` allocation territory to this step; assign its number at the implementation base). Proposed proof files: `trident/worker-owner-boundary.test.ts` and `runtime/adapters/claude-code/persistent/__tests__/worker-authority.test.ts`. Update `docs/trident-gates-inventory.md` with proved boundary ownership. Run the existing escalation/board tests unmodified, particularly `trident/escalation-block.test.ts`.

**Gate risk:** G010–G012 and G068–G078 (escalation semantics), G090–G096 and G128–G131 (resolver/board boundaries), G132/G133/G136–G138/G159 (prompt/tool authority), G124 (failure propagation). G075/G094/G132/G133 are in the inventory's unpinned set; step 1 must resolve them first. Sink-role isolation and durable delivery are outside that inventory's three-file scope (`docs/trident-gates-inventory.md:294`), so explicitly add and mutation-certify their new guards instead of claiming the 165 rows cover them.

**Proof:** attempt every §4 output family from a worker, including native ask, returned question, error text, arbiter `OWNER_ONLY`, raw sink HTTP, todo sync, registered tool execution, app system notice, shell-mediated access and native-child use of a parent's credential. Show refusal or a result delivered only to the project REPL; show the same valid request from the project REPL can ask in the right chat. Separately assert passive progress and terminal status survive. With the REPL unavailable, persist the need for later admission; do not substitute an automatic owner ask or drop it after an at-most-once claim. The existing wake claims before composing and catches failure after claim (`gateway/proactive/terminal-build-wake.ts:105`, `gateway/proactive/terminal-build-wake.ts:124`), so durable pending/completed delivery is a required replacement here.

**Continuous maintainer:** host-side credential/role checks and a durable result inbox owned by the gateway, checked on every request and after re-adoption. Their correctness cannot depend on the worker respecting a prompt or on the failed REPL remaining alive. Prove that native children cannot use the parent's owner grant. If the selected harness cannot supply/enforce that distinction, this step is not complete and step 3 cannot be enabled; hiding an ask tool is not an acceptable substitute. This is an implementation feasibility finding to resolve before cutover, not permission for a dual path.

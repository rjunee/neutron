...
| Work / harvest | Workflow agents plan, build, review, re-plan and clean up (`pre-#845 trident/inner-workflow.mjs:8118`, `pre-#845 trident/inner-workflow.mjs:2331`, `pre-#845 trident/inner-workflow.mjs:7193`, `pre-#845 trident/inner-workflow.mjs:8793`, `pre-#845 trident/inner-workflow.mjs:9304`). Terminal JSON is written via another agent invoking the checkpoint script (`pre-#845 trident/inner-workflow.mjs:2962`); the outer `applyResult` begins at `trident/orchestrator.ts:4954`. |
...
| --- | --- | --- |
| Workflow role routing | `routeModel(label, tag)` starts from build complexity or `ROLE_MODEL`; phase overrides can change family and transport (`pre-#845 trident/inner-workflow.mjs:767`, `pre-#845 trident/inner-workflow.mjs:571`, `pre-#845 trident/inner-workflow.mjs:688`). Unsupported family/transport changes are logged and ignored (`pre-#845 trident/inner-workflow.mjs:730`). | Role/phase chooses the work's requested tier. That is useful policy to preserve, but the executor decision must compare the resolved provider with the **current project's REPL provider**. The present function accepts label/tag and resolves against the launcher-oriented table; E2 supplies the bounded negative control. |
| Build executor | `forgeAgent` runs native `agent()` if transport is not `cli`; otherwise it uses a Claude bridge agent that invokes the Codex wrapper and transcribes its result (`pre-#845 trident/inner-workflow.mjs:2324`, `pre-#845 trident/inner-workflow.mjs:2338`). `withModel` deliberately leaves CLI bridge agents on their default model (`pre-#845 trident/inner-workflow.mjs:818`). | Already a model-derived transport split, but relative to the dedicated Claude launcher. A Codex project conversation does not make this build a native Codex subagent; the fire composition remains the same (`open/composer.ts:1106`). |
| Review executors | Adversarial review branches on `transport`; peer review branches on `route.group`, enabled seats and credential availability (`pre-#845 trident/inner-workflow.mjs:7210`, `pre-#845 trident/inner-workflow.mjs:7231`, `pre-#845 trident/inner-workflow.mjs:7235`). A peer moved to Claude uses a real Claude reviewer; CLI peers run through bridge agents (`pre-#845 trident/inner-workflow.mjs:7241`, `pre-#845 trident/inner-workflow.mjs:7260`). | Keep review role, independence and readiness policy; delete bridge-agent shell transcription. Kimi's wrapper ultimately uses a text API request (`trident/kimi-review.ts:207`, `trident/kimi-review.ts:225`), a concrete disagreement with a blanket claim that every other-provider seat is already a headless harness. |
...
| --- | --- | --- |
| Structured review escalation | Reviewer `escalate` claims and deterministic convergence decisions become terminal `blockKind` plus `escalation` (`pre-#845 trident/inner-workflow.mjs:3681`, `pre-#845 trident/inner-workflow.mjs:3776`, `pre-#845 trident/inner-workflow.mjs:9177`). Terminal writer → parser → outer harvest → deriver (`pre-#845 trident/inner-workflow.mjs:2962`, `trident/inner-loop.ts:915`, `trident/orchestrator.ts:4954`, `trident/escalation-block.ts:94`). | The deriver requires failed phase, harvested timestamp and coherent kind/payload. Delivery includes missing-work summary and decision advice, then sends it to the originating channel (`trident/delivery.ts:755`, `trident/delivery.ts:1274`, `trident/delivery.ts:1353`). This is a worker-triggered owner-directed ask before the project REPL decides whether to ask. |
| Ordinary result / failure / diagnostics | Terminal cause, findings and error text flow through the same result and harvest; thrown workflow errors also write failure results (`pre-#845 trident/inner-workflow.mjs:9218`). | All terminal delivery classes must be reviewed, not just `escalated`: `composeTerminalDelivery` includes `input_needed` on infrastructure, published-unreviewed and ordinary failures (`trident/delivery.ts:1262`, `trident/delivery.ts:1279`, `trident/delivery.ts:1282`). Unknown failures can expose short worker-influenced prose (`trident/delivery.ts:1185`). A new spelling falling through does not become silent. |
...
| Review result | The terminal writer records `REVIEW_NOT_RUN` unless an actual review satisfies its verdict/findings condition (`pre-#845 trident/inner-workflow.mjs:2906`). Preserve server-side review provenance, not just worker strings. |
...
**Production change/deletion:** none. Add behavioral pins around current boundaries; remove no gate, loop or passing test. The inventory identifies **165 entries, 11 without a located pin, 155 silent and 10 loud if lost** (`docs/trident-gates-inventory.md:3`). Its complete unpinned set is G001, G018, G083, G085, G089, G094, G097, G111, G132, G133, G140 (`docs/trident-gates-inventory.md:41`). G021, G022, G023, G024, G031, G034 and G075 left the unpinned set when behavioral pins were added; G135 moved from Silent to Loud when independent hook enforcement was added, which is why the direct row count is now 155/10. Those 11 must earn pins before their owning file is replaced; a source-substring assertion alone cannot establish survival through a rewrite.
...
**Change:** keep the build loop executing. Terminal failure *asks* — `input_needed`, resolver questions, arbiter owner-only outcomes — become evidence for the project conversation. Try the arbiter before the project decision; only that conversation decides whether to ask the owner. Keep `interpretFailure` and `composeTerminalDelivery` as the sole evidence/advice formatters; `composeTerminalDelivery(run, { include_advice: false })` is how the announce sheds the ask. The deterministic announce KEEPS #352's interpreted failure summary — the decision turn's reply is free-form model text, so degrading the announce to a stub would leave the owner with no reliable account of the failure at all. Passive terminal status remains deliverable. Skill proposals keep the button-backed owner delivery (#820); the decision turn may additionally raise one through `skill_forge_list`.

**Acceptance:**
- Worker failure *advice* and resolver questions do not reach deterministic owner delivery; the project conversation receives the formatted evidence and can still ask in the originating chat. The interpreted failure *summary* still reaches the owner deterministically (#352), so "the ask moved" is never satisfied by "the message went silent".

...
For `task_sequence`, `N` is the one-based `task_iteration + 1` and `R` is
...
never `max_task_iterations`, which limits spending and says nothing about plan size.
...
card seed preserve a known total. A `single` run reads only `Round R`;
...
the known review round instead of guessing a strategy. Before the initial
planner's selection is persisted, the card reads `Planning pending`.

Historical storage maps `ralph = 1` to `task_sequence`, `ralph = 0` to
`single`, `ralph_round` to `task_iteration`, `max_ralph_rounds` to
`max_task_iterations`, and `ralph_task_total` to `task_total`. Migration and
test names retain the old tokens as provenance.
...
- [x] A `task_sequence` task with iteration 9, total 15 and review round 1 renders
      `Task 10/15 · Round 1` on phone and web; missing total renders `?`.
      verify: mobile row/helper and web Work Board render tests.
- [x] `single` runs show `Round R`; terminal rows show no counter. Malformed

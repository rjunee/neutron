...
Branch `feat/auth-failure-reauth-prompt` (new PR vs main). Real dogfood failure (2026-07-24): a chat turn's underlying `claude` child hit `API Error: 401 OAuth access token is invalid` from Anthropic and (headless) printed `Please run /login` then produced ZERO further PTY output. The activity-based inactivity watchdog correctly detected the silence as frozen but MISCLASSIFIED it as a generic timeout, so the owner saw the useless "That one took too long… tap Retry" bubble instead of the actionable "reconnect your Claude token". Ryan: "fix this systematically… popup an auth window in the UX."

- **NEW `runtime/adapters/claude-code/persistent/auth-failure-signature.ts`** — an output-scan signature (sibling of `rate-limit-banner.ts`), wired into the existing `OutputScanner` framework, NOT a from-scratch mechanism. Matches a small credential-shaped cue set whitespace/case-insensitively (survives the Ink TUI's per-word cursor shredding + a normally-streamed line): `OAuth access token is invalid`, `Please run /login`, `invalid x-api-key`, and an `API Error` + `401`/`403` pairing. NOTIFY-ONLY (no `keys` — there is nothing to press headlessly). Honest best-effort limitation documented in-file: a text-scrape of a human-facing CLI error, not a structured API, so a reworded banner can miss (degrades to the prior generic timeout, no worse) — deliberately not overfit to the one observed line.
...
- **`pool.ts` turn driver** — the auth-failure flag is CLEARED at each turn's start (before the inject, since the banner arrives during it) and, in the per-turn timeout watchdog, its mere presence FAST-FAILS the turn with a distinct `{ code: 'auth_invalid', retryable: false, message: 'persistent-repl: auth token invalid — reconnect required' }` (poisoning the warm session like the freeze path) INSTEAD of waiting out the inactivity window and emitting `turn_timeout`. New `auth_invalid` value in `SubstrateErrorClass` (`runtime/events.ts`) + `SUBSTRATE_ERROR_CODES` (`runtime/errors.ts`).
...
- **Tests:** NEW `auth-failure-signature.test.ts` (drives the REAL OutputScanner: real 401 line + each variant fire; Ink-shredding survival; unrelated errors / a bare `401` / a 500 do NOT fire; edge-latch no-re-fire; doc-quote + bottom-N guards). NEW `auth-failure-classification.test.ts` (REAL persistent-REPL substrate + fake PTY host feeding the observed line then going silent → turn errors with `code:auth_invalid` retryable:false, NOT `turn_timeout`). NEW `build-live-agent-turn-auth-reconnect.test.ts` (auth-invalid failure ships `AUTH_RECONNECT_BODY` not `TIMEOUT_BODY`/`FAILURE_BODY`, no auto-retry, no Retry button, persisted; classifier distinctness; seed turn stays silent).
- **Gates:** `bunx tsc --noEmit` clean; affected suites green (`runtime/adapters/claude-code/persistent` + the live-agent/llm-call wiring — 672 pass); `bash scripts/ci/depcruise.sh` 0 new violations. NO FEATURE FLAGS.

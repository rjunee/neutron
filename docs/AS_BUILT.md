...
The persistent REPL crash watchdog now awaits a substrate-owned durable-work callback whenever its existing `pid-dead` detector fires, including capped and recovery-in-progress children that are not respawned. The Open Trident fire substrate (one pooled launcher per repository) uses one conditional store update to mark that launcher's live `code_trident_runs` rows `crashed`; a stale tick snapshot cannot overwrite that marker, while the next `trident/tick.ts` sweep may persist the normal terminal failure and invoke the existing Work Board reconciler. This removes the phantom phase-based running count and pulsing board indicator after child death without adding another reaper, imposing a scan cap, or relying on loop-registry presence.

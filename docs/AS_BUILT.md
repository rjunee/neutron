...
- **`/clear` file-rotation empirical check:** attempted but left **unverified — the design is behavior-independent.** The trigger is a per-`ReplSession`-object BASELINE DELTA (`WeakMap`), so whether CC keeps appending the same pinned-session JSONL after `/clear` or rotates to a new file, an absolute-size trigger would re-fire forever on the stale pre-clear bytes while the delta fires only on growth SINCE the last reset — and a respawned session (a new `ReplSession`) restarts at baseline 0 naturally. Correctness does not depend on which behavior CC has.

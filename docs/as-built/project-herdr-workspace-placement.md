...
A definite initial worker refusal cleans up an allocation only when the live
workspace contains solely its verified placeholder; an unrelated pane prevents
closure. Failed post-spawn setup closes the unreturned child where possible and
...
empty-allocation, lock, repair, and revision guards, plus absent and over-applied
environment clearing. The original code was restored after each mutation.
...
inherited-environment clearing with a positive control, and cleanup of an empty
failed allocation versus preservation when an unrelated pane arrives. The first

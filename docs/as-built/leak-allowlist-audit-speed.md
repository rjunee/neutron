...
The same named test at `scripts/ci/leak-gate-selftest.test.ts:1530` now calls that
shared production preparation directly against the real tree and committed
allowlist. It requires successful execution and empty stdout; audit failures
exit nonzero and make the child invocation throw. Redundant stdout checks for
diagnostics emitted on stderr were removed. All fixture gates copy the helper
alongside the gate.

Review hardening at `scripts/ci/leak-gate.sh:215` explicitly exits 2 when the
preparation function fails, including a loadable helper that omits the function.
The paired fixtures at `scripts/ci/leak-gate-selftest.test.ts:1454` exercise an
absent function and a function returning 23; both must exit 2 before the scan
banner and never report SILENT. The valid exact-path fixture still passes.
...
With dependencies installed, the final consuming-suite run reported 905.01 ms.
...
- `bun test scripts/ci/leak-gate-selftest.test.ts scripts/ci/leak-gate-explain.test.ts scripts/ci/leak-gate-nul-tripwire.test.ts scripts/ci/ci-workflow.test.ts`:
  162 pass, zero fail, 424 assertions, 53.54 seconds. An earlier run had one
...
- Malformed entries, directory globs, excessive breadth and stale paths are
  ...
  `scripts/ci/leak-gate-selftest.test.ts:1548` seeds both an allowed finding and
...
- Semantic mutations: replacing the audit rejection condition with `false`
  ...
  The review regression fixtures both failed against the original unchecked
  invocation (exit 1 instead of 2). Mutating the hardened invocation to always
  exit 2 also failed the legitimate exact-path sibling. The final conditional
  refusal was restored before validation.

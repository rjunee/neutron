...
A current revocation probe belongs to its access-token fingerprint, so a result
...
Local verification passed 262 consuming build integration tests, 175 credential,
rotation and probe tests, 74 native-owner tests plus the focused grant-removal
approval/decline test, 13 shared-client/web tests, and the rendered phone route test.

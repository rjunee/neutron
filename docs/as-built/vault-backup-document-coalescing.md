...
(`gateway/git/project-backup-store.ts:304`). Backup callers wait for document
...
release only their own writer entry (`gateway/git/project-backup-store.ts:536`).
...
queues restores behind a document commit and checks recovery parents, restored
bytes and preservation of the latest edit. These three controls passed with

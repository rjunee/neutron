...
the worker (`trident/build-run.ts:480`). Resume validates that request, execution
inputs, round and snapshot (`trident/build-run.ts:332`), then requires the runner's
explicit recovery-only capability (`trident/build-run.ts:467`). It neither
...
panel (`trident/build-run.ts:488`). Publication nomination repair cannot discard
an unresolved pending identity (`trident/build-run.ts:688`). Writable planner,
...
Verification: `bun test trident/build-run.test.ts` passes 244 tests, including
...
its regression. Restoring the implementation restores all 244 passing tests.

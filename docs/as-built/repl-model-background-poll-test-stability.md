...
The focused 18-test file passed, and two concurrent reruns of the race case passed. A temporary reverse mutation removing the component's generation check made that case fail with `Model: cheap` instead of `frontier`; the guard was restored. Root and app TypeScript checks passed. This validates the test synchronization locally; it does not constitute a shared-host or served-product receipt.

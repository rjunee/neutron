...
**Status:** PLAN-ONLY. No code written. Four architecture deltas (below) need the owner's sign-off BEFORE any build.
...
## 1. Architecture deltas requiring the owner's sign-off BEFORE build
...
> If the owner disagrees with any delta, the affected phase changes shape — do not start Phase 2 until 1/2/4 are locked (Phase 1 is delta-independent and can start immediately).
...
- **SPEC.md** — `SPEC.md:186` moves `[ ]`→`[x]` only when the full acceptance is met; append a Decisions Log entry recording the four deltas as locked once the owner signs off.
...
## 4. Open questions for the owner (blockers before Phase 2)

...
>
> **Residual (owner-accepted):** an untracked hand-set env on the live Managed VPS is the only thing not covered by the repo grep. To neutralize even that, the deletion is NOT silent: `resolveDeploymentMode` keeps a **tombstone** (`RETIRED_DEPLOYMENT_MODE_ALIAS`) that emits a LOUD `console.error` (migrate to `NEUTRON_ROLE`) if a box still sets the retired alias — so a stray managed unit screams instead of silently degrading to `open` (which gates managed-only credential isolation in `resolve-llm-credentials.ts`). Fixing Codex r1's valid severity catch.

**Done in the K11b2 PR:** `gateway/deployment-mode.ts` — removed the `DEPLOYMENT_MODE_ENV` const + the `fromAlias` resolver branch, kept `NEUTRON_ROLE` canonical, added the loud tombstone guard. `deployment-mode.test.ts` — alias rows converted to `NEUTRON_ROLE`; new tests pin the alias-inert contract (`{NEUTRON_DEPLOYMENT_MODE:'managed'} → open`, mixed-invalid → open, the migration warn fires / no spurious warn). `resolve-llm-credentials.test.ts` — the managed-mode shared-env-key **security pin** kept on `NEUTRON_ROLE` (the redundant duplicate removed). `build-landing-stack.ts` — stale alias comment dropped. **Accept met:** tsc + matrix + both test files green; `git grep NEUTRON_DEPLOYMENT_MODE` in prod `.ts` returns only the intentional tombstone.

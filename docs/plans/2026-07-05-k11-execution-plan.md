...
## 8. K11b2 — NEUTRON_DEPLOYMENT_MODE alias: MIGRATE, not delete

> ⏸️ **DEFERRED — OWNER/OPS-GATED (2026-07-07; investigation extended + a prior claim CORRECTED).** In-source verification (done by the orchestrator, not punted): `NEUTRON_DEPLOYMENT_MODE` / `NEUTRON_ROLE` are **set by nothing in current source** — not the Open per-tenant provisioner (`neutron-managed/src/provision/systemd.ts` `renderSystemdUnit`/`buildTenantEnv` write `NEUTRON_HOME`/`NEUTRON_PORT`/`NEUTRON_INSTANCE_SLUG`/… but neither mode key), not anywhere else in `neutron-managed/src/` (whole-repo grep clean; all hits are in docs/archive + the vendored worktree), and not the live dogfood instance (`~/neutron/core/.env` + `neutron-server.plist` set neither). **⚠️ CORRECTION of the earlier note:** managed-mode is NOT "derived from the identity signing key." `gateway/deployment-mode.ts` resolves mode **purely from env** — `NEUTRON_ROLE` > `NEUTRON_DEPLOYMENT_MODE` (alias) > default `'open'`. The identity signing key only (a) triggers a boot **warning** if mode resolves to `'open'` while a key is present, and (b) forces `'managed'` for the narrow shared-projects-resolver. So the alias **IS load-bearing** for any box whose unit sets it. The alias's documented purpose (deployment-mode.ts header) is explicitly back-compat for "already-rendered systemd units that set `NEUTRON_DEPLOYMENT_MODE=managed`." **Residual risk:** the live **Managed control-plane** gateway's systemd unit is on-disk infra on a remote VPS, in NO repo — if it sets the alias (not the canonical `NEUTRON_ROLE`), removing the Open alias read flips it to `'open'` on restart (managed-outage). **This is a 2-step OPS migration, not a code change:** (1) ensure the live managed unit sets `NEUTRON_ROLE=managed`, deploy; (2) THEN remove the alias read. Step 1 is against production infra the orchestrator can't safely touch headlessly. **Unblock:** Ryan (or a fleet SSH/access pointer) confirms the live managed unit sets `NEUTRON_ROLE` — or migrates it — then step 2 runs as specified below. Value is negligible (2-line shim); not worth an unverified managed-outage.

Independent, any time. `gateway/deployment-mode.ts`: remove the alias read (`DEPLOYMENT_MODE_ENV` :40, `fromAlias` branch :64-65, doc block :23-28) keeping `NEUTRON_ROLE` (:37) canonical. **MIGRATE the security pin**: `gateway/realmode-composer/__tests__/resolve-llm-credentials.test.ts:568-575` (managed-mode shared-env-key refusal exercised via the alias) → same assertion via `NEUTRON_ROLE` — the refusal behavior must stay pinned, only the env key changes. Update `gateway/deployment-mode.test.ts:16-67` (alias rows become "alias ignored" or are dropped; the :46 "back-compat intact" test inverts). **Owner pre-merge action:** 30-second grep of live systemd/launchd units + any Managed provisioning templates for `NEUTRON_DEPLOYMENT_MODE=` (memory: verify via pushed-branch checkout, never hand-edit `~/neutron/core`). **Accept:** tsc + both test files green; `git grep NEUTRON_DEPLOYMENT_MODE` returns only historical docs.

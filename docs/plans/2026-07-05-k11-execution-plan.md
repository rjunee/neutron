...
> ⏸️ **DEFERRED — OWNER-GATED (2026-07-07).** The mandatory owner pre-merge action was run: `NEUTRON_DEPLOYMENT_MODE` / `NEUTRON_ROLE` appear **nowhere** in current Managed source (`neutron-managed` `src/`). The current provisioner (`src/provision/systemd.ts`) writes an `EnvironmentFile=` carrying only `NEUTRON_HOME`; managed-mode is derived from the local identity signing key, not this env alias. The systemd-template refs that set `NEUTRON_DEPLOYMENT_MODE=managed` are HISTORICAL (old `tenant-provisioning/render-systemd-unit.ts` structure, superseded). So **current provisioning does not depend on the alias** and repo-evidence says removal is safe. The residual risk is purely operational: an **already-provisioned live box** provisioned by the old code path could have `NEUTRON_DEPLOYMENT_MODE=managed` baked into its on-disk systemd unit / `.env`; removing the Open alias would flip it to `open` mode on next restart (misconfigured-Managed outage). That is a **live-fleet fact not verifiable from either repo** → per the hard stop-condition ("don't break live Managed without owner decision"), the alias stays until Ryan confirms no live box sets it. Value is negligible (2-line back-compat shim); it is NOT worth an unverifiable outage risk. **Unblock condition:** Ryan confirms the live Managed fleet has no unit/`.env` setting `NEUTRON_DEPLOYMENT_MODE=` (e.g. `grep` across provisioned instance dirs), then this runs exactly as specified below.

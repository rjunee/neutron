...
**Target:** the Expo/React-Native app under `app/` (NOT `landing/chat-react` — that is web, done in PR-3). Mobile counterpart of PR-3's desktop rail/tabs. Depends on PR-1..5 (all merged). **NO FEATURE FLAGS. One code path.** Dark-token themed. owner-locked design (SPEC 2026-07-02 "M1 UX REDESIGN — SIGNED OFF").

Authoritative visual reference: the dedicated mobile prototype `https://neutron-redesign-proto.netlify.app/mobile/` (saved copy analysed). **Override:** the prototype's rail is a 56px **emoji-only** icon rail, but the owner explicitly flagged emoji-only as insufficient and asked for **emoji + project NAME below** (Telegram-folder-style). Section A below is authoritative over the prototype on that point; every other treatment (work-dot, seated tabs, Work-tab badge, tokens, spacing) follows the prototype.

Simple-enough-for-a-non-technical-user bar: the rail must read like Telegram's left folder rail — obvious.

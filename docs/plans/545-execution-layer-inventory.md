...
| `resume_checkpoint` | `trident/launch-preparation.ts:47` | truthiness selects `start` at `trident/project-launcher.ts:125` | **Consumed as a switch**. The latest validated `build-mode-state` supplies it for typed same-run recovery; legacy fields still serve the retained workflow and fresh retry seed. Detailed resume comes from `build-mode-state`, `trident/production-host-effects.ts:234,265`. Falsifier: legacy checkpoint alone authorizes recovery of an abandoned project-driver reservation |

...
**Do not delete `launch`, the unconfirmed-fire machinery, or result settlement.** The gateway installs `orchestrator.step` into `TridentTickLoop` (`gateway/composition/build-core-modules.ts:826`), the step injects `launch` into recovery (`trident/orchestrator.ts:2876`), and recovery calls it (`trident/recovery-liveness.ts:602`). `launch` calls the **new** launcher through the injected `fireWorkflow` (`trident/orchestrator.ts:1974`); the binding is `createProjectLauncher` (`open/composer.ts:1187`). Its producer still returns all three launch statuses (`trident/project-launcher.ts:117,169,172,179`). A closure's name containing “fire” does not make it legacy.
...
| Composer → injected firer | `open/composer.ts:1189` constructs the launcher; :1194 injects `prepareProjectBuild`; :7186 exposes `fire_inner_workflow`; gateway copies it at `gateway/composition/build-core-modules.ts:633` | High for this composer | A different `tridentWiring.fire_inner_workflow` provider |
...
| Bound review → injected firer | `trident/orchestrator.ts:1825` → `trident/bound-review.ts:160` → default panel (`trident/review-run.ts:244,273,499`) | High for invocation; compatibility UNKNOWN | Exercise the real composed bound-review path and show it completes against the intended store |
...
The review's `fireWorkflow(` grep is correct as a **lexical** count, not a complete runtime call-site count. Bound review forwards the same callback under `deps.fire_workflow` and invokes it at `trident/review-run.ts:273`. It manufactures a panel run and isolated database (`trident/review-run.ts:251,255,258,277`), whereas `createProjectLauncher` reserves the composition store (`trident/project-launcher.ts:120`; `open/composer.ts:1190`). The latter does not select a store from `input.db_path` (C3's complete input-access listing). This is a measured interface mismatch, not proof that bound review succeeds. A real composed bound-review fixture is the falsifier; no product decision or fix is made here.
...
| `base_sha` scalar; `run.base_sha` | fetch/pin preparation, `trident/launch-preparation.ts:260,298,724` | scalar unused (C3); **run pin mandatory**, `open/wiring/project-build.ts:208`; branch creation uses it :220 | **Not re-derived**. Removing scalar alone differs from removing pin producer. Falsifier: preparation proves/persists a fresh base without the run pin |
...
**C2 — lexical fire versus injected fire**

```sh
rg -n 'fireWorkflow\(|fire_workflow' trident/orchestrator.ts trident/bound-review.ts trident/review-run.ts
```

Direct call: `trident/orchestrator.ts:1974`; positive injected-call control: `trident/review-run.ts:273`. The `fire_workflow` forwarding property is at `trident/orchestrator.ts:1827` and `trident/bound-review.ts:150`. This instrument can see the callback mechanism the earlier outward import walk missed.

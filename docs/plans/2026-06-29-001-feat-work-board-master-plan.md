...
**Two decisions RESOLVED (Ryan 2026-06-29):** (1) board is **READ+WRITE for the human** — add/edit/reorder/delete (full CRUD is the same `WorkBoardStore` + `POST/PATCH /work-board` machinery, so build all). The "how does the agent understand a human-added one-line item?" concern needs no new mechanism: the board is injected every turn so the agent sees a human-added item next turn; a one-line item is a pointer — when the agent goes to WORK it and lacks detail it ASKS (like any underspecified ticket), or the item links to a design doc; no auto-act on add. (2) **Layer-B aggressive-reset is its OWN later phase (UPCOMING), NOT in the first board PRs** — Layer A (board on disk + per-turn injection) ships first as the reliability win; Layer B needs a substrate-support check.

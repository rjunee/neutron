...
- **main HEAD:** `0dc717c` — **FABLE-WINDOW BATCH 1 COMPLETE (9 units, 2026-07-09):** D1 #282 (pool-state extract) · C4 #283 (RouteSlot registry + diverged-gate fix) · P1 #284 (ProjectDb get/all/runSync + tx-assert) · D4 #285 (backup-store facade + keyed-mutex) · D9a #286 (engine SpecResolutionFlow carve) · RA1 #288 (writeEntity per-key lock) · S0 #287 (app-ws same-origin + per-boot token SECURITY) · D6 #289 (admin pane split) · D5 #290 (email backend split). Prior: L5 #280 → **L-PHASE COMPLETE (L1–L7)**; **WAVE 0 COMPLETE** (G1–G10 + W0); **WAVE 1 K-phase** (below).
...
- **⚡ FABLE WINDOW (2026-07-09 → ~07-11):** Ryan directive — Fable free → route build+analysis to `fable` (opus for delicate/security, haiku for trivial) + MAXIMIZE PARALLELIZATION (4-6 disjoint-file units concurrently; merge stays serial). See [[refactor-fable-window-2026-07-09]]. **BATCH 2 IN FLIGHT:** D2 (substrate banner split, opus), C6 (credential-resolver unify, opus), P3 (openSidecar, fable), D8 (landing/server cleanup, fable). NEXT: MG-3 (verify NEUTRON_GRAPH_COMPOSER_MODULE not in Managed buildTenantEnv first) + P5/W3a/D3/fillers.

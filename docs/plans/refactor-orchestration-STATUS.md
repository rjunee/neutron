...
- **⚡ FABLE WINDOW (2026-07-09 → ~07-11):** Ryan directive — Fable free → route build+analysis to `fable` (opus for delicate/security, haiku for trivial) + MAXIMIZE PARALLELIZATION (4-6 disjoint-file units concurrently; merge stays serial). See [[refactor-fable-window-2026-07-09]]. **BATCH 4 IN FLIGHT:** D9c (engine GapFill/Resume/ProjectsProposed + router core carve, opus), C5 (auth-gate seam + landing route manifest, opus, ⚠️live-auth-path), P10 (trident checkpoint hardening, fable). NEXT (queued): MG-3 (precond CHECKED 2026-07-09 — `NEUTRON_GRAPH_COMPOSER_MODULE` ∉ Managed `buildTenantEnv`; seam deletable, but boot-shell+cross-repo → handle NON-CONCURRENTLY after C5) · W3a · D3 (re-scope post-K11b0) · fillers RC1/P11(after P4)/F1(after F7✓)/F2. HOLD: RA3 (behavior+ollama), D7 (browser-smoke), O5 (feature+route-reg near C5).

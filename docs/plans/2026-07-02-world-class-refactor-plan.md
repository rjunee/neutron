...
## 0. Mission & ground rules (locked by the owner, 2026-07-02)
...
3. **Nothing is frozen — and the real Managed ABI is tiny (investigation-corrected).**
   ...
   package APIs may all change. No external users yet. The owner owns and directs the
...
6. **One path per feature, zero feature flags** (the owner, Q4): no duplicate code paths
...
model, self-driven, no the legacy harness fleet-chat dependency — build prerequisites verified 2026-07-03:
...
SYSTEM-OVERVIEW, the G10 invariants inventory, and the owner's locked decisions quoted in
...
(D-5 RESOLVED by trace — the owner: "no two onboarding flows… no duplicate code paths… no
...
— keep one, import it). **Care:** `trident/legacy-fixes.test.ts` anchors ported wedge
...
loop + store + tick + prompts actually loaded; legacy-fix parity preserved; exactly one
...
>
> ⚠️ **`runWithActiveProject` now has ZERO callers — and the app-ws slash-command path has a PRE-EXISTING active-project credential-scoping gap the credential-scoping unit MUST close.** After K11b0, `gateway/cores/active-project-context.ts:runWithActiveProject` has **no production caller anywhere in the repo** (only its own unit tests — `gateway/cores/__tests__/active-project-context.test.ts` + `core-credential-resolver.test.ts`). The dead bridge's chat-command path was its ONLY live wrap site; K11b0 removed that caller but **deliberately keeps the module** (latent, for the credential-scoping unit). This is NOT a regression K11b0 introduced — the app-ws surface **never** established the frame: `gateway/http/app-ws-surface.ts:605` and `:783` both call `chat_command_filter.match(matchInput)` DIRECTLY, unwrapped, on `main` and after K11b0 alike. So a project-topic slash command (`/cal`, `/email`, `/research`, `/remind`) run on `/ws/app/chat` resolves its Core credentials against **global scope**, never the active project's scope — a **LIVE, pre-existing gap** (K11b0 changes zero behavior here; it only deletes the dead bridge's caller). **The per-project credential-scoping plan (`docs/plans/2026-06-30-per-project-settings-tab-credential-scoping-plan.md`) MUST close it**: wrap BOTH `chat_command_filter.match` calls at `app-ws-surface.ts:605` + `:783` in `runWithActiveProject(matchInput.project_id, () => …)` and add a regression test that a project-scoped Core credential is resolved under the active project (not global). Do NOT fold that behavior change into K11b0 (pure deletion) — it belongs to the credential-scoping unit with its own tests.
>
...
**The owner's decision: finish the S5/S6 wiring, don't delete.** Both supervision systems
...
Folds in the live trident bug the owner handed off (neutron-managed
...
(D-10 resolved: **make it real** — the owner: multi-channel capability (Telegram/Slack
...
Folds in the live chat-UX jank cluster the owner handed off (neutron-managed
...
the fire-time agent to use `tg-post.sh`/`weather.sh` (the legacy harness machinery Open doesn't
...
Nova(106 files)/the legacy harness(92)/Topline(88)/Hermes(21)/LegacyAgent(11) provenance residue: keep
provenance in test anchors (legacy-fixes.test.ts is a live parity anchor), scrub from
published-contract doc comments (core-sdk/types.ts), add GLOSSARY.md.
**Accept:** contributors can decode "per the legacy harness FIX 9".

---

## 14. Phase 11 — `[BEHAVIOR]` Security hardening to the public-launch bar (S)

> All flagged behavior changes; the owner pre-approved the direction ("fail-closed gate
...
by hand on the owner's Mac. Add GitHub Actions: `bun test` (incl. `tests/open-contract.test.ts`
...
> **Goal (the owner, 2026-07-03):** make neutron's memory best-in-class — "perfect recall +
...
>
> **The owner's split:** A (hardening) + C (coordination substrate) fold into the refactor;
...
> **Portability & swappability invariants (the owner, 2026-07-03 — MUST-PRESERVE, not features to build).**
> The owner wants the refactor to keep two future doors open without building either now:
> **(I1) the brain persists across harnesses** (switch neutron ↔ LegacyAgent ↔ Hermes and the
...
> the window (the owner: "no need to add to the spec yet").
...
(Hindsight belief-evolution / memory-store `kg_invalidate`): a timeline entry can mark a prior
...
## 15. Decision queue — RESOLVED by the owner, 2026-07-02
...
> **07-03 reconciliation (live-dogfood handoff).** Three units fold in bugs the owner hit

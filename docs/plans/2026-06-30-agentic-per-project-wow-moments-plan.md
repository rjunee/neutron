...
- **This never refreshes.** It is the exact "one-liner want-to-X" surface the owner wants to replace with meatier agentic moves.
...
## 5. Sign-off items (need the owner)
...
- **S5 — Catalogue scope for v1.** Ship all four actions (`p01`–`p04`), or land `p01-draft-doc` + `p02-deadlines` first (the two the owner named verbatim) and add `p03-start-work` after? (Recommend: `p01` + `p02` + `p04-brief` in v1; `p03` in a fast-follow once the draft-doc quality bar is proven.)

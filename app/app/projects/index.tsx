 ...
 * endpoint. The list starts EMPTY and `fetchProjects()` fills it; a
 * failure shows the error banner rather than placeholder rows (#393).
...
export default function ProjectListScreen() {
  ...
  // the gateway. The list stays EMPTY until this resolves (#393) —
  // placeholder rows are never shown. Extracted to a callback so the degraded-
  ...
  const projects = useMemo(() => {
    // NEVER substitute placeholder rows for real data (ISSUES #393). This
    // previously fell back to `loadProjects()` — three hardcoded demo
    // projects ("Acme", "Northwind Labs") whose own docstring called them
    // fake — so a failed fetch rendered FABRICATED projects captioned
    // "Showing cached list". They were never cached and never real. An
    // empty list plus the existing error banner is strictly better: it is
    // honest, and it cannot be mistaken for the owner's actual data.
    const base = serverProjects ?? [];
    ...
    <View style={styles.container}>
      ...
      <View style={styles.header}>
        ...
        <View style={styles.headerActions}>
          {/* ISSUES #385 — the ONLY on-device server editor lives on
              /settings, and until this entry existed the route was
              registered (`app/app/_layout.tsx` `<Stack.Screen
              name="settings" />`) but never pushed from anywhere, so a
              persisted-bad host (e.g. after a DHCP lease change) could
              only be fixed by reinstalling. Argus r2 BLOCKER. */}
          <Pressable
            accessibilityRole="button"
            accessibilityLabel="Open settings"
            testID="projects-settings-btn"

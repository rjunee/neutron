...
describe('WebTokenStorage', () => {
  ...
  it('server config round-trips; a null field removes its key (ISSUES #385)', async () => {
    const backing = makeSyncBacking();
    const ts = new WebTokenStorage(backing);
    expect(await ts.getServerConfig()).toEqual({
      gateway_base_url: null,
      auth_base_url: null,
    });
    await ts.setServerConfig({
  ...
  it('clearAll (sign-out) does NOT forget which server this install belongs to', async () => {
    const backing = makeSyncBacking();
    const ts = new WebTokenStorage(backing);
    await ts.setToken('x');
    await ts.setUser(sampleUser);
    ...
    await ts.clearAll();
    expect(backing.snapshot()).toEqual({
      [SERVER_GATEWAY_KEY]: 'https://neutron.example.com',
    });
  });
});
...
describe('NativeTokenStorage', () => {
  ...
  it('server config round-trips through the async backing (ISSUES #385)', async () => {
    const backing = makeAsyncBacking();
    const ts = new NativeTokenStorage(backing);
    expect(await ts.getServerConfig()).toEqual({
      gateway_base_url: null,

...
export interface MobileChatSessionOptions {
  ...
  /** Injectable reconnect timer (tests). Defaults to the transport timer. */
...
export class MobileChatSession {
   ...
   * re-delivery never re-fires the agent). A no-op while the socket is down — the
  ...
  private async resumeAndFlush(): Promise<void> {
    ...
    // forward resume, and on mobile that means per foreground and per foregrounded
    // push, since `catchUp` re-drives this on an already-open socket.
    //

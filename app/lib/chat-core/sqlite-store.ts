...
export class SqliteChatStore implements Store {
  ...
  // `rehomeLegacyTopics()` REMOVED (ISSUES #399). It was added in #398 to
  // rescue history stranded by a rotated bearer, matching
  // `topic_id LIKE 'app:%' AND topic_id <> canonical`. Under per-project topics
  // that pattern matches EVERY `app:<user>:<project>` row, so it would have
  // collapsed every project's history into General on the next launch — a far
  // worse bug than the one it fixed. The rotation cause itself is fixed at
  // source (an opaque bearer no longer becomes the user id), so nothing new can
  // be stranded, and there is no production data to rescue. If a legacy repair
  // is ever needed again it MUST exclude `app:<user>:<project>` keys.

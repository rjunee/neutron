/**
 * @neutronai/gbrain-memory — agent tool surface (`gbrain_search`).
 *
 * P0-2 — native memory RECALL. Scribe WRITES entities + extracted facts into
 * the GBrain page index on every chat turn (`scribe/write-to-gbrain.ts` →
 * `GBrainSyncHook` → `MemoryStore.add` → GBrain `put_page`). Until now the
 * spawned agent had NO tool to read any of it back: the only agent-facing
 * search tools were `doc_search` (project markdown) and `message_search` (raw
 * chat history) — neither covers the entity / company / project / fast-fact
 * drawers the scribe populates. This registers `gbrain_search` so the live
 * agent can recall what it already knows about a person / company / topic
 * mid-reasoning, closing the write→read asymmetry the lift audit flagged.
 *
 ...
 *
 * Backed by the SAME `GBrainMemoryStore.query` the admin "Memory" tab uses, so
 * the write path (scribe) and the read path (this tool) share one index. The
 * widening "beyond project docs to a vault-wide search" the audit asks for is
 * the corpus itself: GBrain holds the entity pages (people/companies/projects/
 * meetings/concepts/originals) + scribe facts, a different corpus than
 * `doc_search`'s project files.
 *
 * Read-only; gates on `read:memory`. A host without the `gbrain` binary
 * degrades to an empty result rather than a broken tool (mirrors the admin
 * Memory tab's best-effort read). Results are plain JSON the registry
...
const gbrainSearchInputSchema: JsonSchemaDocument = {
  type: 'object',
  properties: {
    query: {
      type: 'string',
      description:
        ...
        'recently updated memory pages.',
    },
    limit: {
      type: 'integer',
      minimum: 1,
      maximum: MAX_LIMIT,
      description: 'Max memory pages to return (default 10).',
    },
  },
  required: ['query'],
...
const gbrainSearchOutputSchema: JsonSchemaDocument = {
  type: 'object',
  properties: {
    results: {
      type: 'array',
...
interface GBrainSearchArgs {
  query?: unknown
  limit?: unknown
}

/**
 * Register `gbrain_search` against `registry`, backed by the owner's
 * `MemoryStore` (the same `GBrainMemoryStore` the admin Memory tab + the scribe
 * sync hook share). Returns the registered tool name.
 */
export function registerGBrainSearchToolSurface(
  registry: ToolRegistry,
  store: MemoryStore,
): string[] {
  registry.register({
    name: GBRAIN_SEARCH_TOOL,
    description:
      ...
      '— and return the most relevant memory pages, ranked, with a matching excerpt. ' +
    ...
    input_schema: gbrainSearchInputSchema,
    output_schema: gbrainSearchOutputSchema,
    capability_required: 'read:memory',
    approval_policy: 'auto',
    handler: async (args) => {
      const a = (args ?? {}) as GBrainSearchArgs
      ...
      try {
        ...
        // A host without the `gbrain` binary degrades to "no memory" rather
        // than a broken tool — mirror the admin Memory tab's best-effort read.

...
describe('WebWorkBoardClient', () => {
  ...
  // but the HTTP surface keys General on the reserved '~general' id and 400s on an
...
describe('parseWorkBoardItems', () => {
  it('preserves task progress and leaves malformed or absent totals unknown', () => {
    for (const [wire, expected] of [[15, 15], [null, null], [undefined, null], ['15', null], [0, null], [-1, null], [1.5, null], [Infinity, null], [NaN, null]] as const) {
      const parsed = parseWorkBoardItems([{ ...row(), run_progress: {
  ...
  it('validates current strategies and maps legacy mode only when explicitly present', () => {
    const current = parseWorkBoardItems([{ ...row(), run_progress: {
    ...
    const legacyTask = parseWorkBoardItems([{ ...row(), run_progress: {
    ...
    const legacySingle = parseWorkBoardItems([{ ...row(), run_progress: {
      run_id: 'legacy-single', phase_label: 'building', ralph: false,
    } }])[0]!.run_progress!
    expect(legacySingle.execution_strategy).toBe('single')

    const absent = parseWorkBoardItems([{ ...row(), run_progress: {
    ...
    expect(parseWorkBoardItems([{ ...row(), run_progress: {
  ...
  it('parses run_progress.resume_note through, and null when absent or not a string', () => {
    const base = {
      run_id: 'run-1', phase_label: 'building', round: 1, execution_strategy: 'single', task_iteration: 0,

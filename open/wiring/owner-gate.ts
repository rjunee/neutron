 ...
 *   1. `coldStartRedirect(req)` — mints a fresh owner cookie + one-shot local
 ...
 * `claimAndMintThenServe(startToken, sourceRes, req)` helper below. Both former
...
import { shouldSetSecureCookie } from '@neutronai/gateway/http/app-ws-surface.ts'
 ...
 *
 * The decision comes from {@link shouldSetSecureCookie}, NOT from the request
 * URL's own protocol (ISSUES #303). A hosted install sits behind a reverse
 * proxy that terminates TLS and forwards over plain-HTTP loopback — so
 * `url.protocol` is `http:` for a request the browser genuinely made over
 * HTTPS, and the owner session cookie was being minted WITHOUT `Secure` on
 * every such deployment.
 * That let the cookie ride a plain-http downgrade request. Honouring
 * ISSUES #392 then INVERTED the polarity: `Secure` is the default and is
 * dropped only for a demonstrably plain-http loopback dev session, so a
 * client-supplied `X-Forwarded-Proto: http` can no longer strip it.
 */
...
export function buildOpenOwnerGate(
  ...
  const coldStartRedirect = (req: Request): Response => {
    const token = startTokenAuth.mint({ project_slug, user_id: OWNER_USER_ID })
    const headers = new Headers({
      location: `/chat?start=${encodeURIComponent(token)}`,
    })
    headers.append('set-cookie', formatOwnerSetCookie(project_slug, cookieSecret, req))
  ...
  const openFetch = (
    ...
    if (isGet && url.pathname === '/chat' && !hasStart && hasValidCookie) {
      return hasResumableState().then((resumable) =>
        resumable ? withReactBootstrap(landing.fetch(req, server)) : coldStartRedirect(req),
      )
    }
    ...
    if (isGet && isSpaClientRoute(url.pathname, req.method)) {
      ...
      if (!hasValidCookie) {
        ...
        return claimAndMintThenServe(startToken, spaRes, req)
      }
    ...
    if (isGet && url.pathname === '/chat' && !hasValidCookie) {
      const startToken = url.searchParams.get('start')
      return claimAndMintThenServe(startToken, res, req)
    }
  ...
  const gate: HttpGate = {
    apply(req, server, next): Promise<Response> {
      const url = new URL(req.url)
      // ISSUES #367 — bare `GET /` MUST reach `openFetch`. `isLandingRoute('/',
      // 'GET', false)` is deliberately false (the landing page is invite-gated),
      // and no other predicate matches `/`, so the root fell through to `next()`
      // and the instance answered 404. Typing your own instance URL said "Not
      // Found"; only `/chat` worked.
      //
      // The handler this unblocks was never missing — `openFetch`'s `/` branch
      // (valid cookie → 302 /chat, else cold-start) has been sitting at :380
      // this whole time, provably dead. That is why the original issue's
      // root-cause guess (`landing/server.ts`) was wrong: the bug is the
      // predicate that never routes into the handler, not the handler.
      const isBareRoot = req.method === 'GET' && url.pathname === '/'

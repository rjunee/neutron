...
import { FileClientReportStore } from '@neutronai/gateway/diagnostics/client-report-store.ts'
...
export function buildOpenGraphComposer(
  ...
  return async ({ db, project_slug, slug_is_fallback }): Promise<OpenComposition> => {
    ...
    // NATIVELY via the built-in `Skill` mechanism — the same loader the legacy harness's
    ...
    // adhoc → a one-shot agent) that mirrors the legacy harness's `spawn-agent.sh`. It is
    ...
    // ── Skill-forge → Open boot (the legacy harness parity gap #5) ──────────────────────
    ...
    // ── Free Cores → Open boot (the legacy harness parity gap #2) ───────────────────────
    ...
    //
    // LOGIN-FIRST (2026-07-25) — the third credential. A remote client (the
    // Expo app) has no way to learn `appWsToken`: it signs in at an identity
    // service and is handed an RS256 bearer scoped to ONE instance slug.
    // Without a verifier for it, sign-in succeeded and then EVERY authenticated
    // request 401'd. `NEUTRON_IDENTITY_JWKS_URL` names that service's published
    // JWKS, which switches the base resolver into `jwks` mode
    // (`channels/adapters/app-ws/auth.ts:resolveJwks`) — offline RS256
    // verification, no shared secret, plus a constant-time check that the
    // token's `slug` claim names THIS instance. Unset on a self-host install,
    // where the owner bearer + the dev lanes are the whole story.
    const identityJwksUrl = readIdentityJwksUrl(env['NEUTRON_IDENTITY_JWKS_URL'])
    const identityAudience = readNonEmpty(env['NEUTRON_IDENTITY_AUDIENCE'])
    // Optional `iss` pin. Both this and the audience are opt-in because their
    // values are one deployment's vocabulary, which this public tree does not
    // carry; when set they narrow which minted tokens this instance will take.
    const identityIssuer = readNonEmpty(env['NEUTRON_IDENTITY_ISSUER'])
    const appOwnerAuth: AppWsAuthResolver = ((): AppWsAuthResolver => {
      const base = createAppWsAuthResolver({
        project_slug,
        bypass: bindIsLoopback,
        ...(identityJwksUrl === undefined ? {} : { jwks_url: identityJwksUrl }),
        ...(identityAudience === undefined ? {} : { jwks_audience: identityAudience }),
        ...(identityIssuer === undefined ? {} : { jwks_issuer: identityIssuer }),
      })
      return {
        mode: base.mode,
        resolve: async (token) => {
          ...
          // A JWKS-verified bearer was minted by the identity service for the
          // owner of the slug it carries, and the resolver already
          // constant-time-checked that slug against THIS gateway's slug. Open is
          // single-owner, so that bearer IS the owner — normalise the identity
          // service's account id onto the local owner identity. Everything
          // downstream (the WS channel topic, the owner-timezone write, every
          // /api/app/* gate) compares against OWNER_USER_ID, so returning the
          // remote account id instead would authenticate the owner and then
          // deny them, and would fork the chat transcript per account id.
    ...
    //
    // App remote diagnostics — the SAME surface also owns
    // `/api/app/admin/diagnostics/reports`, where the owner's mobile app
    // delivers its own JS error reports so a phone failure is diagnosable
    // WITHOUT a USB cable and without any third-party service. Reports land in
    // `<owner_home>/diagnostics/client-reports.jsonl`, bounded by record count,
    // redacted of anything credential-shaped on arrival.
    const appDiagnosticsSurface = createAppDiagnosticsSurface({
      auth: appOwnerAuth,
      project_slug,
      client_reports: new FileClientReportStore({
        path: joinPath(owner_home, 'diagnostics', 'client-reports.jsonl'),
      }),
      diagnostics: () =>
        composeDiagnostics(
    ...
    const appWsChatTurn =
      liveAgentSubstrate !== null
        ? buildLiveAgentTurn({
            ...
            // On-demand Claude reconnect (2026-07-24) — mint a FRESH install-token
            // command by driving the SAME `installTokenHandler` the first-time auth
            // gate drives (Open's handler, or a Managed-injected one), so the owner's
            // tap of the auth-reconnect "Reconnect" button reuses the same signup_id →
            // capture-token → restart flow. The minted command is NOT byte-for-byte the
            // onboarding one: the gate mints against the real browser request (public
            // `X-Forwarded-*` origin) whereas this mints at a LOOPBACK origin
            // (`127.0.0.1:<port>`). That is intended — the bubble tells the owner to run
            // the command IN A SHELL on the machine running Neutron, and the box reaches
            // its own listener over loopback — but it means only someone with a shell on
            // the box can run it (a shell-less remote / hosted-deployment owner is a
            // tracked limitation; see `buildReconnectHandoff`). Port = the configured
            // NEUTRON_PORT else `resolveListenPort`'s default (the bound port for the
            // deployments that set NEUTRON_PORT env; a `--port=` argv / free-port dev run
            // is the only unshipped gap). The seam + closure are unit-tested against a
            // real handler in `open/__tests__/install-token-handoff.test.ts`.
            reconnectHandoff: buildReconnectHandoff(
              installTokenHandler,
              options.config?.port ?? DEFAULT_LISTEN_PORT,
            ),
            buttonStore: landing.buttonStore,
            project_slug,
            owner_home,
    ...
    return {
      ...
      host_deploy: {
        ...
        install: hostDeployInstall,
        // WHERE THE BUTTON GOES WHEN THE CALL CARRIED NO TOPIC. A warm-REPL
        // agent has no bound `TopicContext` (`mcp/server.ts:279`), so every tool
        // it calls arrives `topic_id: null` and the prompt fell back to General —
        // the owner told to tap in a conversation he was not in, which is the
        // exact failure the calling-topic change was written to end. The sink
        // DOES thread `project_id` (that is how `work_board_*` reaches the right
        // board), so the topic is recoverable from it.
        //
        // The SAME validated resolver the reminder surface uses, not a second
        // copy of the rule: an id that is not one of this owner's live projects
        // downgrades to General rather than fabricating a destination, and the
        // downgrade is reported.
        resolveProjectTopic: (project_id: string): string =>
          resolveAppWsReminderTopic(`app-project:${project_id}`),
      },
...
/** Trim an optional env var to `undefined` when it carries nothing usable. */
...
/**
 * Resolve `NEUTRON_IDENTITY_JWKS_URL` — the identity service's JWKS endpoint,
 * which switches the app-ws resolver into RS256/`jwks` mode.
 *
 * FAIL LOUD, NOT SILENT. This value decides whether a remote client can
 * authenticate at all, so a typo must not degrade to "identity quietly off"
 * (the operator would see sign-in succeed and every request 401, which is the
 * exact confusing failure this whole path exists to remove). An unparseable or
 * non-http(s) value throws at composition; only an absent/blank var means
 * "no identity service", which is the normal self-host case.
 */
function readIdentityJwksUrl(raw: string | undefined): string | undefined {
  ...
  try {
    ...
    throw new Error(
      `NEUTRON_IDENTITY_JWKS_URL is not a valid absolute URL: ${JSON.stringify(value)}`,
    )
  }
  if (parsed.protocol !== 'https:' && parsed.protocol !== 'http:') {
    throw new Error(
      `NEUTRON_IDENTITY_JWKS_URL must be http(s), got ${JSON.stringify(parsed.protocol)}`,
    )
  }
  return value
}

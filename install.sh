...
# ── clone-origin guard (ISSUES #316) ────────────────────────────────────────
# A re-run over an EXISTING checkout used to `git pull` that checkout's own
# origin, whatever it happened to be. Ryan hit this for real on 2026-06-20: an
# earlier test install had cloned `~/neutron/core` from a LOCAL path, so the
# canonical `curl https://neutronagent.ai/install.sh | sh` "succeeded" while
# pulling local code and never touching the public repo at all. The install
# printed nothing unusual — it is silent by construction, which is what makes
# it worth a guard rather than a doc note.

# Reduce a git remote URL to a comparable form: no scheme, no trailing `.git`
# or `/`, `git@host:path` and `ssh://git@host/path` folded to `host/path`,
# lowercased. Without this, `git@github.com:rjunee/neutron.git` and
# `https://github.com/rjunee/neutron` would read as different origins and the
# guard would block legitimate installs — a false positive here is worse than
# the bug, because it breaks people who did nothing wrong.
normalise_git_url() {
  _u=$1
  case "$_u" in
    git@*:*)      _u="https://$(printf '%s' "$_u" | sed 's|^git@||; s|:|/|')" ;;
    ssh://git@*)  _u="https://${_u#ssh://git@}" ;;
  esac
  _u=${_u#https://}
  _u=${_u#http://}
  _u=${_u%/}
  _u=${_u%.git}
  _u=${_u%/}
  printf '%s' "$_u" | tr '[:upper:]' '[:lower:]'
}

# Refuse to update a checkout that points somewhere other than $NEUTRON_REPO.
# Deliberately fails rather than re-pointing: fixing it in place would mean
# `remote set-url` + `fetch` + `reset --hard`, which silently discards whatever
# the user had in that directory. An install script should not destroy a
# working tree to repair its own assumption. Anyone who WANTS a different
# source can say so with NEUTRON_REPO, and then the origins match.
assert_clone_origin() {
  _dir=$1
  _want=$2
  _have=$(git -C "$_dir" remote get-url origin 2>/dev/null || true)
  if [ -z "$_have" ]; then
    die "$_dir is a git checkout with no 'origin' remote, so there is nothing to update from. Remove it and re-run: rm -rf $_dir"
  fi
  if [ "$(normalise_git_url "$_have")" != "$(normalise_git_url "$_want")" ]; then
    die "$(printf '%s\n' \
      "$_dir already exists, but its git origin is not the repo this installer installs from." \
      "  found:    $_have" \
      "  expected: $_want" \
      "Updating it would install code from the wrong source (ISSUES #316). Pick one:" \
      "  remove it and get a clean clone   rm -rf $_dir" \
      "  re-point it yourself              git -C $_dir remote set-url origin $_want" \
      "  install from that origin on purpose   NEUTRON_REPO=$_have sh install.sh")"
  fi
}
...
ensure_codex() {
  ...
  # prefers `brew install codex` (macOS + Linuxbrew, the the legacy harness pattern) and falls
...
# Test seam (ISSUES #316) — run the clone-origin guard in isolation against an
# arbitrary directory and exit, mirroring the seams above. Drives the REAL
# `assert_clone_origin`, not a reimplementation of it, so a test can prove both
# that a foreign origin aborts AND that equivalent URL spellings still pass.
if [ -n "${NEUTRON_INSTALL_CHECK_ORIGIN:-}" ]; then
  assert_clone_origin "$NEUTRON_INSTALL_CHECK_ORIGIN" "$NEUTRON_REPO"
  printf 'origin_ok=%s\n' "$NEUTRON_INSTALL_CHECK_ORIGIN"
  exit 0
fi
...
if [ "$MODE" = "local" ]; then
  ...
  if [ -d "$CLONE_DIR/.git" ]; then
    # Check WHERE it points before pulling — a pull against a foreign origin is

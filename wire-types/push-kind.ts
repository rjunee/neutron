...
export const PUSH_KINDS = [
   ...
   *
   * ⚠️ HAS A SENDER, BUT NOTHING CURRENTLY REACHES IT.
   */
  'calendar_pre_meeting_brief',
  /**
   * The Email Core's daily triage. Carries `project_id`.
   *
   * ⚠️ RETAINED FOR OLD NOTIFICATION ROUTING. The daily digest sender was deleted;
   * this value remains so previously emitted notifications still deep-link.
   *
   * It is LISTED ANYWAY so the resolver keeps handling old payloads. Removing it
   * would silently reintroduce the disjoint-lists defect this file exists to
   * prevent. The exhaustiveness test therefore proves backward-compatible
   * routing, not that a notification is currently being sent.
   */
  'email_daily_triage',
] as const

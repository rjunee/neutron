 ...
 * EXPLICIT drop ("drop Wellness-lab") still subtracts exactly that one.
 *
...
describe('GAP1 import-analysis additive merge (retained post-turn extractor path)', () => {
  test('a confirm whose extraction is SHORTER keeps the full seeded list (no silent shrink → all shells)', () => {
    ...
    // Buddhism + Wellness-lab (the 7→3 regression). The additive union with the
    // seeded 7 keeps all 7 → all 7 get shells downstream.
    const primary = mergedPrimary(
      priorWithSeven(),
      { primary_projects: ['Topline', 'Northwind', 'Acme'] },
      'go with Topline, Northwind, Acme, Buddhism and Wellness-lab',
    )
    expect(primary.length).toBe(7)
    for (const p of SEVEN) expect(primary).toContain(p)
  })

  test('an explicit-removal ("drop Wellness-lab") still drops exactly that one', () => {
    const remaining = SEVEN.filter((p) => p !== 'Wellness-lab')
    const primary = mergedPrimary(
      priorWithSeven(),
      { removed_projects: ['Wellness-lab'] },
      'looks good but drop Wellness-lab',
    )
    expect(primary).not.toContain('Wellness-lab')
  ...
  test('a review-completing REMOVAL drops exactly the named project (no silent re-add → no shell) — Argus r3 BLOCKER', () => {
    // Realistic flow the r3 additive-union regressed: "drop Wellness-lab, the
    // rest are good, go ahead" restates the kept 6 (anchored to the proposed
    // list) AND names removed=['Wellness-lab']. WITHOUT the additive-minus-removal
    // rule the union re-adds Wellness-lab from the seeded prior-7 and it would get
    // a shell.
    const kept = SEVEN.filter((p) => p !== 'Wellness-lab')
    ...
    const patch = buildPhaseStatePatch(
      priorWithSeven(),
      fields,
      'looks great, just drop Wellness-lab and go ahead',
    )
    ...
    expect(patch['dropped_projects']).toEqual(['Wellness-lab'])
  })

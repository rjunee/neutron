 ...
 * the user never edits it. This is the Neutron-Open port of the legacy harness's
 ...
 *
 * Two Neutron divergences from the legacy harness (intentional):
 *   1. `[context:<path>]` resolves relative to the PROJECT REPO ROOT
 *      (`Projects/<slug>/`), not LEGACY_HOME — each project owns its context
 *      files.
 ...
 *
 * The `[context:]` HARD GATE is preserved verbatim from the legacy harness: a queued item
 ...
 * and files over the 64 KB cap. Verbatim port of the legacy harness's `resolveContextFile`

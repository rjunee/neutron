...
describe('synthesizeProjectContext', () => {
  ...
  test('GAP2: unmatched project draws content from cross-project import signal', () => {
    ...
    ir.topics = [{ name: 'Wellness-lab cold plunge', recurrence_score: 0.8, recency_score: 0.9 }]
    ir.inferred_interests = [{ name: 'wellness-lab', basis: 'mentioned often' }]
    const ctx = synthesizeProjectContext({ name: 'Wellness-lab' }, ir)
    expect(ctx).not.toContain("don't have history")
    expect(ctx.toLowerCase()).toContain('wellness-lab')
    expect(ctx).toContain('Wellness-lab') // names the project
  })
...
describe('findRelatedImportSignal (GAP2)', () => {
  ...
  test('dedups case-insensitively and caps each list', () => {
    ...
    ir.topics = Array.from({ length: 10 }, (_, i) => ({
      name: `wellness-lab topic ${i}`,
      recurrence_score: 0.5,
      recency_score: 0.5,
    }))
    const sig = findRelatedImportSignal('Wellness-lab', ir)
...
describe('weaveRelatedSignal (GAP2)', () => {
  ...
  test('weaves topics, entities, interests into a named paragraph', () => {
    const woven = weaveRelatedSignal('Wellness-lab', {
      entities: ['Whoop band'],
      topics: ['cold plunge protocol'],
      interests: ['wellness-lab'],
    })
    expect(woven).toContain('Wellness-lab')

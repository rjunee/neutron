...
if ! git show "$MAIN_REF:$INVENTORY_REL" > "$MAIN_INVENTORY" 2>/dev/null; then
  ...
  shallow="$(read_shallowness)" || exit 2
  if [ "$shallow" = "true" ]; then
    echo "route-slot-ratchet-guard: the checkout is SHALLOW (git reports shallow history), so $MAIN_REF's history is not available — this is a clone-depth problem, not a missing baseline. Run: git fetch --unshallow origin" >&2
  fi

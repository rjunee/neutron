...
function runGuard(repo: string, mainRef: string): { code: number; out: string } {
  try {
    const out = execFileSync('bash', [GUARD_SH], {
      ...
      env: {
        ...process.env,
        COMPOSITION_FIELD_RATCHET_ROOT: repo,
        COMPOSITION_FIELD_RATCHET_MAIN_REF: mainRef,
...
function originWithCompositionInventory(): { root: string; origin: string } {
  const root = mkdtempSync(join(tmpdir(), 'composition-field-ratchet-origin-'))
  ...
  const inventory = join(
    ...
    'open',
    '__tests__',
    'composition-field-coverage-inventory.ts',
  )
  const reader = join(seed, 'open', '__tests__', 'declared-composition-fields.ts')
  ...
  writeInventory(inventory, ['push_dispatcher', 'work_board'], [])
  writeReader(reader, ['push_dispatcher', 'work_board'])
  git(seed, 'add', '-A')
...
describe('composition-field ratchet guard (shell, against a throwaway git repo)', () => {
  test('the freshen never shallows a full clone', () => {
    const { root, origin } = originWithCompositionInventory()

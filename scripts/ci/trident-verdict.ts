...
/**
 * A home-directory absolute path, in any supported platform shape.
 *
 * WHY A VERDICT IS SCREENED FOR THIS. The verdict is free text posted publicly on
 * a public repository, and the account segment can identify a person. The leak
 * gate covers FILES and COMMIT MESSAGES; a PR comment is
  ...
  if (HOMEDIR_PATH.test(value)) {
    throw new VerdictError(
      `${key} contains an absolute path under a home directory (not quoted here — this log is ` +
        ...
        'verdict. This refuses supported home-directory path shapes, including CI runner paths ' +
        'where there is no account name ' +

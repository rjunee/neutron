...
describe('G8 leak-gate — planted findings FAIL', () => {
  test('a retired multi-tenant CODE token trips tenant-code', () => {
    const dir = freshTree()
    try {
      // A live-surface multi-tenant identifier that must never re-enter the tree.
      writeFileSync(join(dir, 'src', 'db.ts'), 'export const key = tenant_slug\n')
      const { code, out } = runGate(dir)
      expect(out).toContain('[tenant-code]')
  ...
  test('multi-tenant PROSE in a comment trips the Tier-2 prose rule', () => {
    const dir = freshTree()
    try {
      writeFileSync(join(dir, 'src', 'note.ts'), '// each tenant gets an isolated db\nexport const x = 1\n')
      const { code, out } = runGate(dir)
      expect(out).toContain('[tenant-word]')
      expect(code).toBe(1)
    } finally {
      rmSync(dir, { recursive: true, force: true })
    }
  ...
  test('the hosted domain neutron.computer trips the self-host-only rule', () => {
    const dir = freshTree()
    try {
      writeFileSync(join(dir, 'src', 'url.ts'), 'export const host = "https://app.neutron.computer"\n')
      const { code, out } = runGate(dir)
  ...
  test('a NUL-carrying source (token hidden from grep) trips binary-hidden', () => {
    ...
    try {
      ...
      writeFileSync(join(dir, 'src', 'sneaky.ts'), 'const k = "a\x00tenant_slug"\n')
      const { code, out } = runGate(dir)
  ...
  test('summary tallies multiple planted findings at once', () => {
    ...
    try {
      writeFileSync(join(dir, 'src', 'db.ts'), 'export const key = tenant_slug\n')
      writeFileSync(join(dir, 'src', 'url.ts'), 'export const host = "neutron.computer"\n')
      const { code, out } = runGate(dir)
      expect(out).toContain('[tenant-code]')
      expect(out).toContain('[neutron-computer]')

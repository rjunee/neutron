...
describe('a verdict may not carry a home-directory absolute path', () => {
  const HOME_PATHS = [
    ['', 'Users', 'account', 'repos', 'project', 'open', 'composer.ts:11'].join('/'),
    ['', 'home', 'account', 'src', 'thing.ts'].join('/'),
    ['C:', 'Users', 'account', 'repos', 'thing.ts'].join('\\'),
  ]
  ...
  test('the refusal does NOT echo the path — this check log is public too', async () => {
    ...
    expect(h.output()).not.toContain(['', 'Users', 'account'].join('/'))
  })
  ...
  test('a path-shaped word that is not a home directory is not caught', () => {
    ...
    for (const ok of ['/usr/bin/bun', 'open/composer.ts', 'a/home/b.ts', ['', 'homework', 'notes.md'].join('/')]) {
      expect(() =>
  ...
  test('and a non-integer count carrying a home path is redacted too, not just the syntax path', async () => {
    ...
    const body = verdictBlock(HEAD, { 'codex.blocking': ['', 'Users', 'account', 'notes.md'].join('/') })
    const h = harness({ comments: [body] })
    expect(await runGate(h.deps)).toBe(1)
    expect(h.output()).not.toContain(['', 'Users', 'account', 'notes.md'].join('/'))
  })
})

...
function runRunTests(h: Harness, extraEnv: Record<string, string> = {}): RunResult {
  try {
    const out = execFileSync('bash', ['-c', 'bash "$1" 2>&1', 'bash', RUN_TESTS], {
      ...
      // chunk math and file counts, so any inherited NEUTRON_TEST_* knob silently
      // changes the thing under test. It bit for real when CI began sharding:
      // the runner exports NEUTRON_TEST_SHARD=<i>/4, the child run-tests.sh
      // inherited it, and a 5-file fixture was sliced to 1 file — three of these
      // tests failed on shard 4/4 only, which reads like flakiness rather than
      // configuration. Strip the whole prefix rather than the one variable that
      // caused today's failure; the next knob CI sets would land the same way.
      env: {
        ...Object.fromEntries(

...
describe('findDuplicateEntryHeadings', () => {
  ...
  it('agrees with the merge driver on a tab-separated heading', () => {
    // `startsWith('## ')` requires a space; the driver's `/^##[ \t]+\S/` accepts a
    // tab too, as CommonMark does. The driver would merge these as one entry twice
    // over; the gate saw none. This case is also why that regex is not the narrower
    // `/^## /` — under it this fixture parses as ZERO entries and the gate reports
    // clean, which is the silent direction. See `HEADING` in the driver's parser.
    const log = '##\ta — one\n\nx\n\n##\ta — one\n\ny\n'
